...
public class EngineService implements DisposableBean {
	...
	public void logout(String id, String password) throws GeneralException {
		...
		if (engine.getStatus() == EngineStatus.SERVING || engine.getStatus() == EngineStatus.IDLE
	...
	public boolean isLogin(String id, String password) throws GeneralException {
		...
		return status != null && status != EngineStatus.LOST && status != EngineStatus.UNHEALTHY
	...
	public String heartbeat(String id, String password, Date time, double speed) throws GeneralException {
		if (isLogin(id, password)) {
			...
			save(engine);
			_logger.info(String.format("{%s} %s as %s heartbeat, speed %f", time.toString(), id, engine.getEngineRole(),
					speed));
		} else
	...
	public List<Engine> getIdleEngines() {
		return ((List<Engine>) _pm.getObjectsForClass("Engine")).stream()
				.filter(e -> e.getStatus() == EngineStatus.IDLE).collect(Collectors.toList());
	}
	...
	private class HeartbeatChecker {
		...
		private TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				for (Engine e : getActiveEngines()) {
					if (e.getLastHeartbeatTime() == null)
						continue;
		...
		public void start(long heartBeatRate) {
			timer.schedule(task, heartBeatRate, heartBeatRate); // FIXME:
																// 2016/1/24
																// magic number
																// heartbeat
																// rate
	...
	public List<Engine> getActiveEngines() {
		return getAllEngines().stream().filter(e -> e.getStatus() == EngineStatus.SERVING)
