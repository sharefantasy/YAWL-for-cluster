...
public class TenantService {
	...
	public Tenant getTesterTenant(String testName, int engineNum) {
		...
		// tenant.setEngineList(roleService.generateRoleToTenant(tenant,
		// engineNum));
	...
	public long violatedTime(Tenant tenant) {
		...
		// TODO: 2016/2/21 method to calculate tenant violation time and
		// violation rate.
		// historySpeed.forEach((Date d, Double s)->{
		// if (s < SLOspeed) violated[0] +=PERIOD; //magic method .5000 is the
		// period of report.
		// });
		// return violated[0];
		return 0;
	...
	public Tenant updateSpeed(Tenant tenant) {
		if (tenant == null)
			return null;
		double speed = tenant.getEngineList().stream().map(EngineRole::getCurrentSpeed).reduce((double) 0,
				(a, b) -> a + b);
