...
public class SimpleConfigurationAdapter {
	...
	public void loadonfig(Collection<Tenant> tenants, Collection<Host> hosts) {
		...
		engineRoleList = tenants.stream().map(Tenant::getEngineList).reduce(new ArrayList<>(), (e1, e2) -> {
			e1.addAll(e2);
			return e1;
		});
		capabilitySet = getCapabilityset();
		scheduler = new BrutalScheduler(this.tenants, this.hosts, engineRoleList, capabilitySet);
	}
		...
		for (int i = 0; i < tenants.size(); i++) {
			...
			for (int j = 0; j < hosts.size(); j++) {
				if (diff[i][j] > 0) {
					...
					engineRoleList.stream().filter(e -> e.getTenant().getId() == tid)
							.filter(e -> e.getHost().getId() == hid).limit(diff[i][j]).forEach(roles::add);
				}
			}
			Host[] hostA = (Host[]) hosts.stream().sorted((o1, o2) -> (int) Math.signum(o1.getId() - o2.getId()))
					.toArray();
			for (int j = 0; j < hosts.size(); j++) {
				if (diff[i][j] < 0) {
