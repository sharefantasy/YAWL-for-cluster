...
public class TestPlanService {
    ...
    private boolean initTest(TestPlanEntity testPlan) {
        ...
        Host tester = testPlan.getHost();
        List<Engine> idleEngines = engineService.getIdleEngines().stream()
                .collect(Collectors.toList());
        List<Engine> engineOnTester = tester.getEngineList().stream()
                .map(EngineRole::getEngine)
                .collect(Collectors.toList());
        ...
        tester.getEngineList().stream()
                .filter(e -> e.getEngine() != null)
                .forEach(e -> e.getEngine()
        ...
        tester.getEngineList().stream()
                .filter(e -> e.getEngine() != null)
                .forEach(e -> e.getEngine()
                        .setStatus(EngineStatus.TESTING));
        return true;
    }

    private boolean roleSettingOperation(TestPlanEntity testPlan, Host tester, List<Engine> idleEngines, List<Engine> engineOnTester) {
        ...
        if (!finished) {
            if (engineQ.isEmpty()) {
                ...
                if (!finished) {
                    _logger.error(String.format("Testplan: %d can't init. Too many engine role faults", testPlan.getId()));
                    return true;
                }
            } else {
                _logger.error(String.format("Testplan: %d can't init. Not enough available engines", testPlan.getId()));
