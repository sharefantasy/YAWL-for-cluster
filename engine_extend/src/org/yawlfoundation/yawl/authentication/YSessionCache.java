...
/**
 * An extended Hashtable that manages connections to the engine from custom
...
public class YSessionCache extends ConcurrentHashMap<String, YSession> implements ISessionCache {
	...
	/**
	 * Creates and stores a new session between the the Engine and a custom
	 * service or external application.
	 * 
	 * @param name
	 *            the username of the external client
	 * @param password
	 *            the corresponding (hashed) password
	 * @param timeOutSeconds
	 *            the maximum idle time for this session (in seconds). A value
	 *            of 0 will default to 60 minutes; a value less than zero means
	 *            this session will never timeout.
	...
	public String connect(String name, String password, long timeOutSeconds) {
		...
		if (client != null) {
			if (validateCredentials(client, password)) {
				...
				// (an 'admin' is enabled check has already been done in
				// EngineGatewayImpl)
			...
			if (service != null) {
				...
				result = unknownUser(name);
		}
		return result;
	}

	/**
	 * Checks that a session handle represents an active session. If it does,
	 * the session idle timer is restarted also.
	 * 
	 * @param handle
	...
	/**
	 * Checks that a particular custom service has an active session with the
	 * Engine.
	 * 
	 * @param uri
	...
	/**
	 * Checks that a particular external client has an active session with the
	 * Engine.
	 * 
	 * @param client
	 *            the client.
	 ...
	 * @param handle
	 *            a session handle.
	 * @return the session object associated with the handle, or null if the
	 *         handle is invalid or inactive.
	 */
	 ...
	 *            from the Engine. Also writes the disconnection to the session
	 *            audit log.
	 */
	...
	/**
	 * Called when the hosting server shuts down to write a shutdown record for
	 * each active session to the audit log.
	 */
