...
public class DocumentStore extends YHttpServlet {
	...
	public void init() {
		...
		_sessions.setupInterfaceA(context.getInitParameter("InterfaceA_Backend"),
				context.getInitParameter("EngineLogonUserName"), context.getInitParameter("EngineLogonPassword"));
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException {
		try {
			...
			DataInputStream dis = new DataInputStream(new BufferedInputStream(req.getInputStream()));
			...
			if (action == null) {
				...
				if (action.equals("get")) {
					...
					if (_retainWhenCaseCompletes) {
						writeString(res, "Documents not cleared: configured to retain on case completion", "failure");
					} else
						result = clearCase(caseID);
				}
			} else
				writeString(res, "Invalid or disconnected session handle", "failure");
	 ...
	 *            the response
	 * @param doc
	 *            a YDocument wrapper containing the binary file
	 * @throws IOException
	 *             if there's a problem writing to the stream
	 */
	 ...
	 * @param tag
	 *            the xml tag to wrap the message in
	 * @throws IOException
	 *             if there's a problem writing to the stream
	 */
	private void writeString(HttpServletResponse res, String msg, String tag) throws IOException {
		if (msg != null) {
	 ...
	 * @throws IOException
	 *             if no document can be found with the id passed
	 */
	 ...
	 *
	 * @param doc
	 *            a YDocument wrapper for the document to write
	 * @return the id (primary key) of the stored document
	 * @throws IOException
	 *             if the document can't be read from the request stream
	 */
	 ...
	 *
	 * @param id
	 *            the case id to remove documents for
	...
	private String clearCase(String id) {
		...
		if (rowsDeleted > -1) {
			sb.append(rowsDeleted).append(" document").append(rowsDeleted > 1 ? "s " : " ").append("removed for case: ")
					.append(id);
		} else {
	...
	/**
	 * Increases the maximum column length for stored documents in H2 databases
	 * from 255 to 5Mb.
	 * <p/>
	 * H2 defaults to 255 chars for binary types, and this default went out with
	 * the 2.3 release. This method (1) checks if we are using a H2 database,
	 * then if so (2) checks the relevant column length, then if it is 255 (3)
	 * increases it to 5Mb maximum and (4) writes a flag file so that the method
	 * short-circuits on future executions (i.e. it only runs once).
	 *
	 * @param context
	...
	private void fixH2BinarySize(ServletContext context) {
		...
		if (p == null)
			return; // couldn't find hibernate.properties
				...
				if (connection != null) {
					...
					if (rs.getInt("COLUMN_SIZE") <= 255) {
						connection.createStatement()
								.executeUpdate("alter TABLE ydocument alter COLUMN ydoc varbinary(5242880)");
	...
	private Properties loadHibernateProperties(ServletContext context) {
		try {
			...
			// for enterprise builds, hibernate.properties is in the 'classes'
			// dir
