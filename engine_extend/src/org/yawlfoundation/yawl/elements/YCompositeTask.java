...
/**
 * 
 * A YCompositeTask object is the executable equivalent of the YCompositeTask in
 * the YAWL paper. It has the same properties and behaviour.
 * 
...
public final class YCompositeTask extends YTask {
	 ...
	 * 
	 * @param pmgr
	 *            a valid persistence manager instance
	 * @param id
	 *            the task identifier
	...
	protected synchronized void startOne(YPersistenceManager pmgr, YIdentifier id)
		...
		YNetRunner netRunner = new YNetRunner(pmgr, (YNet) _decompositionPrototype, this, id, getData(id));
			...
			for (YIdentifier identifier : _mi_active.getIdentifiers()) {
				...
				if (netRunner != null) {
					...
					for (YWorkItem item : getWorkItemRepository().cancelNet(identifier)) {
						item.cancel(pmgr);
						YEventLogger.getInstance().logWorkItemEvent(pmgr, item, YWorkItemStatus.statusDeleted, null);
					}
	...
	private void logTaskStart(YPersistenceManager pmgr, YNetRunner netRunner) {
		YSpecificationID specID = _decompositionPrototype.getSpecification().getSpecificationID();
		...
		if (logPredicate != null) {
			...
			if (predicate != null) {
				logData = new YLogDataItemList(new YLogDataItem("Predicate", "OnStart", predicate, "string"));
			}
		}
		YEventLogger.getInstance().logSubNetCreated(pmgr, specID, netRunner, this.getID(), logData);
	}
	...
	public void verify(YVerificationHandler handler) {
		...
		if (!(_decompositionPrototype instanceof YNet)) {
			handler.error(this, this + " composite task may not decompose to other than a net.");
		}
	}
