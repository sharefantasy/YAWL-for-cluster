...
/**
 * 
 * The implementation of a net in the YAWL semantics - A container for tasks and
 * conditions.
 * 
...
public final class YNet extends YDecomposition {
	...
	public void verify(YVerificationHandler handler) {
		...
		// check that all elements in the net are on a directed path from 'i' to
		// 'o'.
	...
	private void verifyDirectedPath(YVerificationHandler handler) {
		...
		/*
		 * Function isValid(YConditionInterface i, YConditionInterface o, Tasks
		 * T, Conditions C): Boolean BEGIN: #Initalize variables: visitedFw :=
		 * {i}; visitingFw := postset(visitedFw); visitedBk := {o}; visitingBk
		 * := preset(visitedBk);
		 */
		...
		/*
		 * Begin Loop: visitedFw := visitedFw Union visitingFw; visitingFw :=
		 * postset(visitingFw) - visitedFw; Until: visitingFw = {}
		 */
		...
		/*
		 * Begin Loop: visitedBk := visitedBk Union visitingBk; visitingBk :=
		 * preset(visitingBk) - visitedBk; Until: visitingBk = {}
		 */
		...
		/*
		 * return visitedFw = T U C ^ visitedBk = T U C; // returns true iff all
		 * t in T are on a directed path from i to o END
		 */
	...
	/**
	 * Initialises the variable/parameter declarations so that the net may
	 * execute.
	 */
	...
	public void setIncomingData(YPersistenceManager pmgr, Element incomingData)
		...
		for (YParameter parameter : getInputParameters().values()) {
			...
			// remove any attributes - not required and cause validation errors
			// if left
		...
		for (Element element : incomingData.getChildren()) {
			if (getInputParameters().containsKey(element.getName())) {
				...
				throw new IllegalArgumentException("Element " + element + " is not a valid input parameter of " + this);
			}
		}
	}
