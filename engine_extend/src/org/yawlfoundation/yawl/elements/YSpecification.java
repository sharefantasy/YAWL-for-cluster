...
/**
 * 
 * Objects of this type are a specification of a Workflow checkSchema model in
 * YAWL.
 * 
...
public final class YSpecification implements Cloneable, YVerifiable {
	...
	private Map<String, YDecomposition> _decompositions = new HashMap<String, YDecomposition>();
	...
	/**
	 * @deprecated since v2.0: use getSchemaVersion() instead Gets the version
	 *             number of this specification's schema (as opposed to the
	 *             version number of the specification itself).
	 * @return the version of the engine that this specification was first
	 *         designed for.
	 */
	...
	/**
	 * Gets the version number of this specification (as opposed to the version
	 * number of the specification's schema).
	 * 
	...
	public void setVersion(String version) {
		if (version.equals("beta3"))
			version = "Beta 3";
		_version = YSchemaVersion.fromString(version);
		if (_version == null) {
			throw new IllegalArgumentException("Param version [" + version + "] is not allowed.");
		}
	}
	...
	public String toXML() {
		...
		xml.append("<decomposition id=\"").append(_rootNet.getID())
		...
		List<YDecomposition> sortedDecompositions = new ArrayList<YDecomposition>(_decompositions.values());
		Collections.sort(sortedDecompositions, new Comparator<YDecomposition>() {
			public int compare(YDecomposition d1, YDecomposition d2) {
				if (d1 instanceof YNet) {
					if (!(d2 instanceof YNet))
						return -1; // d1 is YNet, d2 is not
				} else if (d2 instanceof YNet)
					return 1; // d2 is YNet, d1 is not

				if (d1.getID() == null)
					return -1; // either both are YNets
				if (d2.getID() == null)
					return 1; // or both are not
		...
		for (YDecomposition decomposition : sortedDecompositions) {
			if (!decomposition.getID().equals(_rootNet.getID())) {
				String factsType = (decomposition instanceof YNet) ? "NetFactsType" : "WebServiceGatewayFactsType";
				xml.append(String.format("<decomposition id=\"%s\" xsi:type=\"%s\"%s>", decomposition.getID(),
						factsType, decomposition.getAttributes().toXML()));
	...
	public YSpecificationID getSpecificationID() {
		if (_specID == null) {
			_specID = new YSpecificationID(_metaData.getUniqueID(), _metaData.getVersion(), _specURI);
		}
		return _specID;
	}
	...
	public boolean equals(Object other) {
		return (other instanceof YSpecification) && // instanceof = false if
													// other is null
	...
	private void setPersistedXML(String xml) {
		persistedXML = xml;
	}
	...
	private Set<YExternalNetElement> getEmptyPostsetAtThisLevel(Set<YExternalNetElement> aSet) {
		...
		for (YExternalNetElement element : elements) {
			if ((element instanceof YCondition)
					|| ((element instanceof YTask) && (((YTask) element).getDecompositionPrototype() == null))) {
	...
	private void checkTheseTasksForInfiniteLoops(Set<YExternalNetElement> relevantTasks, boolean generateWarnings,
			...
			while (visiting.size() > 0) {
				if (visiting.contains(element)) {
					handler.add(element,
							...
							generateWarnings
									? YVerificationHandler.MessageType.warning
	...
	private Set<YExternalNetElement> selectEmptyAndDecomposedTasks(Set<YDecomposition> relevantNets) {
		...
		for (YDecomposition decomposition : relevantNets) {
			relevantTasks.addAll(unfoldNetChildren(decomposition, new HashSet<YDecomposition>(), "emptyTasks"));
			relevantTasks.addAll(unfoldNetChildren(decomposition, new HashSet<YDecomposition>(), "decomposedTasks"));
		}
	...
	private void checkDecompositionUsage(YVerificationHandler handler) {
		Set<YDecomposition> netsBeingUsed = new HashSet<YDecomposition>();
		unfoldNetChildren(_rootNet, netsBeingUsed, null);
		Set<YDecomposition> specifiedDecompositions = new HashSet<YDecomposition>(_decompositions.values());
		specifiedDecompositions.removeAll(netsBeingUsed);

		for (YDecomposition decomp : specifiedDecompositions) {
			handler.warn(decomp, "The decomposition [" + decomp.getID() + "] is not being used in this specification.");
		}
	}
