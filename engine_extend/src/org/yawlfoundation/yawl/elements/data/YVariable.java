...
/**
 * @author Lachlan Aldred Date: 24/09/2003 Time: 16:10:14
 */
public class YVariable implements Cloneable, YVerifiable, Comparable<YVariable> {
	 ...
	 *
	 * @param dataType
	 *            (mandatory) the datatype
	 * @param name
	 *            (mandatory) name of the var
	 * @param namespace
	 *            (null optional) the URI of the namespace for the type
	 */
	...
	public String getDataTypeNameUnprefixed() {
		return _dataTypeName.contains(":") ? _dataTypeName.substring(_dataTypeName.indexOf(":") + 1) : _dataTypeName;
	 ...
	 * a custom type or "http://www.w3.org/2001/XMLSchema" if the variable uses
	 * a "built in" Schema primitive type.
	 *
	...
	private boolean isSchemaVersionAtLeast2_1() {
		return (_parentDecomposition != null) && _parentDecomposition.getSpecification().getSchemaVersion()
	...
	public String toString() {
		return getClass().getName() + ":" + ((_name != null) ? _name : _elementName + toXML());
	}
	...
	public void setOptional(boolean option) {
		if (option) { // only set attribute if true
			...
			getAttributes().remove("optional"); // no attribute means not
												// optional
		}
	}
	 ...
	 * <P>
	 * Table is keyed by attribute 'name' and contains the string representation
	 * of the XML elements attribute.
	 * <P>
	 *
	...
	public void verify(YVerificationHandler handler) {
		...
		if ((null != _name) && (null != _elementName)) {
			handler.error(this, "Name xor element name for this variable must be set, not both.");
		}
		...
		// check schema contains type with typename.
			...
			if (!(_isUntyped || isValidTypeNameForSchema(_dataTypeName))) {
				handler.error(this,
						"The type library (Schema) in specification contains no " + "type definition with name ["
								+ _dataTypeName + "].  " + "Therefore the decomposition " + _parentDecomposition
								+ " cannot create this variable.");
			}
		} else if (null != _elementName) {
			if (!isValidTypeNameForSchema(_elementName)) {
				handler.error(this,
						"The type library (Schema) in specification contains no " + "element definition with name ["
		...
		if (_dataTypeName.endsWith("YDocumentType")) {
			try {
				if (YEngine.isRunning()) {
					...
					if (service == null) {
						handler.warn(this,
								"Variable [" + getPreferredName() + "] in decomposition [" + _parentDecomposition
										+ "] is of type 'YDocument', "
				...
				// may occur if called in standalone mode (eg. from the editor),
				// caused by
				// the call to a static YEngine which attempts to create a
				// YPersistenceManager object - ok to ignore the verify check in
				// these instances
			}
		}
	}
	...
	private boolean isValidTypeNameForSchema(String dataTypeName) {
		...
		for (String name : _parentDecomposition.getSpecification().getDataValidator().getPrimaryTypeNames()) {
			if (dataTypeName.equals(name))
				return true;
		}
		return false;
	}

	private void checkValue(String value, String label, YVerificationHandler handler) {
		if (!StringUtil.isNullOrEmpty(value)) {
			...
			if (testElem == null) {
				handler.error(this, "The " + label + " value [" + value + "] of variable [" + getPreferredName()
						+ "] in decomposition [" + _parentDecomposition + "] is not well formed.");
			}
			...
			try {
				...
				handler.error(this, "The " + label + " value [" + value + "] of variable [" + getPreferredName()
						+ "] in decomposition [" + _parentDecomposition + "] is not valid for its data type.");
			}
		}
	}

	private void checkDataTypeValue(Element value) throws YDataValidationException {
		_parentDecomposition.getSpecification().getDataValidator().validate(this, value, "");
	}
