...
public class CostPredicateEvaluator implements PredicateEvaluator {
	...
	private static final Pattern PATTERN = Pattern.compile("cost\\(\\s*(case\\(\\s*((|\\d+|\\d+-\\d+|max|min|average|"
			+ "(dow|last|first|random)\\s+\\d+|(from|to)\\s+\\d{4}-[01]\\d-[0-3]\\d)"
			+ "(|\\s*,\\s*))*\\s*\\))?(|\\s*,\\s*)" + "(task\\((['\"]*\\w+\\s*['\"]*(|,|\\s)\\s*)+\\))?(|\\s*,\\s*)"
			+ "(resource\\((['\"]*\\w+\\s*['\"]*(|,|\\s)\\s*)+\\))?\\s*\\)");
	...
	public String replace(YDecomposition decomposition, String predicate, YIdentifier token) {
		for (String expression : extract(predicate)) {
	...
	public String calculate(YDecomposition decomposition, String expression, YIdentifier token) {
		try {
			connect();
			return String.valueOf(_client.calculate(decomposition.getSpecification().getSpecificationID(),
