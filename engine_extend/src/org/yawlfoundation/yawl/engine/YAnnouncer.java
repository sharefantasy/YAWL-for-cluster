...
public class YAnnouncer {
	...
	protected YAnnouncer(YEngine engine) {
		...
		// Currently the only standard gateway is the HTTP driven IB Servlet
		// client.
		try {
	...
	public synchronized void registerInterfaceBObserverGateway(ObserverGateway gateway) throws YAWLException {
	...
	/**
	 * Called by the engine when its initialisation is complete. Broadcast to
	 * all registered services and gateways.
	 * 
	 * @param services
	 *            the set of currently registered services
	 * @param maxWaitSeconds
	 *            timeout to announce to each service before giving up
	 */
	...
	/**
	 * Called by the engine when a case is cancelled. Broadcast to all
	 * registered services and gateways.
	 * 
	 * @param id
	 *            the identifier of the cancelled case
	 * @param services
	 *            the set of currently registered services
	 */
	...
	/**
	 * Called by YWorkItemTimer when a work item's timer expires. Announced only
	 * to the designated service or gateway.
	 * 
	 * @param item
	 *            the work item that has had its timer expire
	 */
	...
	/**
	 * Called by the engine when a case is suspending. Broadcast to all
	 * registered services and gateways.
	 * 
	 * @param id
	 *            the identifier of the suspending case
	 * @param services
	 *            the set of currently registered services
	 */
	...
	/**
	 * Called by the engine when a case has suspended. Broadcast to all
	 * registered services and gateways.
	 * 
	 * @param id
	 *            the identifier of the suspended case
	 * @param services
	 *            the set of currently registered services
	 */
	...
	/**
	 * Called by the engine when a case has resumed from suspension. Broadcast
	 * to all registered services and gateways.
	 * 
	 * @param id
	 *            the identifier of the resumed case
	 * @param services
	 *            the set of currently registered services
	 */
	...
	protected void announceCaseStart(YSpecificationID specID, YIdentifier caseID, String launchingService,
			boolean delayed) {
		...
		_controller.notifyCaseStarting(_engine.getYAWLServices(), specID, caseID, launchingService, delayed);
	}

	/**
	 * Called by a case's net runner when it completes. Announced only to the
	 * designated service or gateway when the 'service' parameter is not null,
	 * otherwise it is broadcast to all registered services and gateways.
	 * 
	 * @param service
	 *            the name of the service or gateway to announce the case
	 *            completion to. If null, the event will be announced to all
	 *            registered services and gateways
	 * @param caseID
	 *            the identifier of the completed case
	 * @param caseData
	 *            the final output data for the case
	 */
	protected void announceCaseCompletion(YAWLServiceReference service, YIdentifier caseID, Document caseData) {
		if (service == null) {
			...
			_controller.notifyCaseCompletion(service, caseID, caseData);
	}
	 ...
	 * 
	 * @param item
	 *            the work item that has had a change of status
	 * @param oldStatus
	 *            the previous status of the work item
	 * @param newStatus
	 *            the new status of the workitem
	 */
	protected void announceWorkItemStatusChange(YWorkItem item, YWorkItemStatus oldStatus, YWorkItemStatus newStatus) {
		...
		_controller.notifyWorkItemStatusChange(_engine.getYAWLServices(), item, oldStatus, newStatus);
	}

	/**
	 * Called by a workitem when it is cancelled. Announced only to the
	 * designated service or gateway.
	 * 
	 * @param item
	 *            the work item that has had a change of status
	 */
	...
	/**
	 * Called by the engine when it begins to shutdown. Broadcast to all
	 * registered services and gateways.
	 */
	...
	/**
	 * Called by the engine each time a net runner advances a case, resulting in
	 * the enabling and/or cancelling of one or more work items.
	 * 
	 * @param announcements
	 *            A set of work item enabling or cancellation events
	 */
	...
	protected YAnnouncement createAnnouncement(YAWLServiceReference ys, YWorkItem item, YEngineEvent event) {
		if (ys == null)
			ys = _engine.getDefaultWorklist();
		return (ys != null) ? new YAnnouncement(ys, item, event, getAnnouncementContext()) : null;
	}
	...
	// this method should be called by an IB service when it decides it is not
	// going
	// to handle (i.e. checkout) a workitem announced to it. It passes the
	// workitem to
	...
	public void rejectAnnouncedEnabledTask(YWorkItem item) {
		...
		if (defaultWorklist != null) {
			debug("Announcing enabled task ", item.getIDString(), " on service ", defaultWorklist.getServiceID());
	...
	protected void announceCheckWorkItemConstraints(YWorkItem item, Document data, boolean preCheck) {
		for (InterfaceX_EngineSideClient listener : _interfaceXListeners) {
			debug("Announcing Check Constraints for task ", item.getIDString(), " on client ", listener.toString());
	...
	protected void announceCheckCaseConstraints(YSpecificationID specID, String caseID, String data, boolean preCheck) {
		for (InterfaceX_EngineSideClient listener : _interfaceXListeners) {
			debug("Announcing Check Constraints for case ", caseID, " on client ", listener.toString());
	...
	protected void announceTimeServiceExpiry(YWorkItem item, List timeOutTaskIds) {
		for (InterfaceX_EngineSideClient listener : _interfaceXListeners) {
			debug("Announcing Time Out for item ", item.getWorkItemID().toString(), " on client ", listener.toString());
	...
	private void announceCaseCancellationToInterfaceXListeners(YIdentifier caseID) {
		for (InterfaceX_EngineSideClient listener : _interfaceXListeners) {
			debug("Announcing Cancel Case for case ", caseID.toString(), " on client ", listener.toString());
	...
	private void rennounceRestoredItems() {
		...
		// MLF: moved from restore logic. There is no point in reannouncing
		// before the first gateway
		// is registered as the announcements will simply fall on deaf errors.
		// Obviously we
		...
		try {
			...
			_logger.error("Failure whilst reannouncing workitems. " + "Some workitems might not have been reannounced.",
					e);
		}
	...
	/**
	 * Causes the engine to re-announce all workitems which are in an "enabled"
	 * state.
	 * <P>
	 *
	...
	/**
	 * Causes the engine to re-announce all workitems which are in an
	 * "executing" state.
	 * <P>
	 *
	...
	/**
	 * Causes the engine to re-announce all workitems which are in an "fired"
	 * state.
	 * <P>
	 *
