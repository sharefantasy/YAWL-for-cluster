...
/**
 * @author Lachlan Aldred Date: 17/06/2003 Time: 13:46:54
 *
...
public class YEngineImpl extends YEngine {
	...
	/**
	 * Initialises the engine (if not already initialised) & returns the engine
	 * instance.
	 * 
	 * @param persisting
	 *            true if engine state is to be persisted
	 * @return a reference to the initialised engine
	 * @throws YPersistenceException
	 *             if there's a problem restoring from persistence
	 */

	public static YEngine getInstance(boolean persisting, boolean gatherHbnStats) throws YPersistenceException {
		if (_thisInstance == null) {
			_thisInstance = new YEngineImpl();
			((YEngineImpl) _thisInstance).persistenceInit(persisting, gatherHbnStats);
		}
	...
	private void persistenceInit(boolean persisting, boolean gatherHbnStats) throws YPersistenceException {
		...
		if (_persisting) {
			...
			((YEngineImpl) _thisInstance).restore();
		} else {
			_pmgr.setEnabled(false);

			// Default clients and services should always be available
			((YEngineImpl) _thisInstance).loadDefaultClients();
		}
	...
	/**
	 * Initialises the engine (if not already initialised) & returns the engine
	 * instance, using the default persistence flag.
	 * 
	 * @return a reference to the initialised engine
	 */
	...
	private void restore() throws YPersistenceException {
		...
		try {
			...
			// a non-YPersistenceException means the restore failed, but the
			// engine is
			// still operational
			...
			_logger.debug("restore <---");
			_restoring = false;
			restorer.persistDefaultClients(); // delayed til restoring is
												// complete
		}
	}
	 ...
	 * properties file on startup when they have not previously been persisted
	 * (ie. on first startup) or when persistence is disabled.
	 * 
	 * @return the set of default clients loaded.
	 * @throws YPersistenceException
	 *             A passthrough - since it is only called when restoring or
	 *             when persistence is disabled, this exception is never thrown.
	 */
	...
	/**
	 * Indicate if user interface metadata is to be generated within a task's
	 * input XML doclet.
	 * 
	 * @param generate
	 *            true to generate metadata, false to not generate it
	 */
	...
	/**
	 * Indicates if user interface metadata will be generated within a task's
	 * input XML doclet.
	 * 
	...
	public Map<String, YParameter> getParameters(YSpecificationID specID, String taskID, boolean input) {
	...
	public String getEngineClassesRootFilePath() {
		return _engineClassesRootFilePath;
	}
	...
	/**
	 * Registers an InterfaceB Observer Gateway with the engine in order to
	 * receive callbacks.
	 * 
	 * @param gateway
	 *            the gateway to register
	 * @throws YAWLException
	 *             if the observerGateway has a null scheme value.
	 */
	...
	/**
	 * Adds a net runner instance to the engine caches. The specification is
	 * derived from the runner instance.
	 * 
	 * @param runner
	 *            the runner to add
	 */
	...
	public void addRunner(YNetRunner runner, YSpecification specification) {
		if (specification != null) {
			...
			_instanceCache.addCase(runner.getCaseID().toString(), specification.getSpecificationID(),
					runner.getNetData().getData(), null, runner.getStartTime());

			// announce the add to the standalone gui (if any)
			if (_interfaceBClient != null) {
				_interfaceBClient.addCase(specification.getSpecificationID(), runner.getCaseID().toString());
			}
		}
	}
	...
	public String getNetData(String caseID) throws YStateException {
		...
		// if this is a root net case id, the net data is equivalent to the case
		// data
		if (!caseID.contains("."))
	 ...
	 * 
	 * @param specStr
	 *            an XML formatted specification
	 * @param ignoreErrors
	 *            ignore verification errors and load the spec anyway.
	 * @param verificationHandler
	 *            an in/out param passing any error messages.
	 * @return the specification ids of the successfully loaded specs
	 */
	public List<YSpecificationID> addSpecifications(String specStr, boolean ignoreErrors,
			YVerificationHandler verificationHandler) throws YPersistenceException {
				...
				if (ignoreErrors || !verificationHandler.hasErrors()) {
					if (loadSpecification(specification)) {
						...
						String errDetail = specification.getSchemaVersion().isBetaVersion()
								? "URI: " + specification.getURI()
								: "UID: " + specification.getID();
						errDetail += "- Version: " + specification.getSpecVersion();
						verificationHandler.error(this, "There is a specification with an identical id to [" + errDetail
								+ "] already loaded into the engine.");
					}
				}
			}
		}
	 ...
	 * 
	 * @param specID
	 *            the identifier of the specification to unload
	 * @throws YStateException
	 *             if the spec is still in use (with a live case)
	 * @throws YPersistenceException
	 *             if there's some persistence problem
	 */
	 ...
	 * It returns those that were loaded as well as those with running instances
	 * that are unloaded.
	 *
	 ...
	 * 
	 * @param key
	 *            the spec's identifier (v2.0+) or uri (pre-v2.0)
	...
	public String getLoadStatus(YSpecificationID specID) {
		return _specifications.contains(specID) ? YSpecification._loaded : YSpecification._unloaded;
	}
	...
	/**
	 * Gets the complete map of all running case ids, grouped by specification
	 * id
	 * 
	...
	public Map<YSpecificationID, List<YIdentifier>> getRunningCaseMap() {
		Map<YSpecificationID, List<YIdentifier>> caseMap = new HashMap<YSpecificationID, List<YIdentifier>>();
		for (YIdentifier caseID : _runningCaseIDToSpecMap.keySet()) {
			YSpecification specForCaseID = _runningCaseIDToSpecMap.get(caseID);
	...
	protected YIdentifier startCase(YSpecificationID specID, String caseParams, URI completionObserver, String caseID,
		...
		if (specification != null) {
			...
			_netRunnerRepository.add(runner);
			logCaseStarted(specID, runner, completionObserver, caseParams, logData, serviceRef, delayed);
	...
	protected Element formatCaseParams(String paramStr, YSpecification spec) throws YStateException {
		...
		if (paramStr != null && !"".equals(paramStr)) {
			...
			} else if (!(spec.getRootNet().getID().equals(data.getName()) || (spec.getURI().equals(data.getName())))) {
				throw new YStateException("Invalid caseParams: outermost element name must match "
	...
	private void logCaseStarted(YSpecificationID specID, YNetRunner runner, URI completionObserver, String caseParams,
			YLogDataItemList logData, String serviceRef, boolean delayed) {
		...
		// cache instance
		_instanceCache.addCase(caseID.toString(), specID, caseParams, logData, runner.getStartTime());
	}
	 ...
	 * 
	 * @param caseID
	 *            the id of the completing case
	 * @throws YPersistenceException
	 *             if theres a persistence problem
	 */
	 ...
	 * 
	 * @param caseID
	 *            the identifier of the cancelling case
	 * @throws YPersistenceException
	 *             if there's some persistence problem
	 */
	...
	public void cancelCase(YIdentifier caseID, String serviceHandle)
		...
		synchronized (_pmgr) {
			startTransaction();
			if (_persisting)
				clearWorkItemsFromPersistence(removedItems);
			...
			for (YWorkItem item : removedItems) {
				_yawllog.logWorkItemEvent(_pmgr, item, YWorkItemStatus.statusCancelledByCase, null);
			}
			commitTransaction();
	...
	public String launchCase(YSpecificationID specID, String caseParams, URI completionObserver,
	...
	public String launchCase(YSpecificationID specID, String caseParams, URI completionObserver, String caseID,
			YLogDataItemList logData, String serviceHandle, boolean delayed)
			throws YStateException, YDataStateException, YEngineStateException, YQueryException, YPersistenceException {
		...
		synchronized (_pmgr) {
			startTransaction();
			try {
				YIdentifier yCaseID = startCase(specID, caseParams, completionObserver, caseID, logData, serviceHandle,
						delayed);
				if (yCaseID != null) {
					commitTransaction();
	...
	public String getStateTextForCase(YIdentifier caseID) {
		...
		stateText.append(hashLine).append(cr).append("CaseID: ").append(caseID).append(cr).append("Spec:   ")
				.append(_runningCaseIDToSpecMap.get(caseID)).append(cr).append(hashLine).append(cr);
			...
			if (element instanceof YCondition) {
				...
				for (YInternalCondition internalCondition : task.getAllInternalConditions()) {
					if (internalCondition.containsIdentifier()) {
						stateText.append('\t').append(internalCondition.toString()).append(cr);
		...
		for (YNetElement element : getCaseLocations(caseID)) {
			if (element instanceof YCondition) {
				...
				for (YFlow flow : condition.getPostsetFlows()) {
					String doc = (flow.getDocumentation() != null) ? flow.getDocumentation() : "";
	...
	/**
	 * AJH: Public method which returns the next available caseID Note: This is
	 * only available with a non-persisting engine and is used to ascertain the
	 * case ID prior to launching a case (eg. for an XForms execution
	 * framework).
	 * 
	 ...
	 * 
	 * @param caseID
	 *            the id of the case to suspend
	 * @throws YPersistenceException
	 *             if there's a problem persisting the change
	 * @throws YStateException
	 *             if case cannot be suspended given the current engine
	 */
	public void suspendCase(YIdentifier caseID) throws YPersistenceException, YStateException {
		synchronized (_pmgr) {
			startTransaction();
			try {
	 ...
	 * @param id
	 *            the case id to clear
	 * @throws YPersistenceException
	 *             if there's a problem clearing the case
	 * @throws YStateException
	 *             if case cannot be suspended given the current engine
	 *             operating state
	 */
	private void suspendCase(YPersistenceManager pmgr, YIdentifier id) throws YPersistenceException, YStateException {
		...
		if (!topLevelNet.hasNormalState()) {
			...
			for (YNetRunner runner : getRunnersForPrimaryCase(id)) {
				debug("Current status of runner ", runner.get_caseID(), " = ", runner.getExecutionStatus());
	 ...
	 * 
	 * @param id
	 *            the id of the case to resume
	 * @throws YPersistenceException
	 *             if there's a problem persisting the resumed case
	 * @throws YStateException
	 *             if case cannot be resumed
	 */
	...
	private void resumeCase(YPersistenceManager pmgr, YIdentifier id)
			throws YPersistenceException, YStateException, YDataStateException, YQueryException {
		...
		if (_netRunnerRepository.get(id).isInSuspense()) {
			for (YNetRunner runner : getRunnersForPrimaryCase(id)) {
				...
				// Update persistence only if this runner has not completed. If
				// it has
				// completed (as in the case where we resume a case and the last
				// workitem has previously been completed), the above call to
				// 'kick'
				// will have progressed the net to its end point, so the
				// persistence
			...
			throw new YStateException(
					"Case " + id + " cannot be suspended as currently not executing normally (SuspendStatus="
	 ...
	 *
	 * Where a Case is "suspending", we scan the workitems associated with all
	 * the nets associated with the case, and where no workitems are enabled,
	 * executing or fired, we progress the suspension state from "suspending" to
	 * "suspended".
	 * 
	 * @param pmgr
	 *            the persistence manager object
	 ...
	 * @throws YPersistenceException
	 *             if there's a problem getting the case
	 * @throws YStateException
	 *             if case cannot be progressed
	 */
	private void progressCaseSuspension(YPersistenceManager pmgr, YIdentifier caseID)
		...
		if (!_netRunnerRepository.get(caseID).isSuspending()) {
			throw new YStateException(
					"Case " + caseID + " cannot be suspended as case not currently attempting to suspend.");
		}
		...
		for (YNetRunner runner : runners) {
			...
			// Go thru busy and executing tasks and see if we have any atomic
			// tasks
	...
	/**
	 * updates the case data with the data passed after completion of an
	 * exception handler
	 */
	...
	public Set<YWorkItem> getAvailableWorkItems() {
		...
		if (_logger.isDebugEnabled()) {
			debug("--> getAvailableWorkItems: Enabled=" + enabledItems.size(), " Fired=" + firedItems.size());
		}
	...
	public Element getStartingDataSnapshot(String itemID)
	...
	/**
	 * Starts a work item. If the workitem param is enabled this method fires
	 * the task and returns the first of its child instances in the executing
	 * state. Else if the workitem is fired then it moves the state from fired
	 * to executing. Either way the method returns the resultant work item.
	 *
	 * @param workItem
	 *            the enabled, or fired workitem.
	 * @param client
	 *            the YAWL external client or service starting the workitem
	 * @return the resultant work item in the executing state.
	 * @throws YStateException
	 *             if the workitem is not in either of these states.
	 * @throws YDataStateException
	 */
				...
				if (workItem != null) {
					switch (workItem.getStatus()) {
						...
						default : // this work item is likely already executing.
							rollbackTransaction();
							throw new YStateException(String.format("Item [%s]: status [%s] does not permit starting.",
	...
	private YWorkItem startEnabledWorkItem(YNetRunner netRunner, YWorkItem workItem, YClient client)
		...
		YWorkItem startedItem = null;
		YTask task = (YTask) netRunner.getNetElement(workItem.getTaskID());
		List<YIdentifier> childCaseIDs = netRunner.attemptToFireAtomicTask(_pmgr, workItem.getTaskID());
	 ...
	 * @param data
	 * @param logPredicate
	 *            - a pre-parse of the completion log predicate for this item
	...
	public void completeWorkItem(YWorkItem workItem, String data, String logPredicate,
		...
		debug("--> completeWorkItem", "\nWorkItem = ", workItem != null ? workItem.get_thisID() : "null", "\nXML = ",
				data);
		checkEngineRunning();
			...
			try {
				if (workItem != null) {
					...
					if (workItem.getStatus().equals(YWorkItemStatus.statusExecuting)) {
						completeExecutingWorkitem(workItem, netRunner, data, logPredicate, completionType);
					} else if (workItem.getStatus().equals(YWorkItemStatus.statusDeadlocked)) {
						_workItemRepository.removeWorkItemFamily(workItem);
					} else {
						throw new YStateException(
								"WorkItem with ID [" + workItem.getIDString() + "] not in executing state.");
					}
	...
	private void completeExecutingWorkitem(YWorkItem workItem, YNetRunner netRunner, String data, String logPredicate,
			WorkItemCompletion completionType)
			throws YStateException, YDataStateException, YQueryException, YPersistenceException, YEngineStateException {
		...
		if (completionType != WorkItemCompletion.Fail) {
			...
			if (netRunner.completeWorkItemInTask(_pmgr, workItem, doc)) {
				...
				/*
				 * When a Task is enabled twice by virtue of having two enabling
				 * sets of tokens in the current marking the work items are not
				 * created twice. Instead an Enabled work item is created for
				 * one of the enabling sets. Once that task has well and truly
				 * finished it is then an appropriate time to notify the
				 * worklists that it is enabled again.
				 */
	...
	private Document getDataDocForWorkItemCompletion(YWorkItem workItem, String data, WorkItemCompletion completionType)
			throws YStateException {
	...
	private String mapOutputDataForSkippedWorkItem(YWorkItem workItem, String data) throws YStateException {
		...
		if (outputs.isEmpty())
			return data; // no output data to map
		...
		for (YParameter outParam : outParamList) {
			...
			// the output param has no corresponding input param, so add an
			// element
			...
			if (defaultValue == null) {
				...
				if (!XSDType.isBuiltInType(typeName)) {
					throw new YStateException(String.format(
							"Could not skip work item [%s]: Output-Only parameter [%s]" + " requires a default value.",
							workItem.getIDString(), name));
				}
	...
	/**
	 * Determines whether or not a task will allow a dynamically created new
	 * instance to be created. MultiInstance Task with dynamic instance
	 * creation.
	 *
	 * @param workItemID
	 *            the workItemID of a sibling work item.
	 * @throws YStateException
	 *             if task is not MultiInstance, or if task does not allow
	 *             dynamic instance creation, or if current number of instances
	 *             is not less than the maxInstances for the task.
	 */
	public void checkElegibilityToAddInstances(String workItemID) throws YStateException {
		...
		if (item != null) {
			if (item.getStatus().equals(YWorkItemStatus.statusExecuting)) {
				...
				throw new YStateException("WorkItem[" + workItemID + "] is not in appropriate (executing) "
	 ...
	 * @param workItemID
	 *            the id of the workitem to check against
	 ...
	 *
	 * @param workItem
	 *            the id of a work item inside the task to have a new instance.
	 * @param paramValueForMICreation
	 *            format "<data>[InputParam]*</data> InputParam ==
	 *            <varName>varValue</varName>
	 * @return the work item of the new instance.
	 * @throws YStateException
	 *             if the task is not able to create a new instance, due to its
	 *             state or its design.
	 * @throws YPersistenceException
	 *             if there's a problem with the persistence session
	 */
	public YWorkItem createNewInstance(YWorkItem workItem, String paramValueForMICreation)
		...
		synchronized (_pmgr) {
			...
			try {
				...
				YIdentifier id = netRunner.addNewInstance(_pmgr, taskID, workItem.getCaseID(), paramValue);
	...
	public YWorkItem unsuspendWorkItem(String workItemID) throws YStateException, YPersistenceException {
		YWorkItem workItem = _workItemRepository.get(workItemID);
		if ((workItem != null) && (workItem.getStatus().equals(YWorkItemStatus.statusSuspended))) {
			synchronized (_pmgr) {
				startTransaction();
	...
	public void rollbackWorkItem(String workItemID) throws YStateException, YPersistenceException {
		YWorkItem workItem = _workItemRepository.get(workItemID);
		if ((workItem != null) && workItem.getStatus().equals(YWorkItemStatus.statusExecuting)) {
			synchronized (_pmgr) {
				...
				if (netRunner.rollbackWorkItem(_pmgr, workItem.getCaseID(), workItem.getTaskID())) {
					...
					rollbackTransaction();
					throw new YStateException(
							"Unable to rollback: work Item[" + workItemID + "] is not in executing state.");
				}
			}
		} else
	...
	public Set getChildrenOfWorkItem(YWorkItem workItem) {
		return (workItem == null) ? null : _workItemRepository.getChildrenOf(workItem.getIDString());
	}

	/**
	 * updates the workitem with the data passed after completion of an
	 * exception handler
	 */
			...
			if ((workItem != null) && workItem.getStatus().equals(YWorkItemStatus.statusExecuting)) {
				...
				synchronized (_pmgr) {
					...
					if ((parent != null) && (parent.getChildren().size() == 1)) {
						...
						((YAtomicTask) workItem.getTask()).cancel(_pmgr, workItem.getCaseID());
	...
	private void cleanupCompletedWorkItem(YWorkItem workItem, YNetRunner netRunner, Document data)
			throws YPersistenceException, YStateException {
		...
		if (parent != null) {
			...
			// If case is suspending, see if we can progress into a fully
			// suspended state
	...
	public void addYawlService(YAWLServiceReference yawlService) throws YPersistenceException {
		...
		if (!_restoring && isPersisting()) {
			_logger.info("Persisting YAWL Service " + yawlService.getURI() + " with ID " + yawlService.getServiceID());
	...
	public YAWLServiceReference removeYawlService(String serviceURI) throws YPersistenceException {
		...
		if ((service != null) && isPersisting()) {
			_logger.info("Deleting persisted entry for YAWL service " + service.getURI() + " with ID "
	...
	/**
	 * Adds an external client credentials object to the engine. An external
	 * client is an application that connects to the engine (as opposed to a
	 * service)
	 * 
	 * @param client
	 *            the external client to add
	 */
	public boolean addExternalClient(YExternalClient client) throws YPersistenceException {
		String userID = client.getUserName();
		if ((userID != null) && (client.getPassword() != null) && (!_externalClients.containsKey(userID))) {
	...
	public boolean updateExternalClient(String id, String password, String doco) throws YPersistenceException {
	...
	public YExternalClient removeExternalClient(String clientName) throws YPersistenceException {
	...
	/**
	 * Sets the custom service that will serve as the default worklist. Called
	 * on startup with values loaded from web.xml
	 * 
	 * @param paramStr
	 *            the URL and password of the service (separated by a hash)
	 * @throws RuntimeException
	 *             if the parameters read from web.xml are incorrectly formatted
	 */
		...
		try {
			if (allow) {
				...
				if (!_externalClients.containsKey("admin")) {
					addExternalClient(new YExternalClient("admin", PasswordEncryptor.encrypt("YAWL", null),
							"generic admin user"));
				}
			} else {
	 ...
	 * 
	 * @param persist
	 *            true to persist, false to not persist
	 */
	...
	/**
	 * Public interface to allow engine clients to ask the engine to store an
	 * object reference in its persistent storage. It does this in its own
	 * transaction block.
	 * <P>
	 *
	 * @param obj
	...
	private void doPersistAction(Object obj, int action) throws YPersistenceException {
		if (isPersisting() && (_pmgr != null)) {
			synchronized (_pmgr) {
				...
				switch (action) {
					case YPersistenceManager.DB_UPDATE :
						_pmgr.updateObject(obj);
						break;
					case YPersistenceManager.DB_DELETE :
						_pmgr.deleteObject(obj);
						break;
					case YPersistenceManager.DB_INSERT :
		...
		if (_persisting) {
			try {
				...
				for (YIdentifier child : list) {
					if (child != null)
						clearCaseFromPersistence(child);
				}
	...
	/**
	 * Removes the workitems of the runner from persistence (after a case
	 * cancellation).
	 *
	 * @param items
	 *            the set of work items to delete from persistnece
	 * @throws YPersistenceException
	 *             if there's some persistence problem
	 */
	private void clearWorkItemsFromPersistence(Set<YWorkItem> items) throws YPersistenceException {
