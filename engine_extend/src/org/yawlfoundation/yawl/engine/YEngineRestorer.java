...
/**
 * Handles the restoration of persisted objects and data pertaining to the
 * Engine.
 *
 * @author Michael Adams Creation Date: 25/06/2008
 */
...
public class YEngineRestorer {
	 ...
	 *             if there's a problem reading from the tables
	 */
	...
	protected void restoreExternalClients() throws YPersistenceException {
		...
		if (it.hasNext()) {
			...
			if (!_hasServices) {
				...
				// no services and no clientapps indicates a fresh db (there
				// should be at
				// least a row for the editor user) - so needs default accounts
				// to be added
	 ...
	 *
	 * @return a YCaseNbrStore object initialised to the next available case
	 *         number
	 * @throws YPersistenceException
	 *             if there's a problem reading from the tables
	 */
	protected YCaseNbrStore restoreNextAvailableCaseNumber() throws YPersistenceException {
		...
		if ((query != null) && (!query.list().isEmpty())) {
			...
			// secondary attempt: eg. if there's no case number stored (as will
			// be
	...
	protected Set<YTimedObject> restoreDelayedLaunches() throws YPersistenceException {
		...
		for (Iterator it = query.iterate(); it.hasNext();) {
			...
			// if the deadline has passed, launch the instance when engine is
			// ready
			if (endTime < System.currentTimeMillis()) {
	...
	protected void restartRestoredProcessInstances() throws YPersistenceException {
		/*
		 * Start net runners. This is a restart of a NetRunner not a clean
		 * start, therefore the net runner should not create any new work items,
	...
	private void loadSpecification(YSpecification spec) throws YPersistenceException {
		try {
			...
			// false == don't validate, since it has already been done when
			// first loaded
	...
	private Vector<YNetRunner> removeDeadRunners(Vector<YNetRunner> runners) throws YPersistenceException {
		...
		for (YNetRunner runner : runners) {
			if (getSpecification(runner) != null) {
				...
				/*
				 * This occurs when a specification has been unloaded, but the
				 * case is still there. This case is removed, since we must have
				 * the specification stored as well.
				 */
				String msg = String.format(
						"YEngineRestorer: The specification '%s' for"
								+ " active case '%s' is not loaded; the active case cannot"
								+ " continue and so has been removed.",
						runner.getSpecificationID().getUri(), runner.getCaseID().toString());
				_log.warn(msg);
	...
	private Hashtable<String, YNetRunner> restoreNets(Vector<YNetRunner> runners) throws YPersistenceException {
		Hashtable<String, YNetRunner> result = new Hashtable<String, YNetRunner>();

		for (YNetRunner runner : runners) {
			runner.setEngine(_engine); // Set engine for parent and composite
										// nets
			if (runner.getContainingTaskID() == null) {
				...
				if (parentrunner != null) {
					...
					try {
						...
						String msg = String.format(
								"YEngineRestorer: The decomposition" + "'%s' for  active case '%s' could not be set."
	...
	private void restoreRunners(Vector<YNetRunner> runners) throws YPersistenceException {
	...
	protected YIdentifier restoreYIdentifiers(Hashtable<String, YNetRunner> runnermap, YIdentifier id,
			YIdentifier parent, YNet net) throws YPersistenceException {
	...
	protected YIdentifier restoreLocations(Hashtable<String, YNetRunner> runnermap, YIdentifier id, YIdentifier parent,
			YNet net) throws YPersistenceException {
		...
		for (String s : id.getLocationNames())
			locationNames.add(s);
	...
	private void postTaskCondition(YTask task, YNet net, String condName, YIdentifier id) throws YPersistenceException {
		if (task != null) {
	...
	/**
	 * When restoring workitems, this method checks (1) if a parent workitem is
	 * in the list of items to restore, all of its children are in the list
	 * also; and (2) each child workitem in the list has a parent. If either is
	 * false, the workitem is put in a list of items to not be restored and to
	 * be removed from persistence
	 *
	 * @param itemList
	 *            the list of workitems to potentially restore
	...
	private List<YWorkItem> checkWorkItemFamiliesIntact(List<YWorkItem> itemList) {
		...
		for (YWorkItem witem : itemList) {
			if (witem.getStatus().equals(YWorkItemStatus.statusIsParent)) {
				...
				// if not a parent and not an orphan, must init (possibly
				// future)
				// children collection to avoid potential future
				// LazyInitializationException
				else
					Hibernate.initialize(witem.getChildren());
			}
		}
		return orphans;
	}
	 ...
	 *
	 * @param items
	 *            the workitems to remove
	 */
	private void removeWorkItems(List<YWorkItem> items) {
		try {

			// clear child items first (to avoid foreign key constraint
			// exceptions)
	...
	/**
	 * Finds and removes any persisted YIdentifiers that refer to cases that are
	 * no longer executing
	 */
	...
	/**
	 * Gets the YTask for the specification and task id passed. Any new requests
	 * are stored in a lookup table to minimise engine calls for subsequent
	 * identical requests (where many workitems are active for a spec/task
	 * combination)
	 *
	 * @param specID
	 *            the spec ID
