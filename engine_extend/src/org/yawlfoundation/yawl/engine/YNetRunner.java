 ...
 *
 *
 * @author Lachlan Aldred Date: 16/04/2003 Time: 16:08:01
 *
...
public class YNetRunner {
	...
	public YNetRunner(YPersistenceManager pmgr, YNet netPrototype, Element paramsData, String caseID)
			throws YDataStateException, YPersistenceException {
		this();

		// initialise and persist case identifier - if caseID is null, a new one
		// is supplied
		_caseIDForNet = new YIdentifier(caseID);
		if (pmgr != null)
			pmgr.storeObject(_caseIDForNet);

		// get case data from external data gateway, if set for this
		// specification
		Element externalData = netPrototype.getCaseDataFromExternal(_caseIDForNet.toString());
		if (externalData != null)
			paramsData = externalData;
	 ...
	 */
	public YNetRunner(YPersistenceManager pmgr, YNet netPrototype, YCompositeTask container, YIdentifier caseIDForNet,
			Element incomingData) throws YDataStateException, YPersistenceException {
	...
	private void initialise(YPersistenceManager pmgr, YNet netPrototype, YIdentifier caseIDForNet, Element incomingData)
			throws YDataStateException, YPersistenceException {
	...
	public boolean equals(Object other) {
		return (other instanceof YNetRunner) && // instanceof = false if other
												// is null
				((getCaseID() != null) ? getCaseID().equals(((YNetRunner) other).getCaseID()) : super.equals(other));
	}
	...
	public void start(YPersistenceManager pmgr)
			throws YPersistenceException, YDataStateException, YQueryException, YStateException {
		kick(pmgr);
	}
	...
	/**
	 * Assumption: this will only get called AFTER a workitem has been
	 * progressed? Because if it is called any other time then it will cause the
	 * case to stop.
	 * 
	 * @param pmgr
	 * @throws YPersistenceException
	...
	public synchronized void kick(YPersistenceManager pmgr)
		...
		if (!continueIfPossible(pmgr)) {
			...
			if ((_engine != null) && isRootNet()) {
				...
				// call the external data source, if its set for this
				// specification
				...
				if (logPredicate != null) {
					...
					if (predicate != null) {
						logData = new YLogDataItemList(
								new YLogDataItem("Predicate", "OnCompletion", predicate, "string"));
					}
				}
	...
	private void announceCaseCompletion() {
		...
		if (_announcer.hasInterfaceXListeners()) {
			...
			_announcer.announceCheckCaseConstraints(_specID, _caseID, JDOMUtil.documentToString(data), false);
		}
	}
	...
	private void createDeadlockItem(YPersistenceManager pmgr, YTask task) throws YPersistenceException {
		if (!_deadlockedTasks.contains(task)) {
			_deadlockedTasks.add(task);

			// create a new deadlocked workitem so that the deadlock can be
			// logged
	...
	private synchronized void processCompletedSubnet(YPersistenceManager pmgr, YIdentifier caseIDForSubnet,
		...
		if (busyCompositeTask.t_complete(pmgr, caseIDForSubnet, rawSubnetData)) {
			...
			// check to see if completing this task resulted in completing the
			// net.
			if (endOfNetReached()) {
				if (_containingCompositeTask != null) {
					...
					if ((parentRunner != null) && _containingCompositeTask.t_isBusy()) {
						...
						Document dataDoc = _net.usesSimpleRootData()
								...
								: _net.getOutputData();
						parentRunner.processCompletedSubnet(pmgr, _caseIDForNet, _containingCompositeTask, dataDoc);
					}
				}
			}
			kick(pmgr);
	...
	public synchronized YIdentifier addNewInstance(YPersistenceManager pmgr, String taskID,
	...
	public synchronized void startWorkItemInTask(YPersistenceManager pmgr, YIdentifier caseID, String taskID)
			throws YDataStateException, YPersistenceException, YQueryException, YStateException {
		YAtomicTask task = (YAtomicTask) _net.getNetElement(taskID);
		task.t_start(pmgr, caseID);
	}

	public boolean completeWorkItemInTask(YPersistenceManager pmgr, YWorkItem workItem, Document outputData)
			throws YDataStateException, YStateException, YQueryException, YPersistenceException {
		return completeWorkItemInTask(pmgr, workItem, workItem.getCaseID(), workItem.getTaskID(), outputData);
	}

	public synchronized boolean completeWorkItemInTask(YPersistenceManager pmgr, YWorkItem workItem, YIdentifier caseID,
	...
	public synchronized boolean continueIfPossible(YPersistenceManager pmgr)
		...
		for (YTask task : _netTasks) {
			...
			if (task.t_enabled(_caseIDForNet)) {
				...
				// if the task is not (or no longer) an enabled transition, and
				// it
				// has been previously enabled by the engine, then it must be
				// withdrawn
			...
			if (task.t_isBusy() && !_busyTasks.contains(task)) {
				...
				throw new RuntimeException(
						"Busy task list out of synch with a busy task: " + task.getID() + " busy tasks: " + _busyTasks);
			}
		}
		...
		for (YEnabledTransitionSet.TaskGroup group : enabledSet.getAllTaskGroups()) {
			if (group.hasEnabledCompositeTasks()) {
				...
				if (groupHasEmptyTask) { // do empty tasks last
					if (emptyTasks == null)
						emptyTasks = new HashSet<YAtomicTask>();
					emptyTasks.addAll(group.getEnabledEmptyTasks());
				}
			}
		}
		if (emptyTasks != null)
			processEmptyTasks(emptyTasks, pmgr);
	}

	private YAnnouncement fireAtomicTask(YAtomicTask task, String groupID, YPersistenceManager pmgr)
			throws YDataStateException, YStateException, YQueryException, YPersistenceException {

		YWorkItem item = createEnabledWorkItem(pmgr, _caseIDForNet, task);
		if (groupID != null)
			item.setDeferredChoiceGroupID(groupID);
		YAWLServiceGateway wsgw = (YAWLServiceGateway) task.getDecompositionPrototype();
		YAnnouncement announcement = _announcer.createAnnouncement(wsgw.getYawlService(), item, YEngineEvent.ITEM_ADD);
	...
	private void fireCompositeTask(YCompositeTask task, YPersistenceManager pmgr)
		...
		if (!_busyTasks.contains(task)) { // don't proceed if task already
											// started
			_busyTasks.add(task);
			_busyTaskNames.add(task.getID());
			if (pmgr != null)
	...
	protected void processEmptyTasks(Set<YAtomicTask> emptyTasks, YPersistenceManager pmgr)
		...
		for (Map.Entry<YIdentifier, YAtomicTask> entry : identifiers.entrySet()) {
			...
			// if net has already completed, ! t_isBusy, so don't attempt to
			// complete
	...
	private void withdrawEnabledTask(YTask task, YPersistenceManager pmgr) throws YPersistenceException {
		...
		if (wItem != null) { // may already have been removed by task.cancel
			...
			YAnnouncement announcement = _announcer.createAnnouncement(wItem, YEngineEvent.ITEM_CANCEL);
			if (announcement != null)
				_announcements.add(announcement);

			// log it
			YEventLogger.getInstance().logWorkItemEvent(pmgr, wItem, YWorkItemStatus.statusWithdrawn, null);
	 ...
	 *
	 * @param caseIDForNet
	 *            the caseid for the net
	 * @param atomicTask
	 *            the atomic task that contains it.
	 */
	private YWorkItem createEnabledWorkItem(YPersistenceManager pmgr, YIdentifier caseIDForNet, YAtomicTask atomicTask)
		...
		boolean allowDynamicCreation = atomicTask.getMultiInstanceAttributes() != null
				&& YMultiInstanceAttributes.CREATION_MODE_DYNAMIC
						.equals(atomicTask.getMultiInstanceAttributes().getCreationMode());

		// creating a new work item puts it into the work item repository
		// automatically.
		YWorkItem workItem = new YWorkItem(pmgr, atomicTask.getNet().getSpecification().getSpecificationID(),
				atomicTask, new YWorkItemID(caseIDForNet, atomicTask.getID()), allowDynamicCreation, false);
	 ...
	 *
	 * @param workItem
	 *            The work item. If null is supplied, this work item cannot be
	 ...
	 * @param outputData
	 *            the document containing output data
	 * @return whether or not the task exited
	 * @throws YDataStateException
	 */
	private synchronized boolean completeTask(YPersistenceManager pmgr, YWorkItem workItem, YAtomicTask atomicTask,
			YIdentifier identifier, Document outputData)
			throws YDataStateException, YStateException, YQueryException, YPersistenceException {
		...
		if (taskExited) {
			if (workItem != null) {
				for (YWorkItem removed : _workItemRepository.removeWorkItemFamily(workItem)) {
					if (!removed.hasCompletedStatus()) { // MI fired or
															// incomplete
					...
					if (parentRunner != null) {
						synchronized (parentRunner) {
							if (_containingCompositeTask.t_isBusy()) {
								...
								parentRunner.processCompletedSubnet(pmgr, _caseIDForNet, _containingCompositeTask,
										dataDoc);

								if (_caseIDForNet == null) {
									_logger.debug("YNetRunner::completeTask() finished local task: " + atomicTask
											+ " composite task: " + _containingCompositeTask
											+ " caseid for decomposed net: " + _caseIDForNet);
								}
							}
						}
					}
		...
		_logger.debug("<-- completeTask: " + atomicTask.getID() + ", Exited=" + taskExited);
	...
	public synchronized void cancel(YPersistenceManager pmgr) throws YPersistenceException {
		...
		if (_containingCompositeTask != null) {
			YEventLogger.getInstance().logNetCancelled(pmgr, getSpecificationID(), this,
					_containingCompositeTask.getID(), null);
		}
		if (isRootNet())
	...
	public synchronized boolean rollbackWorkItem(YPersistenceManager pmgr, YIdentifier caseID, String taskID)
			throws YPersistenceException {
		YAtomicTask task = (YAtomicTask) _net.getNetElement(taskID);
	...
	private void logCompletingTask(YPersistenceManager pmgr, YIdentifier caseIDForSubnet,
		...
		YLogDataItemList logData = null;
		if (logPredicate != null) {
			String predicate = logPredicate.getParsedCompletionPredicate(busyCompositeTask.getDecompositionPrototype());
			if (predicate != null) {
				logData = new YLogDataItemList(new YLogDataItem("Predicate", "OnCompletion", predicate, "string"));
			}
		}
	...
	private boolean warnIfNetNotEmpty(YPersistenceManager pmgr) {
		...
		for (YExternalNetElement element : _net.getNetElements().values()) {
			if (!(element instanceof YOutputCondition)) { // ignore end
															// condition tokens
				if ((element instanceof YCondition) && ((YCondition) element).containsIdentifier()) {
					...
					for (YIdentifier id : exeCondition.getIdentifiers()) {
						YWorkItem executingItem = _workItemRepository.get(id.toString(), element.getID());
						if (executingItem != null)
							executingItem.setStatusToDiscarded(pmgr);
					}
				}
			}
		}
		if (!haveTokens.isEmpty()) {
			...
			msg.append("Although Net [").append(_net.getID()).append("] of case [").append(_caseIDForNet.toString())
	...
	public void restoreObservers() {
		if (_caseObserverStr != null) {
			YAWLServiceReference caseObserver = _engine.getRegisteredYawlService(_caseObserverStr);
			if (caseObserver != null)
				setObserver(caseObserver);
		}
	}
	...
	/**
	 * returns true if the specified workitem is registered with the Time
	 * Service
	 */
	...
	/**
	 * returns the task id of the task that the specified task flows into In
	 * other words, gets the id of the next task in the process flow
	 */
	...
	public boolean evaluateTimerPredicate(String predicate) throws YQueryException {
		...
		if (pos > -1) {
			...
			if (timerVar != null) {
				...
				throw new YQueryException("Unable to find timer state for task named " + "in predicate: " + predicate);
		} else
			throw new YQueryException("Malformed timer predicate: " + predicate);
	}
	...
	public boolean isSuspending() {
		return _executionStatus == ExecutionStatus.Suspending;
	}

	public boolean isSuspended() {
		return _executionStatus == ExecutionStatus.Suspended;
	}

	public boolean isResuming() {
		return _executionStatus == ExecutionStatus.Resuming;
	}
	...
	public boolean hasNormalState() {
		return _executionStatus == ExecutionStatus.Normal;
	}

	public void setStateSuspending() {
		_executionStatus = ExecutionStatus.Suspending;
	}

	public void setStateSuspended() {
		_executionStatus = ExecutionStatus.Suspended;
	}

	public void setStateResuming() {
		_executionStatus = ExecutionStatus.Resuming;
	}
	...
	public void setExecutionStatus(String status) {
		_executionStatus = (status != null) ? ExecutionStatus.valueOf(status) : ExecutionStatus.Normal;
	}
