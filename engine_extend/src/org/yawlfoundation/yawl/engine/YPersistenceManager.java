 ...
 *
 * @author Andrew Hastie (M2 Investments) Date: 21/06/2005 Time: 13:46:54
...
public class YPersistenceManager {
	 ...
	 *
	 * @return true if a transaction is started successfully, false if the
	 *         session already has an active transaction
	 * @throws YPersistenceException
	 *             if there's a problem starting a transaction
	 */
	...
	/**
	 * Causes the supplied object to be updated when the current transaction is
	 * committed.
	 *
	 * @param obj
	 *            The object to be persisted
	...
	/**
	 * Causes the supplied object to be removed from the persistence cache when
	...
	/**
	 * Causes the supplied object to be persisted when the current transaction
	 * is committed. This method simply calls {@link #storeObject(Object)} but
	 * is public in scope.
	 *
	 * @param obj
	 *            The object to be persisted
	...
	/**
	 * Causes the supplied object to be updated within the persistence cache
	 * when the current transaction is committed. This method simply calls
	 * {@link #updateObject(Object)} but is public in scope.
	 *
	 * @param obj
	 *            The object to be persisted
	 */
	...
	/**
	 * Causes the supplied object to be unpersisted when the current transaction
	 * is committed. This method simply calls {@link #deleteObject(Object)} but
	 * is public in scope.
	 *
	 * @param obj
	...
	private synchronized void doPersistAction(Object obj, boolean update) throws YPersistenceException {
		if (logger.isDebugEnabled()) {
			logger.debug("--> doPersistAction: Mode=" + (update ? "Update " : "Create ") + "; Object = "
					+ obj.getClass().getName() + ": " + obj.toString() + "; Object identity = "
	 ...
	 * @throws YPersistenceException
	 *             if there's a problem reading the db
	 */
	 ...
	 *
	 * @param className
	 *            - the name of the class to retrieve instances of
	 ...
	 *            the name of the class to retrieve instances of
	 * @param whereClause
	 *            the condition (without the 'where' part) e.g. "age=21"
	 * @return a List of the instances retrieved
	 * @throws YPersistenceException
	 *             if there's a problem reading the db
	 */
	public List getObjectsForClassWhere(String className, String whereClause) throws YPersistenceException {
		try {
	 ...
	 * @param field
	 *            - the column name which contains the queried value
	 * @param value
	 *            - the value to find in the 'field' column
	 * @return the first (or only) object matching 'where [field] = [value]'
	 */
	public Object selectScalar(String className, String field, String value) throws YPersistenceException {
		String qryStr = String.format("from %s as tbl where tbl.%s=%s", className, field, value);
	 ...
	 */
	public Object selectScalar(String className, String field, long value) throws YPersistenceException {
