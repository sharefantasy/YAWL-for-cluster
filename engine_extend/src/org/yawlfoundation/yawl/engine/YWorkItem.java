...
public class YWorkItem {
	...
	private YWorkItemStatus _prevStatus = null; // this item's next to last
												// status
	...
	private void completeParentPersistence(YPersistenceManager pmgr) throws YPersistenceException {
		...
		synchronized (_parent) { // sequentially handle children
			...
			if (_parent.getChildren().size() > 1) { // short-circuit if not
													// multi-task
	...
	// set by custom service on checkin, and immediately before workitem
	// completes
		...
		if (data != null) {
			...
			for (Element child : data.getChildren()) {
				...
				if (param != null) {
					...
					// add any configurable logging predicates for this
					// parameter
	...
	/**
	 * announces and logs that this workitem has been discarded - ie. left in
	 * the net when the net completed
	 */
	...
	private YLogDataItem getDataLogPredicate(YParameter param, boolean input) {
		...
		if (logPredicate != null) {
			String predicate = input
					? logPredicate.getParsedStartPredicate(param)
