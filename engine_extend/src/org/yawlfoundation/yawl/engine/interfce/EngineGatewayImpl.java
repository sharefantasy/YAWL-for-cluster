...
public class EngineGatewayImpl implements EngineGateway {
	 ...
	 * 
	 * @param persist
	 *            true if a reference to a persisting engine is required
	 * @throws YPersistenceException
	 *             if persist is true and a persisting engine is unavailable
	 */
	 ...
	 * @param persist
	 *            true if a reference to a persisting engine is required
	 * @param gatherHbnStats
	 *            true to turn on hibernate statistics gathering
	 * @throws YPersistenceException
	 *             if persist is true and a persisting engine is unavailable
	 */
	...
	/**
	 * Sets the URL for the 'default' worklist - i.e. the service that will
	 * receive task enabled notifications for those tasks that are not
	 * explicitly associated with a custom service
	 * 
	 * @param url
	 *            the URL of the service that will serve as the default worklist
	 */
	 ...
	 * 
	 * @param stream
	 *            a stream of the file containing the build properties
	 */
	...
	/**
	 * Sets the actual absolute file location which contains the engine's class
	 * file root
	 * 
	 * @param path
	 *            the file path
	...
	public String getWorkItemExpiryTime(String workItemID, String sessionHandle) throws RemoteException {
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
		...
		YWorkItem workItem = _engine.getWorkItem(workItemID);
		return (workItem != null)
				? String.valueOf(workItem.getTimerExpiry())
				: failureMessage("WorkItem with ID '" + workItemID + "' not found.");
	}
	...
	public String getStartingDataSnapshot(String workItemID, String sessionHandle) throws RemoteException {
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
	...
	public String getWorkItemsWithIdentifier(String idType, String itemID, String sessionHandle)
			throws RemoteException {
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
	...
	public String launchCase(YSpecificationID specID, String caseParams, URI caseCompletionURI, String caseID,
		...
		try {
			...
			return _engine.launchCase(specID, caseParams, caseCompletionURI, caseID, logData, sessionHandle, false);
		} catch (YAWLException e) {
			if (e instanceof YPersistenceException) {
				enginePersistenceFailure = true;
			}
	...
	public String launchCase(YSpecificationID specID, String caseParams, URI caseCompletionURI, String logDataStr,
		...
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
		new YLaunchDelayer(specID, caseParams, caseCompletionURI, null, new YLogDataItemList(logDataStr), sessionHandle,
				mSec, true);
		...
		if (workItem != null) {
			if (workItem.getStatus().equals(YWorkItemStatus.statusExecuting)) {
				options.append("<option operation=\"suspend\">"
						+ "<documentation>Suspend the currently active workItem</documentation>" + "<style>post</style>"
			...
			if (workItem.getStatus().equals(YWorkItemStatus.statusEnabled)
				...
				options.append("<option operation=\"start\">" + "<documentation>Starts a work item</documentation>"
						+ "<style>post</style>" + "<url>")
						.append(thisURL)
	...
	public String loadSpecification(String specificationStr, String sessionHandle) {
		...
		try {
			uploadedList = _engine.addSpecifications(specificationStr, false, verificationHandler);
		} catch (Exception e) {
			if (e instanceof YPersistenceException) {
				enginePersistenceFailure = true;
			}
	...
	public String createAccount(String userName, String password, String doco, String sessionHandle) {
		...
		try {
			...
			boolean success = _engine.addExternalClient(new YExternalClient(userName, password, doco));
		...
		if (null != service) {
			if (null == _engine.getRegisteredYawlService(service.getURI())) {
				...
				return failureMessage(
						"Engine has already registered a service with " + "the same URI [" + service.getURI() + "]");
			}
		} else {
	...
	public String removeInterfaceXListener(String listenerURI) {
		return _engine.removeInterfaceXListener(listenerURI)
				? SUCCESS
