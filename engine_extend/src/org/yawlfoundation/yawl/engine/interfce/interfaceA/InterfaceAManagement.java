...
/**
 * Defines the 'A' interface into the YAWL Engine corresponding to WfMC
 * interface 5 - Administration + Monitoring.
 *
 * @author Andrew Hastie Creation Date: 10-Jun-2005
 */
public interface InterfaceAManagement {
	/**
	 * Register an InterfaceA management client with the engine in order to
	 * recieve callbacks.
	 * <P>
	 *
	 * @param observer
	...
	/**
	 * Returns a set of keys for the loaded process specifications currently
	...
	/**
	 * Loads a process specification into the engine from an XML definition file
	 * created by the YAWL Designer.
	 * <P>
	 *
	...
	List<YSpecificationID> addSpecifications(String specificationStr, boolean ignoreErors,
			YVerificationHandler errorMessages) throws JDOMException, IOException, YPersistenceException;
	 ...
	 * <P>
	 *
	 * The process must have been loaded into the engine either when it
	 * initialised (using persisted specifications held in the database), or
	 * specifications added during the lifetime of the current engine instance.
	 * Null is returned if the process ID requested is not found in the engines
	 * internal cache. Further, this returns the newest version of the
	 ...
	 * <P>
	 *
	 * Note: This interface current;y only supported workitems in the following
	 * states:
	 * <li>Enabled
	 * <li>Executing
	 * <li>Fired
	...
	/**
	 * Returns the specification for a loaded specification via the supplied
	 * specification ID.
	 * <P>
	 * 
	 * @param specID
