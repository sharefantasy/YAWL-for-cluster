...
/**
 * An abstract class designed to be implemented by YAWL custom services. It
 * provides access to, and receives event notifications from, the Engine via
 * Interface B.
 *
 * @author Lachlan Aldred Date: 19/03/2004 Time: 11:44:49
 *
...
public abstract class InterfaceBWebsideController {
	...
	/**
	 * Provided that you set up the web.xml file according to the YAWL
	 * documentation you do not need to call this method. It allows implementers
	 * to set up their webapp so that it has the right objects, and correctly
	 * addressed pointers to the engine.
	 * 
	 * @param backEndURI
	 *            the uri of the engine.
	 */
	...
	/**
	 * Receives notification from the engine of an enabled workitem. Typically,
	 * a custom service will implement this method to check out the work item,
	 * and process it as appropriate for the service.
	 * 
	 * @param enabledWorkItem
	 *            the enabled work item
	 */
	 ...
	 * cancelled. A custom service will implement this method to take any
	 * cleanup action on a previously checked out workitem.
	 * 
	 ...
	 * 
	 * @param caseID
	 *            the id of the case that has been cancelled
	 */
	 ...
	 *            the id of the case that has been cancelled
	 * @param tasks
	 *            the list of deadlocked task ids
	 */
	public void handleDeadlockedCaseEvent(String caseID, String tasks) {
	}

	/**
	 * Receives notification from the engine that a case has been started. By
	 * overriding this method a service can process case start events as
	 * required.
	 * 
	 * @param specID
	 *            the specification id of the started case
	 * @param caseID
	 *            the id of the case
	 * @param launchingService
	 *            the uri (String) of the service that started the case
	 * @param delayed
	 *            true if this is a delayed case start, false if immediate
	 */
	...
	/**
	 * Receives notification from the engine that an active case has been
	 * completed. By overriding this method a service can process case
	 * completion events as required.
	 * 
	 * @param caseID
	 *            the id of the completed case.
	 * @param casedata
	 *            the set of net-level data for the case when it completes
	 */
	 ...
	 * expired. Override this method to handle timer expiries on timed
	 * workitems.
	 * 
	 * @param expiredWorkItem
	 *            the workitem that has an expired timer
	 */
	...
	/**
	 * Receives notification from the engine that the status of a workitem has
	 * been modified. Override this method to handle any effects a status change
	 * might have on a custom service
	 * 
	 * @param workItem
	 *            the work item that has had its status modified
	 * @param oldStatus
	 *            the work item's previous status
	 * @param newStatus
	 *            the work item's new status
	 */
	...
	/**
	 * Receives notification when the environment is being shutdown. Override
	 * this method to take any necessary finalisation action.
	 */
	public void destroy() {
	}
	...
	/**
	 * If a custom service is installed outside of a firewall then you will need
	 * to set this method once. Doing so will allow the WSIF code to access Web
	 * services outside your organisation's firewall (http authenticating
	 * proxy).
	 *
	 * NOTE: If your custom YAWL service doesn't need to negotiate a a firewall
	 ...
	 *            mandatory
	 * @param httpProxyHost
	 *            optional (if using a non authenticating proxy firewall).
	 * @param proxyPort
	 *            optional (if using a non authenticating proxy firewall).
	 */
	public void setRemoteAuthenticationDetails(String userName, String password, String httpProxyHost,
			String proxyPort) {
	...
	/**
	 * Override this method to provide a welcome screen for your Custom YAWL
	 * Service For instance you could redirect to a JSP or just write typical
	 ...
	 *            the response.
	 * @throws IOException
	 *             if an error is detected when the servlet handles the GET
	 *             request
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		output.append("<html><head><title>Your Custom YAWL Service Welcome Page</title>" + "</head><body>"
				+ "<H3>Please create a welcome page for your custom YAWL Service</H3>"
				+ "<p>One option is to just override the method "
				+ "\"doGet()\" of  InterfaceBWebsideController, when you extend this " + "class with your own code.</p>"
				+ "<p>alternatively you could redirect to a JSP of your design.</p>" + "</body>" + "</html>");
	 ...
	 * @throws IOException
	 *             if a connection with the engine cannot be established.
	 */
	...
	/**
	 * Creates a session with the engine. A session is automatically
	 * disconnected after 60 minutes of inactivity (although this value can be
	 * changed via config value in the web.xml file).
	 * 
	 * @param userID
	 *            the userID
	 * @param password
	...
	/**
	 * Checks a work item out of the engine. Also stores a local copy of the
	 * active item.
	 * 
	 * @param workItemID
	 *            the work item id.
	 ...
	 *             if the checkout was unsuccessful
	 */
	...
	/**
	 * @deprecated since 2.1 - use checkInWorkItem(String, Element, Element,
	 *             String, String) Checks a work item back into the engine.
	 * @param workItemID
	 *            the work item id.
	 * @param inputData
	 *            the input data.
	 ...
	 * @throws IOException
	 *             if there is a problem contacting the engine.
	 * @throws JDOMException
	 *             if there is a problem parsing XML of input or output data
	 */
	public String checkInWorkItem(String workItemID, String inputData, String outputData, String sessionHandle)
	...
	/**
	 * @deprecated since 2.1 - use checkInWorkItem(String, Element, Element,
	 *             String, String) Checks a work item into the engine.
	 * @param workItemID
	 *            the work item id.
	 * @param inputData
	 ...
	 *             if there is a problem parsing XML of input or output data
	 */
	public String checkInWorkItem(String workItemID, Element inputData, Element outputData, String sessionHandle)
			throws IOException, JDOMException {
	 ...
	 *            completes. May be a simple string or an xml'd YLogDataItemList
	 *            object.
	 * @param sessionHandle
	 *            a valid session handle
	 * @return a diagnostic result of the action - in XML.
	 ...
	 *             if there is a problem parsing XML of input or output data
	 */
	public String checkInWorkItem(String workItemID, Element inputData, Element outputData, String logPredicate,
			String sessionHandle) throws IOException, JDOMException {

		// first, get workitem record from local cache; if not there, check the
		// engine
		// for it; if not there, fail
		WorkItemRecord workitem = getCachedWorkItem(workItemID);
		if (workitem == null)
			workitem = getEngineStoredWorkItem(workItemID, sessionHandle);
		if (workitem == null) {
		...
		// Now if this is beta4 or greater then remove all those input only bits
		// of data
		...
		if (!specData.usesSimpleRootData()) {
			TaskInformation taskInfo = getTaskInformation(specID, workitem.getTaskID(), sessionHandle);
			List<YParameter> outputParams = taskInfo.getParamSchema().getOutputParams();
			filteredOutputData = Marshaller.filterDataAgainstOutputParams(mergedOutputData, outputParams);
		} else {
			filteredOutputData = mergedOutputData;
		}

		String result = _interfaceBClient.checkInWorkItem(workItemID, filteredOutputData, logPredicate, sessionHandle);
	 ...
	 * 
	 * @param workItemID
	 *            the id of the workitem to retrieve
	...
	public WorkItemRecord getEngineStoredWorkItem(String workItemID, String sessionHandle) throws IOException {
		...
		if (successful(itemXML)) {
			WorkItemRecord wir = Marshaller.unmarshalWorkItem(_interfaceBClient.stripOuterElement(itemXML));
			_ibCache.addWorkItem(wir);
			return wir;
		} else
			throw new IOException(StringUtil.unwrap(itemXML));
	}

	/**
	 * Gets metadata of a task.
	 * 
	 * @deprecated superseded by getTaskInformation(YSpecificationID, String,
	 *             String)
	 * @param specID
	 ...
	 *             if there is a problem contacting the engine.
	 */
	public TaskInformation getTaskInformation(String specID, String taskID, String sessionHandle) throws IOException {
	 ...
	 *             if there is a problem contacting the engine.
	 */
	public TaskInformation getTaskInformation(YSpecificationID specID, String taskID, String sessionHandle)
			throws IOException {
		TaskInformation taskInfo = _ibCache.getTaskInformation(specID, taskID);
		if (taskInfo == null) {
			String taskInfoASXML = _interfaceBClient.getTaskInformationStr(specID, taskID, sessionHandle);
	...
	/**
	 * Gets a reference to the authentication configuration for the local
	 * service (if any)
	 * 
	 ...
	 *             if there is a problem contacting the engine.
	 */
	public List<WorkItemRecord> getChildren(String workItemID, String sessionHandle) throws IOException {
		return _interfaceBClient.getChildrenOfWorkItem(workItemID, sessionHandle);
	}

	/**
	 * Logs the failure of a client to contact the YAWL engine. gives some
	 * suggestion of why?
	 * 
	 * @param e
	...
	public static void logContactError(IOException e, String backEndURIStr) {
		Logger.getLogger(InterfaceBWebsideController.class)
				.error("[error] problem contacting YAWL engine at URI [" + backEndURIStr + "]");
		if (e.getStackTrace() != null) {
			Logger.getLogger(InterfaceBWebsideController.class)
					.error("line of code := " + e.getStackTrace()[0].toString());
		}
	}
	...
	/**
	 * Gets the specification data object with the spec id. If called it will
	 * return the specdata object and the entire XML representation of the
	 * specification.
	 * 
	...
	/**
	 * Gets the specification data object with the spec id. If called it will
	 * return the specdata object including the entire XML representation of the
	 * specification.
	 * 
	 * @param specID
	 *            the specification id
			...
			for (SpecificationData data : specs) {
				if (data.getID().equals(specID)) {
					if (data.getAsXML() == null) {
						data.setSpecAsXML(_interfaceBClient.getSpecification(specID, sessionHandle));
					}
	 ...
	 * 
	 * @param enabledWorkItem
	 *            an enabled WorkItemRecord.
	 * @param sessionHandle
	 *            a valid session handle with the engine.
	 * @return a list of work item records that correspond to the executing
	 *         work-items that should be checked back into the engine when the
	 *         task is complete.
	 * @throws IOException
	 *             if there is a problem communicating with the engine.
	 * @throws YAWLException
	 *             if the checkout is unsuccessful
	 */
	protected List<WorkItemRecord> checkOutAllInstancesOfThisTask(WorkItemRecord enabledWorkItem, String sessionHandle)
		...
		_logger.debug("Result of item [" + enabledWorkItem.getID() + "] checkout is : " + result);
		...
		for (WorkItemRecord itemRecord : mixedChildren) {
			if (WorkItemRecord.statusFired.equals(itemRecord.getStatus())) {
				...
				if (firedItem != null)
					instances.add(firedItem);
				_logger.debug("Result of item [" + itemRecord.getID() + "] checkout is : " + firedItem);
			}
		}
		return instances;
	}
	...
	/**
	 * Utility method to prepare the reply element for checking a work item back
	 * into the engine.
	 * 
	 * @param enabledWorkItem
	 *            the enabled work item.
	 * @param sessionHandle
	 *            the session handle
	 * @return an empty root element correctly named according to the version of
	...
	protected Element prepareReplyRootElement(WorkItemRecord enabledWorkItem, String sessionHandle) throws IOException {
		...
		String rootName = sdata.usesSimpleRootData() ? "data" : enabledWorkItem.getTaskName().replaceAll(" ", "_");
	 ...
	 *            the task id
	 * @param sessionHandle
	 *            a valid sessionhandle
	 * @return the task's resourcing specification, or null if it doesn't have
	 *         one (pre v2.0)
	 * @throws IOException
	 *             if there is a problem communicating with the engine.
	 */
	protected Element getResourcingSpecs(YSpecificationID specID, String taskID, String sessionHandle)
			throws IOException {
