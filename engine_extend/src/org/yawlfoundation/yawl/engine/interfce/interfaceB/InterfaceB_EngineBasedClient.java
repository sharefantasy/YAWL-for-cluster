 ...
 *
 * @author Lachlan Aldred Date: 22/01/2004 Time: 17:19:12
 *
...
public class InterfaceB_EngineBasedClient extends Interface_Client implements ObserverGateway {
	...
	private static final ExecutorService _executor = Executors
	 ...
	 *
	 * @param services
	 *            the set of registered custom services
	 * @param specID
	 *            the specification id of the started case
	 * @param caseID
	 *            the case that has started
	 * @param launchingService
	 *            the service that started the case
	 * @param delayed
	...
	public void announceCaseStarted(Set<YAWLServiceReference> services, YSpecificationID specID, YIdentifier caseID,
		...
		for (YAWLServiceReference service : services) {
			_executor.execute(new Handler(service, specID, caseID, launchingService, delayed, CASE_START));
		}
	}
	 ...
	 *            a set of custom services to receive the announcement
	 * @param id
	 *            the case id of the deadlocked case
	 * @param tasks
	 *            the deadlocked tasks
	 */
	...
	public void shutdown() {
		...
		// Nothing else to do - Interface B Clients handle shutdown within their
		// own servlet.
	}
	...
	private class Handler implements Runnable {
		...
		public Handler(YAWLServiceReference yawlService, YIdentifier id, Set<YTask> tasks, YEngineEvent command) {
			_yawlService = yawlService;
			_caseID = id;
			_tasks = tasks;
			_command = command;
		...
		public Handler(YAWLServiceReference yawlService, YSpecificationID specID, YIdentifier id,
				String launchingService, boolean delayed, YEngineEvent command) {
			_yawlService = yawlService;
			_specID = specID;
			_caseID = id;
		...
		public void run() {
			...
			try {
				...
				if (_command == ITEM_ADD) {
					...
					try {
						_logger.warn(
								MessageFormat.format("Failed to announce engine initialisation to {0} at URI [{1}]",
										_yawlService.getServiceName(), _yawlService.getURI()));
					} catch (IllegalStateException ise) {
						// can happen on shutdown when the service has already
						// stopped
		...
		private void redirectWorkItem(boolean connect) {
			...
			if (defWorklist == null) {
				_logger.error(MessageFormat.format(
						...
						connect ? "connect to" : "find", _yawlService.getURI(), _workItem.getIDString()));
				...
				_logger.error(MessageFormat.format("Could not announce enabled workitem [{0}] to default worklist "
						+ "handler at URL [{1}]. Either the handler is missing or offline, " + "or the URL is invalid.",
