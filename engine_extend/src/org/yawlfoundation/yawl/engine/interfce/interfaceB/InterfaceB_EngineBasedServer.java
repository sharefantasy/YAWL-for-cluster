...
public class InterfaceB_EngineBasedServer extends YHttpServlet {
	...
	public void init() throws ServletException {
		...
		try {
			...
			// set flag to disable logging (only if false) - enabled with
			// persistence by
			// default
			...
			// set the path to external db gateway cluster classes (if any)
			...
			int maxWait = StringUtil.strToInt(context.getInitParameter("InitialisationAnnouncementTimeout"), -1);
			if (maxWait >= 0)
				maxWaitSeconds = maxWait;

			// read the current version properties
			_engine.initBuildProperties(context.getResourceAsStream("/WEB-INF/classes/version.properties"));
	...
	private void registerObserverGateway(String gatewayClassName) {
		...
		try {
			...
			// If the class has a getInstance() method, call that method rather
			// than
			// calling a constructor (& thus instantiating 2 instances of the
			// class)
			try {
			...
			if (gateway != null)
				...
				_log.warn("Error registering external ObserverGateway '" + gatewayClassName + "'.");
		} catch (ClassNotFoundException e) {
			_log.warn("Unable to locate external ObserverGateway '" + gatewayClassName + "'.", e);
		} catch (InstantiationException ie) {
			_log.warn("Unable to instantiate external ObserverGateway '" + gatewayClassName
		...
		try {
			...
			if (action != null) {
				if (action.equals("checkConnection")) {
					...
					msg.append(_engine.completeWorkItem(workItemID, data, logPredicate, false, sessionHandle));
					...
					if (mSecStr != null) {
						msg.append(_engine.launchCase(specID, caseParams, completionObserver, logDataStr,
						...
						msg.append(_engine.launchCase(specID, caseParams, completionObserver, logDataStr, date,
								sessionHandle));
						...
						msg.append(
								_engine.launchCase(specID, caseParams, completionObserver, logDataStr, sessionHandle));
				...
				msg.append(_engine.getWorkItemOptions(workItemID, request.getRequestURL().toString(), sessionHandle));
			} else
	...
	private void debug(HttpServletRequest request, String service) {
		_log.debug("\nInterfaceB_EngineBasedServer::do" + service + "() " + "request.getRequestURL = "
				+ request.getRequestURL());
		_log.debug("\nInterfaceB_EngineBasedServer::do" + service + "() request.parameters = ");
