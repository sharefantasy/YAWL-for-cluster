 ...
 *
 * @author Lachlan Aldred Date: 23/01/2004 Time: 13:26:04
 *
...
public class InterfaceB_EnvironmentBasedServer extends HttpServlet {
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		try {
			...
			// If there is an auth proxy firewall and it has been configured it
			// in the
			...
			// if there are overridden engine logon & password, set them for the
			// service
			...
			if (logonName != null)
				_controller.setEngineLogonName(logonName);
			if (logonPassword != null)
				_controller.setEngineLogonPassword(logonPassword);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		if (_debug) {
			_logger.debug(
					"\nInterfaceB_EnvironmentBasedServer " + "request.getRequestURL = " + request.getRequestURL());
			...
			while (paramNms.hasMoreElements()) {
				...
				_logger.debug("\trequest.getParameter(" + name + ") = " + request.getParameter(name));
			}
		}
	...
	private String processPostQuery(HttpServletRequest request) {
		if (_debug) {
			_logger.debug(
					"\nInterfaceB_Server_WebSide::doPost() " + "request.getRequestURL = " + request.getRequestURL());
		...
		WorkItemRecord workItem = (workItemXML != null) ? Marshaller.unmarshalWorkItem(workItemXML) : null;

		// where there are two choices for 'action' below, those on the left are
		// passed from 2.2 or later engine versions, while those on the right
		// come
		...
		if ("announceItemEnabled".equals(action) || "handleEnabledItem".equals(action)) {
			...
			YSpecificationID specID = new YSpecificationID(request.getParameter("specidentifier"),
					request.getParameter("specversion"), request.getParameter("specuri"));
