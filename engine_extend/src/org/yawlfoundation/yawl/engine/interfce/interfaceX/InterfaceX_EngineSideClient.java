...
/**
 * InterfaceX_EngineSideClient passes exception event calls from the engine to
 ...
 *
 * Schematic of Interface X: | EXCEPTION | INTERFACE X GATEWAY | SERVICE
 * (implements) | | (implements) | | | | +==========+ -----> ENGINE-SIDE ---|-->
 * SERVICE-SIDE -----> +=============+ || YAWL || CLIENT | SERVER || EXCEPTION
 * || || ENGINE || | || SERVICE || +==========+ <----- ENGINE-SIDE <--|---
...
public class InterfaceX_EngineSideClient extends Interface_Client implements ExceptionGateway {
	...
	public boolean equals(Object other) {
		return (other instanceof InterfaceX_EngineSideClient) && (getURI() != null)
	...
	public void announceCheckWorkItemConstraints(YWorkItem item, Document data, boolean preCheck) {
		executor.execute(new Handler(_observerURI, item, data, preCheck, NOTIFY_CHECK_ITEM_CONSTRAINTS));
	}

	public void announceCheckCaseConstraints(YSpecificationID specID, String caseID, String data, boolean preCheck) {
		executor.execute(new Handler(_observerURI, specID, caseID, data, preCheck, NOTIFY_CHECK_CASE_CONSTRAINTS));
	}
	...
	/**
	 * Called when the Engine is shutdown (servlet destroyed); the listener
	 * should to do its own finalisation processing
	 */
	...
	/**
	 * Handler class called by the above announcement methods - passes the
	 * events to the service side.
	 */
	...
	private class Handler implements Runnable {
		...
		public Handler(String observerURI, YWorkItem workItem, Document data, boolean preCheck, int command) {
			_observerURI = observerURI;
			_workItem = workItem;
			_preCheck = preCheck;
			_command = command;
		...
		public Handler(String observerURI, YSpecificationID specID, String caseID, String data, boolean preCheck,
				int command) {
			_observerURI = observerURI;
			_specID = specID;
			_caseID = caseID;
