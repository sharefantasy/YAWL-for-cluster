...
/**
 * InterfaceX_ServiceSideServer passes exception event calls from the engine to
...
public class InterfaceX_ServiceSideServer extends HttpServlet {
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		String controllerClassName = context.getInitParameter("InterfaceX_Service");
		try {
	...
	private String processPostQuery(HttpServletRequest request) {
		...
		if (sPreCheck != null)
			preCheck = sPreCheck.equalsIgnoreCase("TRUE");

		boolean primary = true;
		String sPrimary = request.getParameter("primary");
		if (sPrimary != null)
			primary = sPrimary.equalsIgnoreCase("TRUE");
		...
		switch (actionToNotifyType(request.getParameter("action"))) {
			case InterfaceX_EngineSideClient.NOTIFY_CHECK_CASE_CONSTRAINTS :
				_controller.handleCheckCaseConstraintEvent(new YSpecificationID(specID, specVersion, specURI), caseID,
						data, preCheck);
				break;
			...
			case InterfaceX_EngineSideClient.NOTIFY_CANCELLED_CASE :
				...
				break;
			default :
				return "<failure>Unknown action: '" + request.getParameter("action") + "'</failure>";
		}
		return "<success/>";
	}
