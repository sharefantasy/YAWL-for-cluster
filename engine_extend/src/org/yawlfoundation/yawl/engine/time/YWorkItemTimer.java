...
public class YWorkItemTimer implements YTimedObject {
	...
	private String _ownerID; // the owner work item - each can have at most one
								// timer
	...
	public YWorkItemTimer(String workItemID, long units, YTimer.TimeUnit interval, boolean persisting) {
		_ownerID = workItemID;
		_persisting = persisting;
	...
	public boolean equals(Object other) {
		return (other instanceof YWorkItemTimer) && ((getOwnerID() != null)
	...
	public void handleTimerExpiry() {
		...
		if (item != null) {
			...
			// special case: if the workitem timer started on enabled, and the
			// item
			// has since been started, the ownerID now refers to the parent, and
			// so the
			...
			try {
				if (item.getStatus().equals(YWorkItemStatus.statusEnabled)) {
					...
					if (item.requiresManualResourcing()) // not an autotask
						engine.completeWorkItem(item, item.getDataString(), null, YEngine.WorkItemCompletion.Force);
					engine.getAnnouncer().announceTimerExpiryEvent(item);
				}
			} catch (Exception e) {
				// handle exc.
