...
/**
 * The server side of interface E. An API to retrieve data from the process
 * event logs and pass it back as XML.
 * <p/>
 * Create Date: 29/10/2007. Last Date: 12/12/2007 Completely revised for the new
 * logging schema in v2.1 11/09 - 6/10
 *
...
public class YLogServer {
		...
		if (connected()) {
			try {
				...
				if (spec != null) {
					...
					result = "<failure>No records for specification '" + specID.toString() + "'.</failure>";
				}
			} catch (YPersistenceException ype) {
	...
	/**
	 * @param specKey
	 *            the specification PK to get the case instances for
	...
	public String getNetInstancesOfSpecification(long specKey) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr
						.createQuery("select ni from YLogNet as n, YLogNetInstance as ni "
	...
	public String getCaseEvents(long rootNetInstanceKey) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogEvent as e where " + "e.rootNetInstanceID=:key")
						.setLong("key", rootNetInstanceKey).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
					xml.append(String.format("<events rootNetInstanceKey=\"%d\">", rootNetInstanceKey));
					while (itr.hasNext()) {
	...
	public String getCaseEvents(String caseID) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogNetInstance as n where n.engineInstanceID=:caseID")
	...
	public String getTaskInstancesForTask(String caseID, String taskName) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr
						.createQuery("from YLogTaskInstance as ti, YLogTask as t "
								+ "where (ti.engineInstanceID=:caseID " + "or ti.engineInstanceID like :caseIDlike) "
								+ "and ti.taskID=t.taskID " + "and t.name=:taskName")
						.setString("caseID", caseID).setString("caseIDlike", caseID + ".%")
						.setString("taskName", taskName).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
					xml.append(String.format("<taskinstances caseID=\"%s\" taskname=\"%s\">", caseID, taskName));
					while (itr.hasNext()) {
	...
	public String getInstanceEvents(long instanceKey) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogEvent as e where " + "e.instanceID=:key")
						.setLong("key", instanceKey).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
					xml.append(String.format("<events instanceKey=\"%d\">", instanceKey));
					while (itr.hasNext()) {
						YLogEvent event = (YLogEvent) itr.next();
						xml.append(event.toXML());
					}
	...
	public String getDataForEvent(long eventKey) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogDataItemInstance as di where " + "di.eventID=:key")
						.setLong("key", eventKey).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
	...
	public String getDataTypeForDataItem(long dataTypeKey) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogDataType as dt where " + "dt.dataTypeID=:key")
						.setLong("key", dataTypeKey).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
	...
	public String getTaskInstancesForCase(String caseID) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogTaskInstance as ti where " + "ti.engineInstanceID like :key")
						.setString("key", caseID + ".%").iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
	...
	public String getTaskInstancesForTask(long taskKey) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr.createQuery("from YLogTaskInstance as ti where " + "ti.taskID=:key")
						.setLong("key", taskKey).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
	...
	public String getCaseEvent(String caseID, String eventType) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr
						.createQuery("select e from YLogEvent as e, YLogNetInstance as ni where "
								+ "ni.engineInstanceID=:caseID and e.instanceID=ni.netInstanceID "
								+ "and e.descriptor=:eventType")
						.setString("caseID", caseID).setString("eventType", eventType).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
	...
	public String getAllCaseEventsByService(String serviceName, String eventType) {
		...
		if (connected()) {
			try {
				Iterator itr = _pmgr
						.createQuery("select e from YLogEvent as e, YLogService as s where "
								+ "s.serviceID=e.serviceID and " + "s.name=:serviceName and e.descriptor=:event")
						.setString("serviceName", serviceName).setString("event", eventType).iterate();
				if (itr.hasNext()) {
					StringBuilder xml = new StringBuilder();
	...
	public String getSpecificationStatistics(long specKey, long from, long to) {
		...
		if (connected()) {
			try {
				...
				node.addChild("completionAvgtime", StringUtil.formatTime((long) totalCompletedTime / casesCompleted));
				...
				node.addChild("cancelledAvgtime", StringUtil.formatTime((long) totalCancelledTime / casesCancelled));
	...
	private void addNetInstance(XNode node, YLogNetInstance logNetInstance, boolean withData)
		...
		for (Object o : getTaskInstanceObjects(logNetInstance.getNetInstanceID())) {
			...
			// now do any sub-nets
			YLogNetInstance logSubNetInstance = getSubNetInstance(logTaskInstance.getTaskInstanceID());
	...
	public String getCompleteCaseLog(String caseID) {
		...
		if (connected()) {
			try {
				...
				result = String.format("<failure>No full record of case '%s'.</failure>", caseID);
			} catch (YPersistenceException ype) {
				result = GENERAL_ERROR;
			}
		} else
	...
	public String getEventsForTaskInstance(String itemID) {
		...
		if (itemID != null) {
			...
			result = "<failure>Null item id.</failure>";
	...
	private String getFullyPopulatedNetInstance(YLogNetInstance instance, YLogNet net, boolean root)
			throws YPersistenceException {
		StringBuilder xml = new StringBuilder();
		xml.append(String.format("<netinstance key=\"%d\" root=\"%b\">", instance.getNetInstanceID(), root));
		xml.append(String.format("<net name=\"%s\"  key=\"%d\"/>", net.getName(), net.getNetID()));
	...
	private String getFullyPopulatedTaskInstance(YLogTaskInstance instance) throws YPersistenceException {
		StringBuilder xml = new StringBuilder();
	...
	private String getFullyPopulatedDataItem(YLogDataItemInstance instance) throws YPersistenceException {
	...
	private YLogSpecification getSpecification(YSpecificationID specID) throws YPersistenceException {
		if (connected()) {
			String identifier = specID.getIdentifier();
			Iterator itr;
			if (identifier != null) {
				itr = _pmgr
						.createQuery("from YLogSpecification as s where "
								+ "s.identifier=:id and s.version=:version and s.uri=:uri")
						.setString("id", identifier).setString("version", specID.getVersionAsString())
						.setString("uri", specID.getUri()).iterate();
			} else {
				itr = _pmgr.createQuery("from YLogSpecification as s where " + "s.version=:version and s.uri=:uri")
						.setString("version", specID.getVersionAsString()).setString("uri", specID.getUri()).iterate();
			}
			if (itr.hasNext()) {
	...
	private List getNets(long specKey) throws YPersistenceException {
		if (connected()) {
			return _pmgr.createQuery("from YLogNet as n where n.specKey=:specKey").setLong("specKey", specKey).list();
		} else
	...
	private YLogNetInstance getNetInstance(String caseID) throws YPersistenceException {
		if (connected()) {
			...
			return (YLogNetInstance) _pmgr.selectScalar("YLogNetInstance", "engineInstanceID", quotedCaseID);
		} else
			return null;
	}
	...
	private YLogNetInstance getSubNetInstance(long key) throws YPersistenceException {
		if (connected()) {
			return (YLogNetInstance) _pmgr.selectScalar("YLogNetInstance", "parentTaskInstanceID", key);
		} else
			return null;
	}
	...
	private List getNetInstances(String caseID) throws YPersistenceException {
		if (connected()) {
			return _pmgr
					.createQuery("from YLogNetInstance as ni where "
							+ "ni.engineInstanceID=:caseid or ni.engineInstanceID like :likeid")
					.setString("caseid", caseID).setString("likeid", caseID + ".%").list();
		} else
			return Collections.emptyList();
	}
	...
	private YLogTaskInstance getTaskInstance(String itemID) throws YPersistenceException {
		if ((itemID != null) && (connected())) {
			...
			if (taskName != null) {
				List result = _pmgr
						.createQuery("from YLogTaskInstance as ti, YLogTask as t where ti.engineInstanceID=:caseID"
								+ " and t.name=:taskName and t.taskID=ti.taskID")
						.setString("caseID", caseID).setString("taskName", taskName).list();
	...
	private YLogTaskInstance getTaskInstance(long key) throws YPersistenceException {
		if (connected()) {
			return (YLogTaskInstance) _pmgr.selectScalar("YLogTaskInstance", "taskInstanceID", key);
		} else
			return null;
	}
	...
	private List getInstanceEventObjects(long key) throws YPersistenceException {
		if (connected()) {
			return _pmgr.createQuery("from YLogEvent as e where e.instanceID=:key " + "order by e.timestamp")
					.setLong("key", key).list();
		} else
			return Collections.emptyList();
	}
	...
	private List getTaskInstanceObjects(long key) throws YPersistenceException {
		if (connected()) {
			return _pmgr.createQuery("from YLogTaskInstance as ti where ti.parentNetInstanceID=:key")
					.setLong("key", key).list();
		} else
			return Collections.emptyList();
	}
	...
	private List getNetInstanceObjects(long key) throws YPersistenceException {
		if (connected()) {
			return _pmgr.createQuery("from YLogNetInstance as ni where ni.netID=:key").setLong("key", key).list();
		} else
			return Collections.emptyList();
	}
	...
	private List getDataItemInstanceObjects(long key) throws YPersistenceException {
		if (connected()) {
			return _pmgr.createQuery("from YLogDataItemInstance as di where di.eventID=:key").setLong("key", key)
					.list();
		} else
			return Collections.emptyList();
	}
