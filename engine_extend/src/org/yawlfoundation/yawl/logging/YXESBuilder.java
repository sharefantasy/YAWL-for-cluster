...
/**
 * Author: Michael Adams Creation Date: 21/04/2010
 */
public class YXESBuilder {
	...
	private XNode beginLogOutput(YSpecificationID specid) {
		...
		log.addChild(extensionNode("Lifecycle", "lifecycle", "http://code.fluxicon.com/xes/lifecycle.xesext"));
		log.addChild(extensionNode("Time", "time", "http://code.fluxicon.com/xes/time.xesext"));
		log.addChild(extensionNode("Concept", "concept", "http://code.fluxicon.com/xes/concept.xesext"));
		log.addChild(extensionNode("Semantic", "semantic", "http://code.fluxicon.com/xes/semantic.xesext"));
		log.addChild(extensionNode("Organizational", "org", "http://code.fluxicon.com/xes/org.xesext"));
		...
		classifier.addAttribute("keys", "concept:name concept:instance lifecycle:transition");
	...
	private XNode eventNode(XNode yawlEvent, String taskName, String instanceID) {
		...
		eventNode.addChild(stringNode("lifecycle:transition", translateEvent(yawlEvent.getChildText("descriptor"))));
	...
	private void processComplexTypeDataValues(XNode eventNode, XNode valueNode, Map<String, String> typeMap,
			String rootName) {
	...
	private XNode formatDataNode(String name, String value, String typeDefinition) {
		...
		if (tag.equals("date"))
			value = formatDateValue(typeDefinition, value);
	...
	private String formatDateValue(String type, String value) {
		...
		if (type.endsWith("date")) {
			...
			// a date type is of the form CCYY-MM-DD - but the year can have
			// more than
			// 4 chars and may start with '-', while the date may be followed by
			// a Z or
			...
			if (insertPos == value.length()) {
				...
				formattedDateTime = value.substring(0, insertPos) + blankTime + value.substring(insertPos);
			}
