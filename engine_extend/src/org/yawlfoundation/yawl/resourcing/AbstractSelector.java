...
/**
 * The base class inherited by all of the 'selector' classes :- filters,
 * constraints and allocators.
 * <p/>
 * As well as giving an inherited class a meaningful class name, extending
 * classes need to provide values for: _name --> the class name _displayName -->
 * a 'pretty' name to show to designers/users when required _description --> a
 * user-oriented description of what the extending class does _params --> a set
 * of parameters needed when applying the extending class's 'perform...' method
 * (the param value should be set to null when initialised).
 * <p/>
...
public abstract class AbstractSelector implements Comparable<AbstractSelector> {
	...
	protected Map<String, String> _params = new Hashtable<String, String>(); // params
																				// used
																				// by
																				// the
																				// 'selection'
	 ...
	 *
	 * @param paramsMap
	 *            the new parameter map of the form [name, value] (both Strings)
	 */
	...
	/**
	 * Adds a key - ie. an attribute which needs to be assigned a value at
	 * design time
	 *
	 * @param key
	 *            the attribute name
	...
	/**
	 * AbstractSelectors are considered equal if their canonical names and name
	 * fields are equal
	 *
	 * @param other
	...
	/**
	 * Evaluates a list of Sets against an expression. Sets may be unioned (when
	 * the expression operator is '|') or intersected (when '&'). Operator
	 * precedence is strictly left to right. The number of operators in the
	 * expression should be one less than the number of Sets in the List
	 *
	 * @param setList
	 *            the list of sets
	 * @param expression
	 *            the expression containing the operators
	 * @param <T>
	 *            the object type contained in each set
	 ...
	 * @param <T>
	 *            the object type contained in each set
	 * @return A Set containing only those members that are present in both A
	 *         and B
	 */
	...
	/**
	 * @return an xml representation of this object's parameters (if any). Used
	 *         to build the specification xml
	 */
	 ...
	 *
	 * @param eParams
	 * @return a [key, value] map of the parameters described by the
	 *         {@code Element}
	 */
	 ...
	 *
	 * @param outerTag
	 *            a value for the surrounding tag (one of the extended classes)
	 ...
	 *
	 * @param e
	 *            a JDOM Element containing the values
