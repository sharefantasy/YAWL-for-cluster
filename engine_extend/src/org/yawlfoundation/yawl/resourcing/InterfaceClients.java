...
public class InterfaceClients {
	 ...
	 * 
	 * @param logonName
	 *            the resource service's logon name (from web.xml)
	 * @param password
	 *            the resource service's logon password (from web.xml)
	 */
	...
	protected void setInterfaceBClient(InterfaceB_EnvironmentBasedClient client) {
		if (client != null) {
			...
			throw new IllegalArgumentException("Unable to setup engine client. " + "Client = " + client);
	}
	 ...
	 * 
	 * @param engineURI
	 *            the URI of the Engine's Interface B
	 * @param exceptionURI
	 *            the URI of the Worklet Exception Service
	 * @param schedulingURI
	 *            the URI of the Scheduling Service
	 * @param costServiceURI
	 *            the URI of the Cost Service
	 * @param docStoreURI
	 *            the URI of the Document Store
	 */
	public void initClients(String engineURI, String exceptionURI, String schedulingURI, String costServiceURI,
			String docStoreURI) {
		_engineURI = engineURI;
		if (engineURI != null) {
			_interfaceAClient = new InterfaceA_EnvironmentBasedClient(engineURI.replaceFirst("/ib", "/ia"));
			_interfaceEClient = new YLogGatewayClient(engineURI.replaceFirst("/ib", "/logGateway"));
		}
		...
		if (costServiceURI != null) {
			_costServiceClient = new CostClient(costServiceURI, _engineLogonName, _engineLogonPassword);
		}
		if (docStoreURI != null) {
			_docStoreClient = new DocStoreClient(docStoreURI, _engineLogonName, _engineLogonPassword);
		}
	}
	 ...
	 * 
	 * @param client
	 *            the Interface B client from InterfaceBWebsideController
	 */
	 ...
	 * 
	 * @param serviceName
	 *            the name of the service to get the URI for
	...
	protected boolean engineIsAvailable() {
		String errMsg = "Failed to locate a running YAWL engine at URL '" + _engineURI + "'. ";
		int timeout = 5;
	...
	/**
	 * Checks if there is a connection to the engine, and if there isn't,
	 * attempts to connect
	 * 
	...
	protected boolean connected() {
		...
		synchronized (ResourceManager.getInstance().getIBEventMutex()) {
			try {
				// if not connected
				if ((_engineSessionHandle == null) || (_engineSessionHandle.length() == 0)
						|| (!checkConnection(_engineSessionHandle))) {

					if (_interfaceBClient == null)
						setInterfaceBClient(null);

					_engineSessionHandle = _interfaceBClient.connect(_engineLogonName, _engineLogonPassword);
				}
	 ...
	 * @throws IOException
	 *             if there's a problem connection to the Engine through IB
	 */
	...
	public void announceResourceUnavailable(AbstractResource resource, WorkItemRecord wir, boolean primary) {
		if (resource != null) {
			try {
				...
				_gatewayServer.announceResourceUnavailable(resource.getID(), wir, caseData, primary);
			} catch (IOException ioe) {
	...
	public void removeCalendarStatusChangeListener(String uri, String userID) {
		if (userID != null)
			_gatewayServer.removeSchedulingInterfaceListener(userID, uri);
	}

	public void removeCalendarStatusChangeListeners(String userID) {
		if (userID != null)
			_gatewayServer.removeSchedulingInterfaceListeners(userID);
	}
	 ...
	 * 
	 * @param wir
	 *            the work item to be redirected.
	 * @param serviceName
		...
		if (wir != null) { // wir exists...
			if (wir.isEnabledOrFired()) { // and has right status
				...
				if (serviceURI != null) { // service exists...
					...
					if (_interfaceBClient.successful(result)) { // and is online
						try {
							if (_gatewayServer != null) {
								...
								result = fail("Gateway server unavailable");
						} catch (Exception e) {
					...
					result = fail("Unknown or unregistered service name: " + serviceName);
			} else
				result = fail("Only work items with enabled or fired status may be " + "redirected; work item ["
						+ wir.getID() + "] has status: " + wir.getStatus());
		} else
			result = fail(WORKITEM_ERR);
	...
	public String uploadSpecification(String fileContents, String fileName) {
		try {
			...
			return fail("<reason><error>IOException uploading specification " + fileName + "</error></reason>");
		}
	}
	...
	protected String updateExternalClient(String id, String password, String doco) throws IOException {
	...
	protected List<WorkItemRecord> getLiveWorkItemsForCase(String caseID) {
		try {
			return _interfaceBClient.getLiveWorkItemsForIdentifier("case", caseID, getEngineSessionHandle());
		} catch (Exception e) {
	...
	protected String launchCase(YSpecificationID specID, String caseData, YLogDataItemList logList) throws IOException {
		if (_serviceURI == null)
			setServiceURI();
		return _interfaceBClient.launchCase(specID, caseData, getEngineSessionHandle(), logList, _serviceURI);
	}
	...
	protected String launchCase(YSpecificationID specID, String caseData, YLogDataItemList logList, long delay)
		...
		if (_serviceURI == null)
			setServiceURI();
		return _interfaceBClient.launchCase(specID, caseData, getEngineSessionHandle(), logList, _serviceURI, delay);
	}
	...
	protected boolean canAddNewInstance(WorkItemRecord wir) {
		try {
			return _interfaceBClient
					.successful(_interfaceBClient.checkPermissionToAddInstances(wir.getID(), getEngineSessionHandle()));
		} catch (IOException ioe) {
			return false;
		}
	}
	...
	public String getEngineXESLog(YSpecificationID specID, boolean withData) {
		try {
			return _interfaceEClient.getSpecificationXESLog(specID, withData, getEngineSessionHandle());
		} catch (IOException ioe) {
			return null;
		}
	}

	public String getEngineSpecificationStatistics(YSpecificationID specID, long from, long to) {
		try {
			return _interfaceEClient.getSpecificationStatistics(specID, from, to, getEngineSessionHandle());
		} catch (IOException ioe) {
			return null;
		}
	}
	...
	public boolean hasCostClient() {
		return _costServiceClient != null;
	}

	public CostClient getCostClient() {
		return _costServiceClient;
	}

	public boolean hasDocStoreClient() {
		return _docStoreClient != null;
	}

	public DocStoreClient getDocStoreClient() {
		return _docStoreClient;
	}
