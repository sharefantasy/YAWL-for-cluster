 ...
 *
 * @author Michael Adams v0.1, 23/08/2007
 */
...
public class QueueSet {
	...
	// moving from offered/allocated to start occurs when workitem moves from
	// enabled to
	// executing. Thus, the offered/allocated queue contains the parent; the
	// started
	...
	public boolean hasWorkItemInAnyQueue(WorkItemRecord wir) {
		if (wir == null)
			return false;
		for (int queue = WorkQueue.OFFERED; queue <= WorkQueue.SUSPENDED; queue++) {
			if (hasWorkItemInQueue(wir.getID(), queue))
				return true;
		}
		return false;
	}
