 ...
 * perspective, including the loading & maintenance of the org model, and
 * overseeing the distribution of tasks to participants.
 *
...
public class ResourceManager extends InterfaceBWebsideController {
	...
	private final Object _autoTaskMutex = new Object(); // for executing
														// autotasks
	private final Object _ibEventMutex = new Object(); // for synchronizing ib
														// events
	private final Object _removalMutex = new Object(); // for removing
														// participants

	private OrgDataRefresher _orgDataRefresher; // if set, reloads db at
												// intervals
	...
	public static boolean serviceInitialised = false; // flag for init on
														// restore

	private ApplicationBean _jsfApplicationReference; // ref to jsf app manager
														// bean
	...
	public void initOrgDataSource(String dataSourceClassName, int refreshRate) {
		...
		if (_orgdb != null) {
			...
			_isNonDefaultOrgDB = !(_orgdb.getClass().getSimpleName().equalsIgnoreCase("HibernateImpl"));
			...
			if (refreshRate > 0)
				startOrgDataRefreshTimer(refreshRate);
		} else {
			_log.warn("Invalid Datasource: No dataset loaded. " + "Check datasource settings in 'web.xml'");
	...
	public synchronized void finaliseInitialisation() {
		...
		// if tomcat is up, it means this was a 'hot' reload, so the final
		// initialisation
		// things that need a running engine need to be re-run now. If tomcat is
		// not yet
		...
		if (s != null) {
			...
			if (parts.length == 2) {
				try {
					...
					// nothing to do - will fall through to the following
					// statements
				}
			}
	...
	public String registerCalendarStatusChangeListener(String uri, String handle) {
		return _services.registerCalendarStatusChangeListener(uri, getUserIDForSessionHandle(handle));
	}
	...
	public void handleDeadlockedCaseEvent(String caseID, String tasks) {
		...
		handleCancelledCaseEvent(caseID); // just for cleaning up purposes, if
											// any
	}
	...
	// here we are only interested in items the service knows about, being
	// updated
	...
	public void handleWorkItemStatusChangeEvent(WorkItemRecord wir, String oldStatus, String newStatus) {
		synchronized (_ibEventMutex) {
			...
			if (!((cachedWir == null) || newStatus.equals(cachedWir.getStatus()))) {
				...
				// if it has been 'suspended', find it on a 'started' queue &
				// move it
				...
				// if it is 'Is Parent', its just been newly started and has
				// spawned
				// child items. Since we don't know who started it, all we can
				// do is
				// pass responsibility to the starting service & remove
				// knowledge of it
	...
	public void handleStartCaseEvent(YSpecificationID specID, String caseID, String launchingService, boolean delayed) {
		...
		if (delayed && launchingService != null && launchingService.equals(_services.getServiceURI())) {
			if (!_cache.logDelayedCaseLaunch(specID, caseID)) {
				EventLogger.log(specID, caseID, null, true); // log without
																// launcher info
			}
		}
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		String fileName = System.getenv("CATALINA_HOME") + "/webapps/resourceService/welcome.htm";
		...
		if (new File(fileName).exists()) {
			...
			output.append("<html><head><title>YAWL Resource Service</title>")
					.append("</head><body><H3>Welcome to the YAWL Resource Service").append("</H3></body></html>");
	...
	private void doFinalServiceToEngineInitialisation(boolean reloaded) {
		synchronized (_ibEventMutex) {
			...
			// if the engine or the service has been restarted during this
			// session
			...
			if (!_services.engineIsAvailable())
				return;
			...
			// if this is the first time the engine has started since service
			// start...
	...
	public void loadResources() {
		if (_orgdb != null) {
			...
			if (!_orgDataRefreshing)
				_resAdmin.createWorkQueues(_persisting);
		} else {
			_orgDataSet = new EmptyDataSource().getDataSource();
		}
	}
	...
	/**
	 * This does final initialisation tasks involved in ensuring the caches
	 * match the engine's known work. It is called via the
	 * EngineInitialisationCompleted event, because it needs the engine to be
	 * completely initialised first. It may also be executed via the 'synch'
	 * button on the admin queues.
	 */
	...
	private List<String> syncCacheWithEngineItems() {
		...
		try {
			...
			for (WorkItemRecord wir : engineItems) {
				if (wir.isAutoTask())
					continue; // ignore automated tasks

				if (!_workItemCache.containsKey(wir.getID())) {

					// Parent items are treated differently. If they have never
					// been started
					// they should be in the cache, otherwise at least one child
					// will be cached
					if (wir.getStatus().equals(WorkItemRecord.statusIsParent)) {
						List children = getChildren(wir.getID());
						if (!((children == null) || children.isEmpty())) { // has
																			// kids
							continue; // so ignore
						}
					}
					...
					_log.warn("Engine workItem '" + wir.getID()
							...
							+ " and placed on the Administrator's 'Unoffered' queue" + " for manual processing.");
				}
	...
	private void removeOrphanedItemsFromCache(List<String> engineIDs) {
		...
		for (String missingID : missingIDs) {
			...
			_log.warn("Cached workitem '" + missingID + "' did not exist in the Engine and was removed.");
		}
	}
	...
	private void cleanseQueues() {
		for (Participant p : _orgDataSet.getParticipants()) {
			...
			if (qSet != null) {
				...
				if (wq != null) {
					for (WorkItemRecord wir : wq.getAll())
		...
		if (q != null)
			q.cleanse(_workItemCache);
	}

	private void finaliseNonDefaultLoad() {

		// for each entity set not supplied by the backend, load the service
		// defaults.
	...
	private void restoreWorkQueues() {
		...
		if (qList != null) {
			for (WorkQueue wq : qList) {
				...
				Hibernate.initialize(wq.getQueueAsMap()); // avoid lazy loading
															// errors
	...
	public boolean isKnownUserID(String userid) {
		return _cache.isKnownUserID(userid);
	}
		...
		if (!isKnownUserID(p.getUserID())) {
			...
			if (!newID.startsWith("<fail")) {
				...
				if (_isNonDefaultOrgDB) {
					...
					_orgDataSet.updateParticipant(p);
	...
	public void updateParticipant(Participant p) {
		if (_orgDataSet.updateParticipant(p)) {
			_cache.addUserKey(p); // and the userid--pid map
			if (_isNonDefaultOrgDB) {
				_persister.update(p.getUserPrivileges()); // persist other
															// classes
	...
	public Set<Participant> getParticipantsAssignedWorkItem(String workItemID, int queueType) {
	...
	public WorkItemRecord offerToAll(WorkItemRecord wir) {
		if (_orgDataSet.getParticipantCount() > 0) {
			...
			for (Participant p : _orgDataSet.getParticipants()) {
				QueueSet qSet = p.getWorkQueues();
				if (qSet == null)
					qSet = p.createQueueSet(_persisting);
	...
	public WorkItemRecord acceptOffer(Participant p, WorkItemRecord wir) {
		...
		if ((starter != null) && (starter.getInitiator() == AbstractInteraction.SYSTEM_INITIATED)) {
			...
			// either start is user-initiated or there's no resource map (beta
			// spec)
	...
	private void mapDeferredChoice(WorkItemRecord wir) {
		...
		if (itemGroup != null) {
			...
			_cache.addDeferredItemGroup(new TaggedStringList(defID, wir.getID()));
	}
	...
	// Deals with live workitems in a participant's queues when the participant
	// is
	// removed. An admin is advised to manually reallocate items before removing
	// this p.,
	// but this is the default behaviour if there are still items in the queue.
	// The strategy is:
	// - Offered: if this is the only p. that has received this offer, give it
	// back to
	// the admin for re-offering. If others have been offered the same item,
	// there's nothing more to do.
	// - Allocated: give it back to admin for reallocating
	// - Started: forceComplete items (since we need another p. to reallocate
	// to)
		...
		synchronized (_removalMutex) {
			if (wq != null) {
				...
				if (wq.getQueueType() == WorkQueue.OFFERED)
					...
					handleStartedQueuesOnRemoval(null, wq);
			}
		}
	}
	...
	public void handleStartedQueuesOnRemoval(Participant p, WorkQueue qStart) {
		synchronized (_removalMutex) {
			if (qStart != null) {
				...
				// if called during restore, there's no engine available yet, so
				// we have to
				// save the wir until there is one; otherwise, we can do the
				// checkin now
	 ...
	 * <p/>
	 * Note that when an item is checked out of the engine, at least one child
	 * item is spawned, and that is the item that executes (i.e. not the
	 * parent).
	 *
	 * @param p
	 *            the participant starting the workitem
	 * @param wir
	 *            the item to start
	...
	public boolean start(Participant p, WorkItemRecord wir) {
		...
		if (checkOutWorkItem(wir)) {
			...
			ResourceMap rMap = getResourceMap(wir);
			if (rMap != null) {
				rMap.removeIgnoreList(wir); // cleanup deallocation list for
											// item (if any)
				rMap.getSecondaryResources().engage(oneToStart); // mark SR's as
																	// busy
			}
	...
	private void reallocateWorkItem(Participant pFrom, Participant pTo, WorkItemRecord wir, EventLogger.event event) {
	...
	public String pileWorkItem(Participant p, WorkItemRecord wir) {
		...
		if (hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_PILE)) {
			...
			result = "Cannot pile task: insufficient privileges.";
	...
	// ISSUE: If p is currently logged on, we'll use p's handle (the engine will
	// use
	// it to log p as the starter). If p is not logged on, the service's handle
	// has to be used, and thus the service will be logged as the starter. There
	// is
		...
		if (!data.getName().equals(wir.getTaskName().replace(' ', '_'))) {
			...
			try {
				...
				if (validator.validateSchema()) {
					...
					// a YDataValidationException is thrown here if validation
					// fails
	...
	public boolean routeChainedWorkItem(Participant p, WorkItemRecord wir) {
		...
		return _cache.getSessionHandle(p) != null && routeWorkItem(p, wir, getEngineSessionHandle());
	}
	...
	public String addChain(Participant p, WorkItemRecord wir) {
		...
		if (!_cache.isChainedCase(caseID)) {
			...
			result = "Cannot chain: case already chained by another user.";
		return result;
	}
	...
	private void cleanCaches() {
		...
		if ((liveCases != null) && (!liveCases.isEmpty())) {
			for (String id : _cache.getChainedCaseIDs()) {
				if (!liveCases.contains(id))
					_cache.removeChainedCase(id);
			}
			// for (WorkItemRecord wir : _workItemCache.values()) {
			// if (!liveCases.contains(wir.getRootCaseID()))
			// _workItemCache.remove(wir);
			// }
		}
	}
	...
	public ResourceMap getPersistedPiledTask(YSpecificationID specID, String taskID) {
		...
		String where = String.format("_specID.identifier='%s' and _specID.version.version='%s' and _taskID='%s'",
	...
	public void deletePersistedPiledTasks(YSpecificationID specID) {
		String where = String.format("_specID.identifier='%s' and _specID.version.version='%s'", specID.getIdentifier(),
				specID.getVersionAsString());
		List mapList = _persister.selectWhere("ResourceMap", where);
		if (mapList != null) {
	...
	public boolean mapSetContains(Set<ResourceMap> mapSet, ResourceMap other) {
		for (ResourceMap map : mapSet) {
			if (map.equals(other) && map.getPiledResourceID().equals(other.getPiledResourceID())) {
				return true;
			}
		}
		return false;
	 ...
	 * or turns an existing timer off if interval < 0
	 *
	 * @param interval
	...
	public ResourceMap getResourceMap(WorkItemRecord wir) {
		...
		// if we don't have a resource map for the task stored yet, let's make
		// one
		if (map == null) {
			try {
				YSpecificationID specID = new YSpecificationID(wir);
				String taskID = wir.getTaskID();
				Element resElem = getResourcingSpecs(specID, taskID, getEngineSessionHandle());

				if ((resElem != null) && successful(JDOMUtil.elementToString(resElem))) {
	 ...
	 *
	 * @param wir
	 *            - the workitem to get the decomp id for
	 */
	...
	private void checkCacheForWorkItem(WorkItemRecord wir) {
		if (getCachedWorkItem(wir.getID()) == null) {
			...
			// if the item is not locally cached, it means a restore has
			// occurred
			// after a checkout & the item is still checked out, so lets put it
			// back
	 ...
	 *
	 * @param p
	 *            - the participant checking in the item
	 * @param wir
	 *            - workitem to check into the engine
	...
	public String checkinItem(Participant p, WorkItemRecord wir) {
		...
		try {
			...
			if (wir != null) {
				...
				if (outData == null)
					outData = wir.getDataList();
				checkCacheForWorkItem(wir);
				if (p != null)
					_cache.addTaskCompleter(p, wir);
				result = checkInWorkItem(wir.getID(), wir.getDataList(), outData, getCompletionLogPredicate(p, wir),
						getEngineSessionHandle());
				if (successful(result)) {
					EventLogger.log(wir, (p != null) ? p.getID() : "", EventLogger.event.complete); // log
																									// it
																									// immediately
	...
	private void freeSecondaryResources(WorkItemRecord wir) {
		ResourceMap rMap = getResourceMap(wir);
		if (rMap != null) {
			rMap.getSecondaryResources().disengage(wir); // mark SR's as
															// released
		}
	}
	...
	private String getCompletionLogPredicate(Participant p, WorkItemRecord wir) {
		...
		if (wir.isDocumentationChanged()) {
			YLogDataItem docoItem = new YLogDataItem("Predicate", "Documentation", wir.getDocumentation(), "string");
			YLogDataItemList itemList = new YLogDataItemList(docoItem);
			if (decompPredicate != null) {
				YLogDataItem decompItem = new YLogDataItem("Predicate", "Complete", decompPredicate, "string");
	...
	public String login(String userid, String password, String jSessionID) {
		if (userid.equals(ADMIN_STR))
			return loginAdmin(password, jSessionID);
	...
	private String loginAdmin(String password, String jSessionID) {
		String handle;
		if (!_cache.hasClientCredentials())
			setAuthorisedServiceConnections();
		...
		if (successful(adminPassword)) {
			...
			handle = adminPassword; // an error message
	...
	public void shutdown() {
		try {
			_cache.shutdown();
			_persister.closeDB();
			if (_orgDataRefresher != null)
				_orgDataRefresher.cancel();
	...
	public Set<SpecificationData> getSpecList() {
		try {
			return new HashSet<SpecificationData>(getSpecificationPrototypesList(getEngineSessionHandle()));
		} catch (IOException ioe) {
	...
	public SpecificationData getSpecData(YSpecificationID spec) {
		...
		if (result == null) {
			try {
				...
				if (result != null)
					_cache.addSpecificationData(result);
			} catch (IOException ioe) {
	...
	/**
	 * Cancels the case & removes its workitems (if any) from the service's
	 * queues & caches. Note: this method is synchronised to prevent any clash
	 * with 'handleCancelledCaseEvent', which is triggered by the call to
	 * cancelCase in this method.
	 *
	 * @param caseID
	 *            the case to cancel
	 * @return a message from the engine indicating success or otherwise
	 * @throws IOException
	 *             if there's trouble talking to the engine
	 */
	...
	public String unloadSpecification(YSpecificationID specID) throws IOException {
		...
		if (successful(result)) {
			...
			deletePersistedPiledTasks(specID); // for persisted tasks with
												// unloaded maps
		}
		return result;
	}

	public String launchCase(YSpecificationID specID, String caseData, String handle) throws IOException {
	...
	public String launchCase(YSpecificationID specID, String caseData, String handle, long delay) throws IOException {
		String result = _services.launchCase(specID, caseData, getLaunchLogData(), delay);
		if (successful(result)) {
			_cache.addDelayedCaseLaunch(new DelayedLaunchRecord(specID, _cache.getWhoLaunchedCase(handle), delay));
		}
		return result;
	}

	public String launchCase(YSpecificationID specID, String caseData, String handle, Date delay) throws IOException {
		String result = _services.launchCase(specID, caseData, getLaunchLogData(), delay);
		if (successful(result)) {
			_cache.addDelayedCaseLaunch(new DelayedLaunchRecord(specID, _cache.getWhoLaunchedCase(handle), delay));
		}
	...
	public String launchCase(YSpecificationID specID, String caseData, String handle, Duration delay)
			throws IOException {
		String result = _services.launchCase(specID, caseData, getLaunchLogData(), delay);
		if (successful(result)) {
			_cache.addDelayedCaseLaunch(new DelayedLaunchRecord(specID, _cache.getWhoLaunchedCase(handle), delay));
	...
	public String getTaskParamsAsXML(String itemID) throws IOException {
		...
		if (wir != null) {
			...
			return fail(WORKITEM_ERR + ": " + itemID);
	}
	...
	public Map<String, FormParameter> getWorkItemParamsInfo(WorkItemRecord wir) throws IOException, JDOMException {
		...
		Element itemData;
		if (wir.isEdited()) {
			wir = _workItemCache.get(wir.getID()); // refresh data list if
													// required
	...
	public String addExternalClient(YExternalClient client) throws IOException {
		...
		if (successful(result)) {
			_cache.addClientCredentials(client.getUserName(), PasswordEncryptor.encrypt(client.getPassword(), ""));
		}
		return result;
	}
	...
	public String updateExternalClient(String id, String password, String doco) throws IOException {
	...
	// any services or client apps authorised to connect to the engine may
	// connect
	// to this service too.
	...
	public Map<String, Element> getSpecificationDataSchema(YSpecificationID specID) throws IOException {
	...
	public boolean assignUnofferedItem(WorkItemRecord wir, String[] pidList, String action) {
		boolean result = true;
		if (wir != null) {
	...
	private void handleAutoTask(WorkItemRecord wir, boolean timedOut) {
		...
		// if this autotask has started a timer, don't process now - wait for
		// timeout
		if ((!timedOut) && (wir.getTimerTrigger() != null))
			return;
		...
		synchronized (_autoTaskMutex) {
			...
			if (checkOutWorkItem(wir)) {
				...
				if ((children != null) && (!children.isEmpty())) {
					wir = (WorkItemRecord) children.get(0); // get executing
															// child
					EventLogger.logAutoTask(wir, true); // log item start
					processAutoTask(wir, true);
				}
			} else
				_log.error("Could not check out automated workitem: " + wir.getID());
		}
	}
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(codelet)) {
				...
				if (taskInfo != null) {
					...
					if (_persisting && runner.persist())
						persistAutoTask(wir, true);
					...
					_log.error(
							MessageFormat.format(
									...
									codelet, wir.getID()));
					checkInAutoTask(wir, wir.getDataList()); // check in
																// immediately
				}
			} else {
			...
			_log.error("Exception attempting to execute automatic task: " + wir.getID(), e);
		}
	}
	...
	private void persistAutoTask(WorkItemRecord wir, boolean isSaving) {
		if (isSaving) {
			...
			PersistedAutoTask task = (PersistedAutoTask) _persister.selectScalar("PersistedAutoTask", wir.getID());
			if (task != null)
				task.unpersist();
		}
		_persister.commit();
	...
	public void checkInAutoTask(WorkItemRecord wir, Element outData) {
		checkCacheForWorkItem(wir); // won't be cached if this is a restored
									// item
		try {
			if (_persisting)
				persistAutoTask(wir, false);
			String msg = checkInWorkItem(wir.getID(), wir.getDataList(), outData, null, getEngineSessionHandle());
			if (successful(msg)) {
				...
				_log.error("Automated task '" + wir.getID() + "' could not be successfully completed. Result message: "
						+ msg);
		} catch (Exception e) {
			_log.error("Exception attempting to check-in automatic task: " + wir.getID(), e);
		}
	}
	 ...
	 *
	 * @param in
	 *            - the JDOM Element containing the input params
	 * @param out
	 *            - the JDOM Element containing the output params
	...
	private TaskInformation getTaskInformation(YSpecificationID specID, String taskID) throws IOException {
	...
	public String getWorkItemDurationsForParticipant(YSpecificationID specID, String taskName, String pid) {
		LogMiner miner = LogMiner.getInstance();
	 ...
	 *
	 * @param itemID
	 *            the id of the work to be redirected.
	 * @param serviceName
	 *            the name of the service to redirect it to
	 * @return a success or diagnostic error message
	 * @pre The item id refers to a work item that is currently in the list of
	 *      items known to the Resource Service, and the work item has enabled
	 *      or fired status
	...
	public String redirectWorkItemToYawlService(String itemID, String serviceName) {
		...
		if (wir == null)
			return fail("Unknown work item: " + itemID);
