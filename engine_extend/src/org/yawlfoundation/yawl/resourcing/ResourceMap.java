...
/**
 * Manages all of the resourcing requirements and distribution strategies for
 * one task.
 *
...
public class ResourceMap {
	...
	private HashMap<String, HashSet<Participant>> _offered = new HashMap<String, HashSet<Participant>>();
	...
	public void removePiledResource() {
		...
		if (isPersisting()) {
			...
			// have to get persisted map first, so we can delete it (since
			// 'this' is not
	...
	public WorkItemRecord distribute(WorkItemRecord wir) {
		...
		// if this task is piled, send directly to the piled participant's
		// started queue
			...
			if (distributionSet != null) {
				...
				if (!routed) {
					...
					if (!distributionSet.isEmpty()) {
						...
						// ignored p's --> empty distribution set: put in
						// unoffered
	...
	private Participant doAllocate(HashSet<Participant> pSet, WorkItemRecord wir) {
		...
		if (_allocate.getInitiator() == AbstractInteraction.USER_INITIATED) {
			...
			// for each participant in set, place workitem on their offered
			// queue
	...
	private void removeIgnoredParticipants(WorkItemRecord wir, HashSet<Participant> distributionSet) {
		TaggedStringList ignoredForWorkItem = getIgnoredList(wir.getID());
		if (ignoredForWorkItem != null) {
	...
	/**
	 * Parse the Element passed for task resourcing info and build the
	 * appropriate objects.
	 * 
	 * @param eleSpec
