...
public class RuntimeCache {
	...
	// a cache of connections directly to the service from client apps &
	// services
	...
	// local cache of specification data schemas: id -> [name, schema defn.
	// element]
	...
	private Map<String, TaggedStringList> _deferredItemGroups = new Hashtable<String, TaggedStringList>();
	...
	// cache of who completed tasks, for four-eyes and retain familiar use
	// <caseid, cache>
	private Map<String, FourEyesCache> _taskCompleters = new Hashtable<String, FourEyesCache>();

	// map of workitem id -> CodeletRunner running codelet for it
	private Map<String, CodeletRunner> _codeletRunners = new Hashtable<String, CodeletRunner>();

	// started workitems that have been restored for a no longer existing
	// participant.
	...
	private SortedSet<DelayedLaunchRecord> _delayedLaunches = Collections
			.synchronizedSortedSet(new TreeSet<DelayedLaunchRecord>());
	...
	protected Set<Participant> getTaskCompleters(String taskID, String caseID) {
		...
		FourEyesCache cache = _taskCompleters.get(caseID);
		if (cache != null)
			result = cache.getCompleters(taskID);
		return result;
	}
	...
	protected void removeTaskCompleter(Participant p, WorkItemRecord wir) {
		...
		if (cache != null)
			cache.removeCompleter(wir.getTaskID(), p);
	}
	...
	protected void cancelCodeletRunnersForCase(String caseID) {
		Set<String> toRemove = new HashSet<String>(); // avoid concurrency
														// exception
	...
	protected List<WorkItemRecord> getOrphanedItems() {
		return _orphanedStartedItems != null ? _orphanedStartedItems : Collections.<WorkItemRecord> emptyList();
	}

	// reference no longer needed (only used on startup)
	protected void clearOrphanedItems() {
		_orphanedStartedItems = null;
	}
	...
	protected String getUserIDForSessionHandle(String handle) {
		Participant p = getParticipantWithSessionHandle(handle); // try users
																	// first
		if (p != null) {
			return p.getUserID();
		}
