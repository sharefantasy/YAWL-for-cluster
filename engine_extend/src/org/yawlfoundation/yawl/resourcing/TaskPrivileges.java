...
/**
 * Maps each of a set of User-Task Privileges to a set of Participants (if any)
 * for one particular task. The mappings are listed in the specification file,
 * and may map a privilege to individual participants and/or all the
 * participants that fulfil a certain Role or Roles.
 *
...
public class TaskPrivileges {
	...
	/**
	 * @return true if all participants have been granted the specified
	 *         privilege
	 */
	...
	/**
	 * these 3 add methods adds to a privilege set a Participant, a Participant
	 * by ID, and a Set of Particpants respectively
	 */
	 ...
	 * @param id
	 *            the id of the Participant (must be a known Participant id)
	 */
	...
	/**
	 * grant the privilege to the Participant without checking the validity of
	 * the id. Written to allow the editor to generate specification xml
	 */
	 ...
	 *
	 * @param e
	 *            the element containing the privileges from the spec xml
	 */
	public void parse(Element e, Namespace nsYawl) {
		if (e != null) {
			...
			for (Element ePrivilege : ePrivileges) {
				...
				// get the set of participant and/or role tags for this
				// privilege
				...
				for (Element eResource : eSet.getChildren()) {
					...
					// if it's a 'role' child, unpack it to a set of
					// participants ...
					if (eResource.getName().equals("role")) {
						String rid = eResource.getText();
						Set<Participant> pSet = ResourceManager.getInstance().getOrgDataSet().getRoleParticipants(rid);
						addSet(privSet, pSet);
