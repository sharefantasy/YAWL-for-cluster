...
public abstract class AbstractAllocator extends AbstractSelector {
	 ...
	 * 
	 * @return an XML'd String containing the member values of an instantiation
	 *         of this class
	 */
	public String toXML() {
	...
	/**
	 * Gets a list of all resource log rows for a given specification + task +
	 * event combination.
	 * 
	 * @param wir
	 *            a workitem record which is an instance of the
	 *            specification+task in question.
	 * @param event
	 *            the type of event to get records for
	 * @return the matching list of ResourceEvent objects (each one a row of the
	 *         log)
	 */
	protected List getLoggedEvents(WorkItemRecord wir, EventLogger.event event) {
		...
		if (persister != null) {
			...
			List events = persister.selectWhere("ResourceEvent",
					String.format("_event='%s' AND tbl._specKey=%d AND tbl._taskID='%s'", eventStr, specKey, taskName));
			persister.commit();
			return events;
		} else
			return null;
	...
	/**
	 * Gets from the cost service the cost per msec of each of the participants
	 * ids listed for the work item
	 * 
	 * @param pids
	 *            a set of participant ids
	 * @param wir
	 *            the work item to allocate
	 * @return a map containing each participant id and its corresponding cost
	 *         per msec to perform this work item
	 */
	protected Map<String, XNode> getCostMap(Set<String> pids, WorkItemRecord wir) {
		...
		try {
			...
			String xml = client.getResourceCosts(new YSpecificationID(wir), wir.getTaskName(), pids, handle);
	...
	/**
	 * Gets the average duration (in msecs) between two events for each
	 * participant recorded against those events for a task
	 * 
	 * @param fromEvent
	 *            the earlier event (start of duration)
	 * @param toEvent
	 *            the later event (end of duration)
	 * @param wir
	 *            a work item to allocate
	 * @return a Map of participant ids and average durations
	 */
	protected Map<String, Long> getAvgDurations(EventLogger.event fromEvent, EventLogger.event toEvent,
			WorkItemRecord wir) {
	 ...
	 * 
	 * @param fromEvents
	 *            the list of earlier events (start of duration)
	 * @param toEvents
	 *            the list of later events (end of duration)
	...
	/**
	 * Gets a set of durations for each participant referenced by a map of event
	 * pairs
	 * 
	 * @param pairedEvents
	 *            a set of event pairs
	...
	protected class EventPair {
		...
		boolean hasPair() {
			return !((fromEvent == null) || (toEvent == null));
		}

		long getDuration() {
			return toEvent.get_timeStamp() - fromEvent.get_timeStamp();
		}
	}
	 ...
	 * 
	 * @param resources
	 *            the distribution set of participants
	 * @param wir
	 *            the work item to allocate
	 * @return the Participant chosen by the allocation strategy
	 */
	public abstract Participant performAllocation(Set<Participant> resources, WorkItemRecord wir);
