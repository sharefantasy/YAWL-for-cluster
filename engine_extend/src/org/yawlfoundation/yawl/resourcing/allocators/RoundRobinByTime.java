...
/**
 * Allocates a workitem to a participant on a round-robin basis, to the
 * participant who has not performed the task for the longest time.
 *
...
public class RoundRobinByTime extends AbstractAllocator {
	...
	public RoundRobinByTime() {
		...
		setDescription("The Round-Robin (by Time) allocator distributes a workitem to the "
				+ "participant in the distribution set who performed " + "the task the least recently.");
	}
				...
				if (!events.isEmpty()) {
					for (Participant p : participants) {
						...
						if (eventTime == Long.MAX_VALUE) {
							chosen = p;
							break; // this p has never performed item
						} else {
