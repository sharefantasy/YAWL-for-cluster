...
/**
 * Performs allocation based on work queue length. This is the default
 * allocator.
 *
...
public class ShortestQueue extends AbstractAllocator {
	...
	/**
	 * selects the Participant with the least number of workitems in the
	 * 'allocated' workqueue, or the first participant with an empty queue
	 * 
	 * @param resSet
	 *            the set of Participants to compare
	 * @param wir
	...
	public Participant performAllocation(Set<Participant> resSet, WorkItemRecord wir) {
		...
		for (Participant p : resSet) {
			...
			if (qSize < shortest) {
				...
				result = p;
				if (qSize == 0)
					break; // got an empty queue, go no further
			}
		}
		return result;
	}
