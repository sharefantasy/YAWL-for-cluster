 ...
 * NOTE: The caller is responsible for committing all db transactions performed
 * by any methods of this class.
 * 
...
public class CalendarLogger {
	...
	public List getLogEntriesForCalendarKey(long calEntryID) {
		List list = _persister
				.createQuery(
						"FROM CalendarLogEntry AS cle " + "WHERE cle.calendarKey=:key " + "ORDER BY cle.entryID DESC")
				.setLong("key", calEntryID).list();
		return (list == null) || list.isEmpty() ? null : list;
	}

	public List getLogEntriesForReservation(String caseID, String activityName, String resourceRec) {
		return _persister
				.createQuery("FROM CalendarLogEntry AS cle " + "WHERE cle.caseID=:caseID "
						+ "AND cle.activityName=:activityName " + "AND cle.resourceRec=:resourceRec")
				.setString("caseID", caseID).setString("activityName", activityName)
				.setString("resourceRec", resourceRec).list();
	}
	...
	public List getLogEntriesForActivity(String caseID, String activityName) {
		return _persister
				...
				.setString("caseID", caseID).setString("activityName", activityName).list();
	}

	public List getLogEntriesForCase(String caseID) {
		return _persister.createQuery("FROM CalendarLogEntry AS cle " + "WHERE cle.caseID=:caseID ")
				.setString("caseID", caseID).list();
	}
