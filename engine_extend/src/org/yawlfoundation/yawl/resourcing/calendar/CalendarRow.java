...
public class CalendarRow extends CalendarEntry {
	...
	public CalendarRow(CalendarEntry entry) {
		super(entry.getResourceID(), entry.getStartTime(), entry.getEndTime(), entry.getStatus(), entry.getWorkload(),
				entry.getAgent(), entry.getComment());
	...
	private String getTimeAsString(long time) {
		...
		return ((diff >= 0) && (diff <= MSECS_IN_ONE_DAY)) ? timeFormat.format(time) : extdFormat.format(time);
	}

	// required for hibernate
	public CalendarEntry toCalendarEntry() {
		CalendarEntry entry = new CalendarEntry(getResourceID(), getStartTime(), getEndTime(), getStatus(),
				getWorkload(), getAgent(), getComment());
