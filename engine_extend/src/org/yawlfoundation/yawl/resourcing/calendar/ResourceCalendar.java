...
/**
 * Maintains the resource calendar. An entry in the calendar denotes that a
 * resource is NOT available between the entry's start and end times. An entry
 * may refer to an individual resource (by id), or all human resources (eg. a
 * public holiday), or all non-human resources (eg. an equipment maintenance
 * run) or all resources (both human and non-human).
 *
...
public class ResourceCalendar {
	...
	public static enum Status {
		nil, unknown, available, unavailable, requested, reserved, busy, hardBlocked, softBlocked
	}

	public static enum ResourceGroup {
		AllResources, HumanResources, NonHumanResources
	}
	 ...
	 * 
	 * @param resource
	 *            the resource to add the entry for
	 * @param startTime
	 *            the date/time when the resource's unavailability starts
	 * @param endTime
	 *            the date/time when the resource's unavailability ends
	 * @param status
	 *            the resource's schedule status
	 * @param workload
	 *            the percentage workload (1-100)
	 * @param agent
	 *            the id of the user or service that is adding the entry
	 * @param comment
	 *            an optional comment string
	 * @return the identifier of the added entry
	 * @throws CalendarException
	 *             if invalid parameters are passed, or the addition otherwise
	 *             fails
	 */
	public long addEntry(AbstractResource resource, long startTime, long endTime, Status status, int workload,
			String agent, String comment) throws CalendarException {
		if (resource != null) {
	 ...
	 * 
	 * @param entry
	 *            an Entry value (for multiple resources)
	 * @param startTime
	 *            the date/time when the resource's unavailability starts
	 * @param endTime
	 *            the date/time when the resource's unavailability ends
	 ...
	 *             fails
	 */
	public long addEntry(ResourceGroup entry, long startTime, long endTime, Status status, String agent, String comment)
			throws CalendarException {
	 ...
	 * 
	 * @param resource
	 *            the resource to get the entries for
	...
	/**
	 * Removes all entries from the Calendar table prior to the timestamp
	 * passed.
	 * 
	 * @param priorTo
	 *            timestamp that marks when to delete entries up to. Note:
	 *            priorTo must be a timestamp prior to the current time.
	 * @return the number of entries deleted
	 */
	public int clean(long priorTo) {
		if (priorTo > System.currentTimeMillis())
			return -1;
		return _persister.execUpdate("DELETE FROM CalendarEntry AS ce WHERE ce.endTime<" + priorTo, getCommitFlag());
	}

	/**
	 * Checks if the resource is available within the specified period. Will
	 * return true if there is not an entry in the calendar table for the
	 * resource that covers the period within its duration
	 * 
	 * @param idsToIgnore
	 *            the primary key(s) of entries to exclude from the check. An
	 *            empty Set (or null) means exclude none, otherwise the
	 *            corresponding entry is excluded (used when updating an entry)
	 * @param resource
	 *            the resource to check availability for
	 * @param from
	 *            the start of a time range to search for
	 * @param to
	 *            the end of a time range to search for
	 * @return true if available, false if not
	 */
	public boolean isAvailable(Set<Long> idsToIgnore, AbstractResource resource, long from, long to) {
		if (resource == null)
			return false;
		...
		if ((idsToIgnore != null) && (!list.isEmpty())) {
			for (Object o : list) {
				...
				if (idsToIgnore.contains(entry.getEntryID()))
					++excludeCount;
			}
		}
	...
	/**
	 * Checks if the resource is currently available. Will return true if there
	 * is not an entry in the calendar table for the resource that covers the
	 * current time within its duration
	 * 
	 * @param resource
	 *            the resource to check availability for
	 * @return true if available, false if not
	...
	/**
	 * Checks if the resource is available within the specified period. Will
	 * return true if the workload total of all the entries in the calendar
	 * table for the resource that covers the period within its duration, plus
	 * the workload passed, doesn't exceed 100%
	 * 
	 * @param idsToIgnore
	 *            the primary key(s) of entries to exclude from the check. An
	 *            empty Set (or null) means exclude none, otherwise the
	 ...
	 *            the end of a time range to search for
	 * @param workload
	 *            the percentage workload to check for (0-100)
	 * @return true if available, false if not
	 */
	public boolean isAvailable(Set<Long> idsToIgnore, AbstractResource resource, long from, long to, int workload) {
		if (resource == null)
			return false;
		if (workload == 100)
			return isAvailable(idsToIgnore, resource, from, to);
		List list = getTimeSlotEntries(resource, from, to);
		if (list != null) {
			for (Object o : list) {
				CalendarEntry entry = (CalendarEntry) o;
	 ...
	 *            the resource to check availability for
	 * @param entry
	 *            a CalendarEntry object containing the required parameters
	 * @return true if available, false if not
	 */
	public boolean isAvailable(AbstractResource resource, CalendarEntry entry) {
		return !((resource == null) || (entry == null))
				&& isAvailable(resource, entry.getStartTime(), entry.getEndTime(), entry.getWorkload());
	}

	/**
	 * Gets the list of calendar entries within a specified period for a
	 * resource. Covers 4 overlapping possibilities : 1. A record's time range
	 * is wholly within start <-> end 2. start <-> end is wholly within a
	 * record's time range 3. start is prior to the record's time range, and end
	 * falls within it 4. end is after the record's time range, and start falls
	 * within it
	 *
	 * @param resource
	 *            the resource to get the entries for
	 * @param from
	 *            the start of a time range to search for (-ve value for all
	 *            start times)
	 * @param to
	 *            the end of a time range to search for (-ve value for all end
	 *            times)
	...
	public List getTimeSlotEntries(String resourceID, boolean isParticipant, long from, long to) {
		if (to <= 0)
			to = Long.MAX_VALUE;
		return _persister
				.createQuery("FROM CalendarEntry AS ce " + "WHERE ce.resourceID IN (:idlist) "
						+ "AND ce.startTime < :end AND ce.endTime > :start " + "ORDER BY ce.startTime")
				.setParameterList("idlist", createIDListForQuery(resourceID, isParticipant)).setLong("start", from)
				.setLong("end", to).list();
	}
	...
	public List getEntries(long from, long to) {
		if (to <= 0)
			to = Long.MAX_VALUE;
		return _persister.createQuery("FROM CalendarEntry AS ce " + "WHERE ce.startTime < :end AND ce.endTime > :start "
				+ "ORDER BY ce.startTime").setLong("start", from).setLong("end", to).list();
	}

	public List<TimeSlot> getAvailability(AbstractResource resource, long startTime, long endTime) {
			...
			if (!entries.isEmpty()) {
				...
				for (Object o : entries) {
					...
					if (entry.getStartTime() > endOfPrevSlot) {
						available.add(new TimeSlot(endOfPrevSlot, entry.getStartTime(), Status.available.name()));
					}
					if (entry.getWorkload() < 100) {
						available.add(new TimeSlot(entry)); // partial
															// availability
					}
				...
				available.add(new TimeSlot(startTime, endTime, Status.available.name()));
		}
		return available;
	}

	public long createEntry(AbstractResource resource, long startTime, long endTime, String status, String comment,
			String agent) throws CalendarException, ScheduleStateException {
		return createEntry(resource, startTime, endTime, strToStatus(status), comment, agent);
	}

	public long createEntry(AbstractResource resource, long startTime, long endTime, Status status, String comment,
			String agent) throws CalendarException, ScheduleStateException {
		long entryID = -1;
		switch (status) {
			case available :
				makeAvailable(resource, startTime, endTime);
				break;
			case unavailable :
				entryID = makeUnavailable(resource, startTime, endTime, agent, comment);
				break;
			case requested :
				entryID = request(resource, startTime, endTime, agent, comment);
				break;
			case reserved :
				entryID = reserve(resource, startTime, endTime, agent, comment);
				break;
			default :
	...
	public long createEntry(AbstractResource resource, CalendarEntry entry)
		...
		switch (strToStatus(entry.getStatus())) {
			case available :
				makeAvailable(resource, entry.getStartTime(), entry.getEndTime());
				break;
			case unavailable :
				entryID = makeUnavailable(resource, entry.getStartTime(), entry.getEndTime(), entry.getAgent(),
						entry.getComment());
				break;
			case requested :
			case reserved :
			case busy :
				entryID = addEntryIfAvailable(resource, entry);
				break;
			default :
				throw new CalendarException("Invalid status change request: " + entry.getStatus());
		}
		return entryID;
	}

	public boolean canCreateEntry(AbstractResource resource, long startTime, long endTime, String status, int workload)
			throws CalendarException {
		return canCreateEntry(resource, startTime, endTime, strToStatus(status), workload);
	}

	public boolean canCreateEntry(AbstractResource resource, long startTime, long endTime, Status status,
			int workload) {
		switch (status) {
			case available :
			case unavailable :
				return true; // can always do
			case requested :
			case reserved :
				return isAvailable(resource, startTime, endTime, workload);
			default :
				return false;
		}
	}

	public Status updateEntry(long entryID, String status) throws CalendarException, ScheduleStateException {
		return updateEntry(entryID, strToStatus(status));
	}

	public Status updateEntry(long entryID, Status status) throws CalendarException, ScheduleStateException {
		switch (status) {
			case available :
				makeAvailable(entryID);
				break;
			case unavailable :
				makeUnavailable(entryID);
				break;
			case reserved :
				confirm(entryID);
				break;
	...
	public boolean canUpdateEntry(long entryID, String status) throws CalendarException, ScheduleStateException {
		return canUpdateEntry(entryID, strToStatus(status));
	}

	public boolean canUpdateEntry(long entryID, Status status) throws CalendarException, ScheduleStateException {
		switch (status) {
			case available :
			case unavailable :
				return true; // can always do
			case reserved :
				return canConfirm(entryID);
			case requested :
				return canUnconfirm(entryID);
			default :
				return false;
		}
	}

	public CalendarEntry reconcileEntry(AbstractResource resource, long entryID, long startTime, long endTime)
			throws CalendarException {
	...
	public void makeAvailable(long entryID) throws CalendarException {
		...
		if (entry != null) {
			...
			throw new CalendarException(UNKNOWN_ID_ERR + entryID);
	}

	public long makeUnavailable(AbstractResource resource, long startTime, long endTime, String agent, String comment)
			throws ScheduleStateException, CalendarException {

		// all trans to unavailable valid
		reassignOrRemoveEntries(resource, startTime, endTime); // remove all
																// current
	...
	private void makeUnavailable(CalendarEntry entry) throws ScheduleStateException, CalendarException {
		if (entry != null) {
	...
	private void makeUnavailable(long entryID) throws ScheduleStateException, CalendarException {
		makeUnavailable(getEntry(entryID));
	}

	public long reserve(AbstractResource resource, long startTime, long endTime, String agent, String comment)
			throws ScheduleStateException, CalendarException {
	...
	public long request(AbstractResource resource, long startTime, long endTime, String agent, String comment)
			throws ScheduleStateException, CalendarException {
	...
	public long addTransientEntry(String id, long startTime, long endTime, Status status, int workload, String agent)
			throws CalendarException {
		return addEntry(id, startTime, endTime, status, workload, agent, TRANSIENT_FLAG);
	}

	public String updateTransientEntry(long entryID, String newStatus) throws CalendarException {
	...
	public Set<String> freeResourcesForCase(String caseID) throws CalendarException {
		...
		if (logList != null) {
			...
			for (Object o : logList) {
				...
				CalendarEntry calEntry = (CalendarEntry) _persister.get(CalendarEntry.class, logEntry.getCalendarKey());
	 ...
	 * 
	 * @param id
	 *            either the resource's id or an Entry value (for multiple
	 *            resources)
	 * @param startTime
	 *            the date/time when the resource's unavailability starts
	 * @param endTime
	 ...
	 *             fails
	 */
	private long addEntry(String id, long startTime, long endTime, Status status, int workload, String agent,
			String comment) throws CalendarException {
		return addEntry(new CalendarEntry(id, startTime, endTime, status, workload, agent, comment));
	}
	 ...
	 * 
	 * @param id
	 *            the resource's id or the Entry type to get the entries for
	 * @return the list of corresponding entries
	 */
	private List getEntries(String id) {
		return _persister.createQuery("FROM CalendarEntry AS ce WHERE ce.resourceID=:id").setString("id", id).list();
	}
	...
	public List getEntries(String id, long from, long to) {
		...
		return _persister
				.createQuery("FROM CalendarEntry AS ce WHERE ce.resourceID=:id"
						+ " AND ce.startTime < :end AND ce.endTime > :start " + " ORDER BY ce.startTime")
				.setString("id", id).setLong("start", from).setLong("end", to).list();
	}
	 ...
	 * 
	 * @param entryID
	 *            the id of the calendar entry to remove
	 * @return true if the removal was successful
	 */
	private boolean removeEntry(long entryID) {
		return _persister.execUpdate("DELETE FROM CalendarEntry AS ce WHERE ce.entryID=" + entryID,
				getCommitFlag()) > 0;
	}
	 ...
	 * 
	 * @param resource
	 *            the resource to remove the entries for
	 ...
	 * 
	 * @param id
	 *            the identifier of the resource to remove the entries for
	 * @return the number of entries removed
	 */
	private int removeEntries(String id) {
		return _persister.execUpdate("DELETE FROM CalendarEntry AS ce WHERE ce.resourceID='" + id + "'",
				getCommitFlag());
	}
	...
	private int removeEntries(String id, long from, long to) {
		String cmd = String.format("DELETE FROM CalendarEntry AS ce WHERE ce.resourceID='%s' "
				+ "AND ce.startTime < %d AND ce.endTime > %d", id, to, from);
		return _persister.execUpdate(cmd, getCommitFlag());
	}

	/**
	 * Creates a list of ids to match against for a query. An entry may match
	 * the resource's particular id, or 'AllResources' (applies to all) or all
	 * resources of its type (human or non-human)
	 * 
	 * @param resourceID
	 *            the resource to query for
	 * @param isParticipant
	 *            true for human resources, false for non-human resources
	...
	private List<String> createIDListForQuery(String resourceID, boolean isParticipant) {
		return Arrays.asList(resourceID, getEntryString(ResourceGroup.AllResources),
				getEntryString(isParticipant ? ResourceGroup.HumanResources : ResourceGroup.NonHumanResources));
	}
	 ...
	 * 
	 * @param entries
	 *            the list of CalendarEntry objects
	...
	private boolean canUpdateStatus(long entryID, Status requiredStatus) throws CalendarException {
		CalendarEntry entry = getEntry(entryID);
		if (entry != null) {
	...
	private void removeEntry(long entryID, Status status) throws ScheduleStateException, CalendarException {
	...
	private void removeEntries(List<CalendarEntry> entries) {
		if ((entries != null) && (!entries.isEmpty())) {
			...
			for (CalendarEntry entry : entries) {
				CalendarEntry blocked = (CalendarEntry) _persister.get(CalendarEntry.class, entry.getChainID());
				if (blocked != null)
					_persister.delete(blocked, tx);
				_persister.delete(entry, tx);
				notifyStatusChange(entry);
			}
		}
	...
	private void checkForClashes(AbstractResource resource, CalendarEntry entry) throws CalendarException {
		...
		if (list.size() > 1) {
			...
			throw new CalendarException("Reconciled reservation has caused a clash for resource '"
	...
	private CalendarEntry getEntryWithStatus(long entryID, Status status)
		...
		if (entry != null) {
			if (entry.getStatus().equals(status.name())) {
				...
				throw new ScheduleStateException("Status change failed: entry requires '" + status.name()
						+ "' status, but has a status of '" + entry.getStatus() + "'");
		} else
			throw new CalendarException(UNKNOWN_ID_ERR + entryID);
	}

	private long addEntryIfAvailable(AbstractResource resource, long startTime, long endTime, Status status,
			String agent, String comment) throws ScheduleStateException, CalendarException {
		CalendarEntry entry = new CalendarEntry(resource.getID(), startTime, endTime, status, 100, agent, comment);
	...
	private void addBlockedEntry(AbstractResource resource, CalendarEntry entry) throws CalendarException {
		long duration = resource.getBlockedDuration();
		if (duration > 0) {
			Status status = resource.getBlockType().equals("Soft") ? Status.softBlocked : Status.hardBlocked;
			CalendarEntry blockedEntry = new CalendarEntry(resource.getID(), entry.getEndTime() + 1,
					entry.getEndTime() + duration, status, 100, entry.getAgent(), entry.getComment());
	...
	private void reassignOrRemoveEntries(AbstractResource resource, long startTime, long endTime) {
		List<CalendarEntry> unreassigned = reassignEntries(resource, startTime, endTime);
		removeEntries(unreassigned);
	}

	private List<CalendarEntry> reassignEntries(AbstractResource resource, long startTime, long endTime) {
