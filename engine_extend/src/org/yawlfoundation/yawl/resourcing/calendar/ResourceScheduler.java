...
public class ResourceScheduler {
	...
	// set of update requests that have time changes - must be updated
	// atomically
	 ...
	 * 
	 * @param planXML
	 *            an XML reresentation of a utilisation plan
	 * @param agent
	 *            the user id of the user or service that generated the plan
	 * @param checkOnly
	 *            if true, checks if the reservations would succeed only, makes
	 *            no changes
	 ...
	 */
	public synchronized UtilisationPlan saveReservations(UtilisationPlan plan, String agent, boolean checkOnly) {
		if (validatePlan(plan) && plan.hasActivities()) {
			clearCaches();
			_calendar.beginTransaction();
			if (checkOnly)
				checkReservations(plan);
			else
				saveReservations(plan, agent);
		}
		return plan;
	}

	/**
	 * Gets all the utilisation plans in which a resource is involved for the
	 * specified period
	 * 
	 * @param resourceXML
	 *            an XML representation of a utilisation resource object,
	...
	/**
	 * Gets the set of utilisation plans in which a resource is involved for the
	 * specified period
	 * 
	 * @param uResource
	 *            a utilisation resource object, containing references to the
		...
		if (uResource != null) {
			...
			for (AbstractResource resource : getActualResourceList(uResource)) {
				for (Object o : _calendar.getTimeSlotEntries(resource, from, to)) {
					...
					CalendarLogEntry logEntry = _uLogger.getLogEntryForCalendarKey(calEntry.getEntryID());
					String caseID = null;
					...
					if (plan != null) {
						...
						if (activity != null) {
							activity.addReservation(reconstructor.reconstructReservation(calEntry));
						} else {
							plan.addActivity(reconstructor.reconstructActivity(calEntry, activityName, phase));
						}
					} else {
						// make a new plan
	...
	/**
	 * Gets the available time slots for a datetime range for each resource
	 * referenced within the given resource XML record.
	 * 
	 * @param resourceXML
	 *            an XML String equivalent of a UtilisationResource record
	 * @param from
	 *            the start of the date/time range
	 * @param to
	 *            the end of the date/time range
	...
	public List<TimeSlot> getAvailability(String resourceXML, long from, long to) {
		...
		return (resourceNode != null) ? getAvailability(new UtilisationResource(resourceNode), from, to) : null;
	}
	 ...
	 * 
	 * @param uResource
	 *            a UtilisationResource record referencing resource(s)
	 * @param from
	 *            the start of the date/time range
	 * @param to
	 *            the end of the date/time range
	...
	/**
	 * Seeks another AbstractResource that meets the original
	 * UtilisationResource parameters and is available, and if found reassigns
	 * the calendar entry to the found resource.
	 * 
	 * @param entry
	 *            the CalendarEntry containing a reference to a resource that is
	 *            no longer available, but may be replaced by another resource
	 *            of the same role, capability or category
	 * @param updating
	 *            true if the caller is updating an existing calendar entry,
	 *            false if it is a new entry
	 ...
	 * 
	 * @param caseID
	 *            the id of case to get the entry ids for
	...
	/**
	 * Builds an appropriate data set and announces a status change for a
	 * calendar record.
	 * 
	 * @param calEntry
	 *            the calendar record that has had a status change
	 */
	...
	/**
	 * Checks whether the reservations for the resources contained in the plan
	 * would succeed against the current state of the calendar
	 * 
	 * @param plan
	 *            the plan to check
	 */
	private void checkReservations(UtilisationPlan plan) {
		...
		commitOrRollback(false); // rollback any changes to db when checking
									// only
	}

	/**
	 * Attempts to save the reservations for the resources contained in the plan
	 * to the calendar
	 * 
	 * @param plan
	 *            the plan to save
	 * @param agent
	 *            the service instigating the save
	 */
	private void saveReservations(UtilisationPlan plan, String agent) {
		for (Activity activity : plan.getActivityList()) {
			if (validateActivity(activity)) {
				...
				// remove any previous calendar entries made for this activity
				// that are
		...
		// if plan contains any updates that changed start time, end time or
		// workload,
		// check that no clashes were caused by the update(s) across all
		// activites
		if (!_timeUpdates.isEmpty())
	...
	/**
	 * Checks if a reservation for a period would succeed. If it would not, the
	 * check will raise an exception that results in an error message being
	...
	/**
	 * Attempts to save a reservation to the calendar. If the save doesn't
	 * succeed, an exception will be raised that results in an error message
	 * being added to the reservation.
	 * 
	 * @param reservation
	 ...
	 *            the end of the period
	 * @param logEntry
	 *            a partially populated log entry
	 */
	private void saveReservation(Reservation reservation, long from, long to, CalendarLogEntry logEntry) {
		if (reservation == null)
			return;
		try {
			if (reservation.isUpdate()) { // pre-existing
				...
				if (reservation.hasResource()) {
					...
					CalendarEntry calEntry = createCalendarEntry(reservation, resource, from, to, logEntry.getAgent());
	 ...
	 *             if a resource could not be resolved from the ids in the
	 *             reservation, or if no resource is available for the period,
	 *             or if the status-to-be in the reservation is invalid
	 */
	private AbstractResource getActualResourceIfAvailable(Reservation reservation, long from, long to)
		...
		try {
			// send a copy of the list because it is altered by the method
			// called
			return getRandomAvailableResource(new ArrayList<AbstractResource>(actualList), from, to,
					reservation.getStatusToBe(), reservation.getWorkload());
		} catch (CalendarException ce) {
			AbstractResource resource = shuffleEntryIfPossible(reservation.getResource(), actualList, from, to);
			if (resource != null) {
				return resource;
			} else
				throw new CalendarException(ce.getMessage());
		}
	}
	 ...
	 * 
	 * @param resourceList
	 *            the list of resources to choose from
	 * @param from
	 *            the start of the period
	 * @param to
	 *            the end of the period
	 * @param status
	 *            the desired booking status
	 * @param workload
	 *            the percentage workload required from the resource
	 * @return an available resource from the list, if any
	 * @throws CalendarException
	 *             if the list is null or empty, or none of the resources in the
	 *             list is available
	 */
	private AbstractResource getRandomAvailableResource(List<AbstractResource> resourceList, long from, long to,
			String status, int workload) throws CalendarException {
		if ((resourceList == null) || resourceList.isEmpty()) {
			throw new CalendarException("Failed to resolve resource."); // none
																		// found
		}
		int resourceCount = resourceList.size();

		// while the list has resources, remove a random selection and check if
		// a
		// reservation for it would succeed
		while (resourceList.size() > 0) {
			AbstractResource actual = resourceList.remove((int) Math.floor(Math.random() * resourceList.size()));
			if (_calendar.canCreateEntry(actual, from, to, status, workload)) {
				return actual; // found a candidate
			}
		}
		throw new CalendarException(
				((resourceCount == 1) ? "Specified resource not" : "No specified resource") + " available for period.");
	}
	 ...
	 *            the list of resources to choose from
	 * @param calEntry
	 *            the potential calendar entry
	 * @return an available resource from the list, if any
	 * @throws CalendarException
	 *             if the list is null or empty, or none of the resources in the
	 *             list is available
	 */
	private AbstractResource getRandomAvailableResource(List<AbstractResource> resourceList, CalendarEntry calEntry)
			throws CalendarException {
		return getRandomAvailableResource(resourceList, calEntry.getStartTime(), calEntry.getEndTime(),
				calEntry.getStatus(), calEntry.getWorkload());
	}

	/**
	 * Builds a list of resources from the ids specified in a reservation's
	 * resource data.
	 * 
	 * @param resource
	 *            the resource data set of a reservation. May contain a single
	 *            id, OR a Role and (optional) Capability pair OR a Capability
	 *            (and no Role) OR a Category and (optional) Subcategory of a
	 *            non-human resource.
	...
	private List<AbstractResource> getActualResourceList(UtilisationResource resource) {
		...
		// if there is a role (and optionally a capability), get all matching
		// resources
		else if (resource.getRole() != null) {
			resourceList
					.addAll(dataSet.getRoleParticipantsWithCapability(resource.getRole(), resource.getCapability()));
		}

		// if there's a capability but no role, get all those with that
		// capability
		else if (resource.getCapability() != null) {
			resourceList.addAll(dataSet.getCapabilityParticipants(resource.getCapability()));
		}

		// if there is a category (and optionally a subcategory), get all
		// matches
		else if (resource.getCategory() != null) {
			resourceList.addAll(dataSet.getNonHumanResources(resource.getCategory(), resource.getSubcategory()));
		}
		return resourceList;
	}
	 ...
	 * @param logEntry
	 *            the matching log entry of the update
	 * @param from
	 *            the reservation start time
	 * @param to
	 *            the reservation end time
	 * @throws CalendarException
	 *             if there's a problem locating the record or saving the update
	 * @throws ScheduleStateException
	 *             if the status transition is invalid
	 */
	private void updateReservation(Reservation reservation, long from, long to, CalendarLogEntry logEntry)
		...
		CalendarEntry calEntry = getCalendarEntry(entryID); // exc. if invalid
															// id

		// ignore this update if there's no change to the reservation
		if (unchangedReservation(calEntry, statusToBe, from, to, workload))
			return;
		...
		if (statusToBe.equals(ResourceCalendar.Status.unavailable.name())) {
			reassignEntryIfPossible(calEntry, false); // adds new rec if
														// possible
		}
		...
		// status updates done separately since several 2ndary changes may be
		// triggered
		...
		if (logEntry.getPhase().equals("SOU")) { // start-of-utilisation
			setResourceBusy(logEntry.getCaseID(), reservation.getResource().getID(), true);
		}
	}
	 ...
	 * 
	 * @param reservation
	 *            the reservation to store
	 * @param resource
	 *            the resource to make the reservation for
	 * @param calEntry
	 *            the calendar entry to save, representing the resource
	 * @param logEntry
	 *            the entry to save to the calendar log
	 * @throws CalendarException
	 *             if there's a problem saving the reservation
	 * @throws ScheduleStateException
	 *             if the status transition is invlaid
	 */
	private void makeReservation(Reservation reservation, AbstractResource resource, CalendarEntry calEntry,
			CalendarLogEntry logEntry) throws CalendarException, ScheduleStateException {
		String statusToBe = reservation.getStatusToBe();
		checkRequestedStatusToBe(statusToBe);
	...
	/**
	 * Checks whether a new reservation can by made for the specified resource
	 * and period (i.e. there are no conflicts with existing reservations).
	 * 
	 * @param reservation
	 *            the reservation to store
	 * @param from
	 ...
	 *            the end of the period
	 * @throws CalendarException
	 *             if the reservation's 'statusToBe' is invalid
	 */
	private void checkMakeReservation(Reservation reservation, long from, long to) throws CalendarException {
	 ...
	 *             if the reservation's 'statusToBe' is invalid
	 * @throws ScheduleStateException
	 *             if the update involves an invalid status transition
	 */
	...
	/**
	 * Reconciles and updates an EOU (end-of-utilisation) plan with the stored
	 * SOU (start-of-utilisation) plan.
	 * 
	 * @param reservation
	 *            the reservation to reconcile
	 * @param from
	 *            the start of the period
	 * @param to
	 *            actual end of the period
	 * @throws CalendarException
	 *             if the reservation's 'statusToBe' is invalid
	 * @param logEntry
	 *            the log entry associated with the reservation
	 * @throws CalendarException
	 *             if the reconciliation causes a clash in the calendar
	 */
	private void reconcileReservation(Reservation reservation, long from, long to, CalendarLogEntry logEntry)
			throws CalendarException {
		AbstractResource resource = getActualResource(reservation.getResource().getID());
		CalendarEntry calEntry = _calendar.reconcileEntry(resource, convertReservationID(reservation), from, to);
		_uLogger.log(logEntry, calEntry, false);
		...
		if (calEntry.getChainID() > 0) {
			...
			if ((blockedEntry != null) && (blockedEntry.getEndTime() < System.currentTimeMillis())) {
				setResourceBusy(logEntry.getCaseID(), reservation.getResource().getID(), false);
			}
		}
	}

	/**
	 * Removes any reservations made in an earlier version of the plan that are
	 * no longer included in the current plan
	 * 
	 * @param caseID
	 *            the case id of the plan
	 * @param activityName
	 *            the name of the activity
	 * @param entryIDs
	 *            the ids of all the reservations for the activity
	 */
	private void handleCancellations(String caseID, String activityName, Set<Long> entryIDs) {
		...
		// compare stored reservations to current ones, and extract the
		// differences
		...
		for (long cancelledID : toCancel) {
			try {
				...
				// safe to ignore - thrown by missing id in calendar, so no more
				// to do
			}
		}
	...
	private void checkTimeUpdatesForClashes() {
		...
		for (Reservation reservation : _timeUpdates) {
			...
			ignoreSet.add(id);
			if (!_calendar.isAvailable(ignoreSet, resource, entry.getStartTime(), entry.getEndTime(),
					entry.getWorkload())) {
	...
	/**
	 * Converts the reservation id stored in a reservation record to its
	 * original 'long' type
	 * 
	 * @param reservation
	 ...
	 * 
	 * @param entryID
	 *            the id of the reservation that has been cancelled
	 * @param caseID
	 *            the reservation's case id
	 * @param activityName
	 *            the name of the activity that contained the reservation
	 */
	private void logCancellation(long entryID, String caseID, String activityName) {
		CalendarLogEntry logEntry = new CalendarLogEntry(caseID, activityName, null, 0, "cancelled", entryID);
	 ...
	 * @param resource
	 *            the resource associated with the reservation
	 * @param from
	 *            the start of the period
	 * @param to
	 *            the end of the period
	 * @param agent
	 *            the userid of the user or service creating the entry
	 * @return the newly constructed calendar entry
	 * @throws CalendarException
	 *             if the reservation's 'statusToBe' is invalid
	 */
	private CalendarEntry createCalendarEntry(Reservation reservation, AbstractResource resource, long from, long to,
			String agent) throws CalendarException {
		ResourceCalendar.Status status = _calendar.strToStatus(reservation.getStatusToBe());
		return new CalendarEntry(resource.getID(), from, to, status, reservation.getWorkload(), agent, null);
	}

	/**
	 * Adds a calendar entry temporarily, to allow reservation checking to
	 * proceed
	 * 
	 * @param resource
	 *            the resource to add
	 ...
	 *            the end of the period
	 * @param reservation
	 *            the reservation containing the resource to add
	 * @return the entry id of the newly constructed calendar entry
	 * @throws CalendarException
	 *             if the reservation's 'statusToBe' is invalid
	 */
	private long addTransientCalendarEntry(AbstractResource resource, long from, long to, Reservation reservation)
			throws CalendarException {
		ResourceCalendar.Status status = _calendar.strToStatus(reservation.getStatusToBe());
		return _calendar.addTransientEntry(resource.getID(), from, to, status, reservation.getWorkload(), null);
	}

	/**
	 * Updates a calendar entry temporarily, to allow reservation checking to
	 * proceed
	 * 
	 * @param entryID
	 *            the entry to update
	...
	/**
	 * Checks that the status-to-be of an update reservation request is not the
	 * same as its current status.
	 * 
	 * @param current
	 *            the current status
	 * @param toBe
	 *            the requested status-to-be
	 * @throws ScheduleStateException
	 *             if the current and requested statuses are identical
	 */
	private void compareStatuses(String current, String toBe) throws ScheduleStateException {
		if (current.equals(toBe)) {
			throw new ScheduleStateException("Reservation already has status of '" + toBe + "'.");
		}
	}
	 ...
	 *            the id of the resource
	 * @param busy
	 *            true if 'busy', false if 'released'
	 */
	...
	/**
	 * Do a final commit or rollback of all the additions and updates made to
	 * the calendar and log for a complete actioning of a plan
	 * 
	 * @param commit
	 *            if true, all changes are committed; if false, all changes are
	 *            rolled back
	 */
	 ...
	 * 
	 * @param entry
	 *            the calendar entry to reassign
	 * @param logEntry
	 *            the matching log entry for the calendar entry
	 * @param uResource
	 *            the resource xml specifying the group object
	 * @param updating
	 *            true if this is an plan update, false if it is a new plan
	 * @return true if the entry was able to be reassigned to a different
	 *         resource
	 */
	private boolean reassignEntry(CalendarEntry entry, CalendarLogEntry logEntry, UtilisationResource uResource,
			boolean updating) {
		...
		try {
			AbstractResource resource = getRandomAvailableResource(resourceList, entry);
			if (resource != null) {
				if (updating) {
	...
	/**
	 * Find a resource that is currently booked for a period as the member of a
	 * grouping (role, capability, category/subcategory), that can be replaced
	 * by another member of that group, so that the resource can be used to
	 * satisfy another booking for which no free resource could be found. For
	 * example, if role X contains resources R1 and R2, and role Y also contains
	 * R1, and R1 has been chosen to satisfy an entry E1 for X, and a request
	 * for an entry E2 of Y is received, then an attempt is made in this method
	 * to swap R1 with R2 in E1, so that R1 can be used to satisfy E2.
	 * 
	 * @param origResource
	 *            the resource xml of the original entry
	 * @param origResourceList
	 *            the list of resources that could potential satisfy the entry
	 * @param from
	 *            the start of the period
	 * @param to
	 *            the end of the period
	...
	private AbstractResource shuffleEntryIfPossible(UtilisationResource origResource,
			List<AbstractResource> origResourceList, long from, long to) {
			...
			if (entries != null) {
				...
				for (Object o : entries) {
					...
					if (isPhase(entry.getEntryID(), Activity.Phase.POU)) {
						...
						// if this entry's resource satsifies the original
						// resource xml
						// and it can be swapped with another, then swap it
						// return it
						AbstractResource shuffled = shuffleEntry(entry, origResourceList, origResource);
						if (shuffled != null) {
							return shuffled;
						}
					}
	 ...
	 * @return the shuffled out resource, if the suffle is possible
	 */
	private AbstractResource shuffleEntry(CalendarEntry entry, List<AbstractResource> resourceList,
		...
		if ((resource != null) && resourceList.contains(resource)) {
			...
			// ...and its resource xml is different to the resource xml passed,
			// and
	...
	private UtilisationResource reconstituteResourceRecord(String resourceRec) {
		if (resourceRec != null) {
			...
			if (node != null)
				return new UtilisationResource(node);
		}
		return null;
	}
	 ...
	 * 
	 * @param calendarKey
	 *            the id of the log entry to check
	 * @param phase
	...
	/**
	 * Consolidates a status into a simpler one in these cases: - if checkonly
	 * is true, status is one of unknown/available/unavailable - else, status is
	 * one of unknown/available/requested/reserved/unavailable
	 * 
	 * @param statusStr
	 *            the status to consolidate
	 * @param checkOnly
	 *            true if the call is a check, false if it is a save
	 * @return the consolidated status
	 * @throws CalendarException
	 *             if statusStr is not a valid status
	 */
	private String getAvailabilityStatus(String statusStr, boolean checkOnly) throws CalendarException {
	...
	/**
	 * Checks that a supplied updated status for a save reservation request is
	 * one of the four valid values: available, unavailable, requested, reserved
	 * 
	 * @param statusStr
	 *            the status string to check
	 * @throws CalendarException
	 *             if the status string is invalid
	 */
	private void checkRequestedStatusToBe(String statusStr) throws CalendarException {
		switch (_calendar.strToStatus(statusStr)) {
			case available :
			case unavailable :
			case requested :
		...
		throw new CalendarException("Invalid StatusToBe value for save reservation request:" + statusStr);
	}

	private void checkIfPreviouslySaved(CalendarLogEntry logEntry, long from, long to) throws CalendarException {
		for (Object o : _uLogger.getLogEntriesForReservation(logEntry.getCaseID(), logEntry.getActivityName(),
				logEntry.getResourceRec())) {
	...
	private boolean unchangedReservation(CalendarEntry calEntry, String statusToBe, long from, long to, int workload) {
		return (calEntry != null) && calEntry.hasPeriod(from, to) && calEntry.getStatus().equals(statusToBe)
