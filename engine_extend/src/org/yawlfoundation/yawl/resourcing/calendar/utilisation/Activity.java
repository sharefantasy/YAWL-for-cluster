...
public class Activity extends StatusMessage {
	...
	public static enum Phase {
		POU, SOU, EOU
	} // pre, start of, end of, utilisation

	public Activity() {
	}

	public Activity(String name, String startTaskID, String endTaskID, String from, String to, String duration) {
		setName(name);
	...
	public boolean isSOU() {
		return _phase.equals(Phase.SOU.name());
	}

	public boolean isEOU() {
		return _phase.equals(Phase.EOU.name());
	}
	...
	public void setFrom(String from) {
		if (_from == null)
			_from = new StringWithMessage("From");
		_from.setValue(from);
	}

	public long getFromAsLong() {
		return getTime(_from);
	}
	...
	public void setDuration(String duration) {
		if (_duration == null)
			_duration = new StringWithMessage("Duration");
	...
	public boolean addReservation(Reservation r) {
		if (_reservationList == null)
			_reservationList = new ArrayList<Reservation>();
	...
	public boolean hasErrors() {
		return hasError() || StringWithMessage.hasError(_from) || StringWithMessage.hasError(_to)
				|| StringWithMessage.hasError(_duration) || reservationsHaveErrors();
	}
	...
	public XNode toXNode() {
		...
		if (_duration != null)
			node.addChild(_duration.toXNode());
		if (_from != null)
			node.addChild(_from.toXNode());
		if (_to != null)
			node.addChild(_to.toXNode());
		...
		if (_relationList != null)
			node.addChildren(_relationList);
		if (_transferList != null)
			node.addChildren(_transferList);
		return node;
	}
	...
	private long getTime(StringWithMessage smTime) {
		...
		if (time < 0)
			smTime.setError("Invalid dateTime value: " + smTime.getValue());
		return time;
	}
