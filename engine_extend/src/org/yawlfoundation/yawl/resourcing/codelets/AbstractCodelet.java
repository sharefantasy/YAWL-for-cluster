...
public abstract class AbstractCodelet {
	...
	/**
	 * Converts the string representation of a parameter value to its specified
	 * data type, for the most frequent data types. Implementers may override
	 * this method to handle other data types if required.
	 * 
	 * @param varName
	 *            the name of the parameter
	 * @return the appropriately typed value
	 * @throws CodeletExecutionException
	 *             if the string value can't be converted to the specified type.
	 */
	protected Object getParameterValue(String varName) throws CodeletExecutionException {
		...
		try {
			...
			throw new CodeletExecutionException("Invalid value '" + value + "' for " + "datatype '" + dataType
					+ "' of parameter '" + varName + "'.");
		}
		return result;
	}

	/**
	 * Adds a new value to an output parameter, and creates the parameter if it
	...
	protected void setParameterValue(String varName, String value) {
		if (_outData == null)
			_outData = new Element("codelet_output");
	...
	protected Element getOutputData() {
		return _outData;
	}

	protected YParameter getInputParameter(String paramName) throws CodeletExecutionException {
		return getParameter(paramName, _inParams);
	}

	protected YParameter getOutputParameter(String paramName) throws CodeletExecutionException {
	 ...
	 * @param params
	 *            the list of parameters containing the parameter
	 * @return the specified parameters
	 * @throws CodeletExecutionException
	 *             if the requested parameter isn't in the list
	 */
	private YParameter getParameter(String paramName, List<YParameter> params) throws CodeletExecutionException {
		...
		if (result != null)
			...
			throw new CodeletExecutionException(
					"Parameter '" + paramName + "' is required by the specified codelet but could not be found in "
	 ...
	 * @return its value
	 * @throws CodeletExecutionException
	 *             if the parameter doesn't exist in the input data
	 */
	protected String getValue(String varName) throws CodeletExecutionException {
		...
		if (result != null)
			...
			throw new CodeletExecutionException("A value for parameter '" + varName
					+ "' is required by the specified codelet but could not be found in " + "the workitem's data.");
	}
	...
	public String toXML() {
		...
		xml.append(StringUtil.wrap(getClassName(), "name"))
				...
				.append(getRequiredParamsToXML()).append("</codelet>");
		return xml.toString();
	}
	...
	public String getRequiredParamsToXML() {
		...
		if (requiredParams != null) {
			for (YParameter param : requiredParams) {
				...
				xml.append("<").append(paramType).append(">").append(StringUtil.wrap(param.getPreferredName(), "name"))
						.append(StringUtil.wrap(param.getDataTypeName(), "datatype"))
						.append(StringUtil.wrap(param.getDataTypeNameSpace(), "namespace"))
						.append(StringUtil.wrap(param.getDocumentation(), "documentation")).append("</")
	...
	/**
	 * This method is called when a codelet first starts. Override to include
	 * any initialisations required.
	 */
	public void init() {
	}

	/**
	 * This method is called when the server is shutting down. Override to
	 * include any state saving required.
	 */
	public void shutdown() {
	}

	/**
	 * This method is called when the codelet is resumed after a server
	 * shutdown. Override to include any state restoring required.
	 */
	public void resume() {
	}

	/**
	 * This method is called when the work item running this codelet is
	 * cancelled. Override to cancel the codelet as required.
	 */
	public void cancel() {
	}

	/**
	 * This method is called when an external entity (such as the YAWL editor)
	 * requests the list of parameters required by the codelet. Override to
	 * populate the list. Each parameter should include a name, data type and
	 ...
	 * 
	 * @param inData
	 *            the work item's input data
	 * @param inParams
	 *            the work item's input parameters
	 * @param outParams
	 *            the work item's output parameters
	 * @return a JDOM Element containing the output data generated by this
	 *         method
	 * @throws CodeletExecutionException
	 *             if there's any problem getting the required input data,
	 *             composing the required output data, or performing the
	 *             execution.
	 */
	public abstract Element execute(Element inData, List<YParameter> inParams, List<YParameter> outParams)
			throws CodeletExecutionException;
