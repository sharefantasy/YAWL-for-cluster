...
/**
 * Stores some information about a codelet class for transporting across the
 * API.
 * 
...
public class CodeletInfo {
	...
	public void fromXML(XNode node) {
		if (node != null) {
			...
			if (params != null)
				_requiredParams = getRequiredParametersFromXML(params);
		}
	}
	...
	public List<YParameter> getRequiredParametersFromXML(XNode node) {
		...
		if (node != null) {
			for (XNode pNode : node.getChildren()) {
				...
				param.setDataTypeAndName(pNode.getChildText("datatype"), pNode.getChildText("name"),
						pNode.getChildText("namespace"));
