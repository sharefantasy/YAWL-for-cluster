 ...
 * abstract ancestor, but of course those base classes do not need to be relied
 * upon - all the work my be done in this class if desired.
 *
 * The Element passed back to the caller of the execute method must contain the
 * results of the execution. It should be constructed as:
 * <root> <paramName>paramValue</paramName> <paramName>paramValue</paramName>
 * ...... </root>
 *
 * "root" can be called anything; each paramName must be the same as one of the
 * triggering workitem's output parameters.
 *
 * @author Michael Adams Creation Date: 18/06/2008
 */
public class RandomWait extends AbstractCodelet {
	...
	public RandomWait() {
		...
		setDescription("This codelet is a simple example of codelet construction and<br> "
				+ "usage. It waits for a random amount of time before completing.<br>" + "Required parameters:<br> "
				+ "Input: interval (type String) - H, M or S (for hour, minute or second)<br>"
				+ "Input: max (type long) - maximum wait time<br>" + "Output: waitTime (type long) - the time waited.");
	}

	/**
	 * The implentation of the abstact class that does the work of this codelet.
	 * Note that calls to most of the base class methods may throw a
	 * CodeletExecutionException which should be passed back to the caller,
	 *
	 * @param inData
	 *            The calling workitem's data
	 * @param inParams
	 *            the workitem's input parameters
	 * @param outParams
	 *            the workitem's output parameters
	...
	public Element execute(Element inData, List<YParameter> inParams, List<YParameter> outParams)
		...
		// get the data values required - note that getParameterValue returns
		// the value
		...
		// set the output result. setParameterValue requires the result to be a
		// String.
		setParameterValue("waitTime", String.valueOf(randomWait));

		// return the Element created in the base class and containing the
		// result.
