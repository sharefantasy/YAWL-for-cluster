...
public class TaskCompleterInfo extends AbstractCodelet {
	...
	public TaskCompleterInfo() {
		...
		setDescription("This codelet gets the name and userid of the participant <br> "
				...
				+ "Input: taskName (string type).<br>" + "Outputs: name (string type)<br>"
				+ "         userid (string type)");
	}
	...
	public Element execute(Element inData, List<YParameter> inParams, List<YParameter> outParams)
		...
		Set<Participant> completers = ResourceManager.getInstance().getWhoCompletedTask(taskName, getWorkItem());
		if (completers.isEmpty()) {
			throw new CodeletExecutionException("Unknown task completer. Either the " + "taskName '" + taskName
					+ "'  is invalid or a task of that name has not yet completed for this case.");
		}
		Participant p = completers.iterator().next(); // only one for atomic
														// tasks
