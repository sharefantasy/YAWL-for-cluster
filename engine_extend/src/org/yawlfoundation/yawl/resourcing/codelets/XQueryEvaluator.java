...
/**
 * @author Michael Adams Creation Date: 30/04/2008
 */
public class XQueryEvaluator extends AbstractCodelet {
	...
	public XQueryEvaluator() {
		...
		setDescription("This codelet executes an XQuery. Required parameters:<br> "
				...
				+ " &nbsp;&nbsp; plus other values used by the XQuery<br>" + "Output: result (type String)");
	}
	...
	private String evaluateQuery(String query, Document dataDoc) throws CodeletExecutionException {
		...
		try {
			...
			if (result != null)
				...
				throw new CodeletExecutionException("No data produced for query: '" + query + "'.");
		} catch (SaxonApiException sapie) {
			throw new CodeletExecutionException(
					"Invalid query: '" + query + "'.\n" + "Message from parser: [" + sapie.getMessage() + "]");
		} catch (Exception e) {
			throw new CodeletExecutionException("Exception in query evaluation: '" + query + "'.");
		}
	}
