...
/**
 * An API to retrieve data from the resource service's event logs and pass it
 * back as XML.
 *
...
public class LogMiner {
	...
	/**
	 * @param specID
	 *            the specification id to get the case eventids for
	 * @return the set of all case ids for the specID passed
	 */
	public String getWorkItemDurationsForParticipant(YSpecificationID specID, String taskName, String participantID) {
		String result;
		List rows;
		if (_reader != null) {
			long specKey = getSpecificationKey(specID);
			StringBuilder template = new StringBuilder(_baseQuery);
			template.append(" WHERE re._taskID='%s'").append(" AND re._specKey=%d").append(" AND re._resourceID='%s'")
	...
	public String getCaseHistoryInvolvingParticipant(String pid, long from, long to) {
		return ((from < 0) && (to < 0))
				? getCaseHistoryInvolvingParticipant(pid)
	...
	public List getBusyResources(String itemID) {
		String query = String.format("%s WHERE re._itemID = '%s' AND re._event='busy' " + "AND re._resourceID NOT IN ("
				+ "SELECT rx._resourceID FROM ResourceEvent AS rx " + "WHERE rx._itemID = '%s' "
	...
	private List getSpecificationEvents(long specKey, long from, long to) {
		return (specKey == -1) ? null : execQuery(getWhereQuery("_specKey", specKey) + getTimeRangeSubclause(from, to));
	}

	private List getCaseEventsList(String caseID) {
		String query = String.format("%s WHERE re._caseID='%s' OR re._caseID LIKE '%s%s'", _baseQuery, caseID, caseID,
				".%");
		return execQuery(query);
	}
