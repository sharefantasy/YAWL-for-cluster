...
/**
 * Author: Michael Adams Creation Date: 30/04/2010
 */
public class ResourceXESLog extends YXESBuilder {

	public ResourceXESLog() {
		super();
	}
	...
	protected String getComment() {
		...
		return "Generated by the YAWL Resource Service " + df.format(new Date(System.currentTimeMillis()));
	}
	...
	private XNode eventNode(XNode yEvent) {
		...
		eventNode.addChild(stringNode("lifecycle:transition", translateEvent(yEvent.getChildText("descriptor"))));
	...
	private Map<String, XNode> buildCaseMap(XNode logNode) {
		...
		for (XNode trace : logNode.getChildren()) { // rs log has 'trace'
													// children only
			String caseID = trace.getChild("string").getAttributeValue("value");
	...
	// These transitions appear in both logs, but each has relevant data, so
	// they
	...
	private boolean masterHasPrecedence(String transition) {
		return mergeableEvent(transition) || transition.equals("ate_abort") || transition.equals("pi_abort");
	}
	...
	private void removeEvent(XNode node, String taskName, String instanceID, String transition) {
		...
		for (XNode event : node.getChildren("event")) {
			if (getTransition(event).equals(transition) && getInstanceID(event).equals(instanceID)
		...
		for (XNode event : node.getChildren("event")) {
			if (getTransition(event).equals(transition) && getInstanceID(event).equals(instanceID)
				...
				if (pos > -1) {
					...
					event.addChild(stringNode("org:resource", orgResource));
				break;
			}
		}
	}
