...
public class TaskStatistics {
	...
	private enum Counter {
		unofferToCancel, unofferToOffer, offerToAllocate, offerToStart, unofferToAllocate, unofferToStart, allocateToStart, startToComplete, startToReallocate, offerToCancel, allocateToCancel, startToCancel, totalDuration, unofferToTimeout, offerToTimeout, allocateToTimeout, startToTimeout
	}
	...
	private Map<String, TaskSummary> generateSummaries() {
		...
		for (Object o : _taskEvents) {
			...
			if (event.get_event().equals("offer")) {
				...
				if (stat.getOfferCount() == 0)
					stat.setOfferTime(event.get_timeStamp());
	...
	private XNode outputXML(Map<String, TaskSummary> table) {
		...
		if (!table.isEmpty()) {
			...
			if (!offers.isEmpty())
				node.addChild(getStateNode(offers, "offers"));
			if (!allocs.isEmpty())
				node.addChild(getStateNode(allocs, "allocations"));
			if (!starts.isEmpty())
				node.addChild(getStateNode(starts, "starts"));
			if (!completes.isEmpty())
				node.addChild(getStateNode(completes, "completions"));
			if (!cancels.isEmpty()) {
				node.addChild(getStateNode(cancels, "cancels", null, cancels.getSubCount()));
			}
			if (!timeouts.isEmpty())
				node.addChild(getStateNode(timeouts, "timeouts"));
			if (!totals.isEmpty())
				node.addChild(getStateNode(totals, "totalDurations"));

			XNode overallNode = node.addChild("overall");
			overall.sort();
			for (TaskSubTally tally : overall.values()) {
				if (tally.count > 0)
					overallNode.addChild(tally.getNode());
			}
		}
		return node;
	}
	...
	private void accumulate(Map<String, TaskSubTallyList> map, TaskSummary ts, String pid, List<Counter> counters) {
		if (pid != null) {
	...
	private XNode getStateNode(Map<String, TaskSubTallyList> tallyMap, String name) {
		...
		for (String pid : tallyMap.keySet()) {
			...
			node.addChild(getStateNode(list, "participant", getParticipantName(pid), list.count));
		}
		return node;
	}
	...
	private List<Counter> getStartCounters() {
		return Arrays.asList(Counter.offerToStart, Counter.unofferToStart, Counter.allocateToStart,
				Counter.startToReallocate);
	}
	...
	private List<Counter> getCancelCounters() {
		return Arrays.asList(Counter.unofferToCancel, Counter.offerToCancel, Counter.allocateToCancel,
				Counter.startToCancel);
	}

	private List<Counter> getTimeoutCounters() {
		return Arrays.asList(Counter.unofferToTimeout, Counter.offerToTimeout, Counter.allocateToTimeout,
				Counter.startToTimeout);
	}
	...
	private String getReferenceItemID(Map<String, TaskSummary> table, String id) {
		if (table.containsKey(id)) {
			...
			if (ts.getStartTime() > 0)
				return getRootItemID(id);
		}
		return id;
	}
	...
	class TaskSummary {
		...
		public void setOfferTime(long time) {
			offerTime = time;
		}
		...
		public void setUnofferTime(long time) {
			unofferTime = time;
		}
		...
		public void setAllocTime(long time) {
			allocTime = time;
		}
		...
		public void setCancelTime(long time) {
			cancelTime = time;
		}
		...
		public void setReallocTime(long time) {
			reallocTime = time;
		}
		...
		public void setAllocateID(String aID) {
			allocateID = aID;
		}
		...
		public void setCompleteID(String cID) {
			completeID = cID;
		}
		...
		public long getOfferToAlloc() {
			return getDiff(offerTime, allocTime);
		}

		public long getAllocToStart() {
			return getDiff(allocTime, startTime);
		}
		...
		public long getStartToComplete() {
			return getDiff(startTime, completeTime);
		}

		public long getStartToCancel() {
			return getDiff(startTime, cancelTime);
		}

		public long getStartToRealloc() {
			return getDiff(startTime, reallocTime);
		}
		...
		public long getUnofferToOffer() {
			return getDiff(unofferTime, offerTime);
		}
		...
		public long getStartToTimeout() {
			return getDiff(startTime, timeout);
		}
		...
		public long getPeriod(Counter counter) {
			long period = 0;
			switch (counter) {
				case unofferToCancel :
					period = getUnofferToCancel();
					break;
				case unofferToOffer :
					period = getUnofferToOffer();
					break;
				case offerToAllocate :
					period = getOfferToAlloc();
					break;
				case offerToStart :
					period = getOfferToStart();
					break;
				case unofferToAllocate :
					period = getUnofferToAlloc();
					break;
				case unofferToStart :
					period = getUnofferToStart();
					break;
				case allocateToStart :
					period = getAllocToStart();
					break;
				case startToComplete :
					period = getStartToComplete();
					break;
				case startToReallocate :
					period = getStartToRealloc();
					break;
				case offerToCancel :
					period = getOfferToCancel();
					break;
				case allocateToCancel :
					period = getAllocToCancel();
					break;
				case startToCancel :
					period = getStartToCancel();
					break;
				case totalDuration :
					period = getTotalTime();
					break;
				case unofferToTimeout :
					period = getUnofferToTimeout();
					break;
				case offerToTimeout :
					period = getOfferToTimeout();
					break;
				case allocateToTimeout :
					period = getAllocToTimeout();
					break;
				case startToTimeout :
					period = getStartToTimeout();
					break;
			}
			return period;
		}
