...
/**
 * Author: Michael Adams Creation Date: 5/03/2010 Last Date: 15/06/2010
 */
public class LDAPSource extends DataSource {
	 ...
	 * from the LDAP server. The map uses a generic key for each attribute and
	 * maps it to the actual corresponding attribute name as read from the
	 * properties file.
	 * 
	...
	/**
	 * Gets the list of all entries from the LDAP server corresponding to the
	 * binding read from the properties file. The list is retrieved in a single
	 * call to the server.
	 * 
	 * @return a (String, String) map of (name, binding) entries
	 * @throws NamingException
	 *             if something goes wrong when reading from the server
	 */
	...
	/**
	 * Gets the list of all entries from the LDAP server corresponding to the
	 * binding read from the properties file. The list is retrieved in multiple
	 * calls to the server, each call retrieving maxSize entries (so as not to
	 * exceed the max size limit configured on the server).
	 * 
	 * @param maxSize
	 *            the max size limit configured
	 * @return a (String, String) map of (name, binding) entries
	 * @throws NamingException
	 *             if something goes wrong when reading from the server
	 * @throws IOException
	 *             if there's a problem creating the PagedResultsControl
	 */
	private Map<String, String> getControlledNameBindingMap(int maxSize) throws NamingException, IOException {

		Map<String, String> nameList = new Hashtable<String, String>();
		byte[] cookie = null;
		LdapContext ctx = new InitialLdapContext(getEnvironment(), null);
		ctx.setRequestControls(new Control[]{new PagedResultsControl(maxSize, Control.CRITICAL)});
		...
		for (String binding : getProperty("binding").split(";")) {
			do {
				...
				ctx.setRequestControls(new Control[]{new PagedResultsControl(maxSize, cookie, Control.CRITICAL)});
	...
	/**
	 * Gets the environment attribute values for server connections. The value
	 * for each attribute is read from the properties file.
	 * 
	...
	private HashMap<String, Participant> loadParticipants() throws NamingException, IOException {
		...
		Map<String, String> nameBindingMap = (maxSize > 0) ? getControlledNameBindingMap(maxSize) : getNameBindingMap();
	 ...
	 *            the LDAP entry name
	 * @param attributes
	 *            the entries attributes
	 * @return an instantiated Participant
	 * @throws NamingException
	 *             if something goes wrong when reading role info from the
	 *             server
	 */
	private Participant createParticipant(String name, Attributes attributes) throws NamingException {
		...
		if (allNotNullOrEmpty(lastname, firstname, userid)) {
			...
			// if authentication is done via LDAP, keep the LDAP name - userid
			// mapping
	 ...
	 * 
	 * @param p
	 *            the Participant to set the Roles for
	 * @param attributes
	 *            the names of the roles the Participant is to be given
	 * @throws NamingException
	 *             if something goes wrong when reading role info from the
	 *             server
	 */
	...
	/**
	 * Sets the Roles, provided as a comma separated value string, for a
	 * participant
	 * 
	 * @param p
	 *            the Participant to set the Roles for
	 ...
	 * 
	 * @param p
	 *            the Participant to add to the Role
	 * @param roleName
	 *            the name of the Role to give to the Participant
	 */
	...
	/**
	 * Loads the LDAP server-stored password for a Participant, if specified to
	 * do so via the properties file.
	 * 
	 * @param attributes
	 *            the LDAP attributes for a user entry
	 * @return an encypted password, or null if configured not to load password
	 *         into each Participant object.
	 */
	...
	private String getStringValue(Attributes attributes, String attributeName) throws NamingException {
		Attribute attr = getAttribute(attributes, attributeName);
		return (attr != null) ? (String) attr.get() : null;
	}

	private byte[] getByteValue(Attributes attributes, String attributeName) throws NamingException {
		Attribute attr = getAttribute(attributes, attributeName);
		return (attr != null) ? (byte[]) attr.get() : null;
	}

	private Attribute getAttribute(Attributes attributes, String attributeName) throws NamingException {
	...
	public ResourceDataSet loadResources() {
		...
		if (_props != null) {
			try {
				...
				// thrown by loadParticipants(); nothing to do, as an empty rds
				// will
				// be returned, initialising a controlled service disablement
				_log.error("Naming Exception thrown when attempting to retrieve org data from LDAP.", ne);
			} catch (IOException ioe) {
				// as above
				_log.error("IO Exception thrown when attempting to retrieve org data from LDAP.", ioe);
			}
		}
		return rds;
	}
	...
	public boolean authenticate(String userid, String password) throws YAuthenticationException {
		...
		try {
			new InitialDirContext(env); // will throw exception if credentials
										// wrong
			return true;
		} catch (AuthenticationException ae) {
			return false; // bad password
		} catch (NamingException ne) {
			throw new YAuthenticationException("Cannot authenticate user: LDAP Authentication exception.", ne);
		} finally {
