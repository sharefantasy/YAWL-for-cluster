...
/**
 * Handles the loading, mapping and runtime management of resources from data
 * source.
 *
...
public class ResourceDataSet {
	...
	public enum ResUnit {
		Participant, Role, Capability, OrgGroup, Position, NonHumanResource, NonHumanCategory
	}

	public enum Identifier {
		FullName, ReverseFullName, LastName, Userid
	}
	...
	// if true, overrides read-only setting of external data sources (set from
	// web.xml)
	private boolean _allowExternalOrgDataMods = true;

	// if true, delegates user authentication to external data source (set from
	// web.xml)
	...
	private synchronized void disconnectResources(AbstractResourceAttribute attrib) {
		...
		for (String id : ids) {
			...
			if (attrib instanceof Role)
				p.removeRole((Role) attrib);
			else if (attrib instanceof Capability)
				p.removeCapability((Capability) attrib);
			else if (attrib instanceof Position)
				p.removePosition((Position) attrib);
			updateParticipant(p);
		}
	}
	...
	// By default, data loaded from external sources is read-only. This value,
	// set from
	...
	public boolean isExternalOrgDataModsAllowed() {
		return _allowExternalOrgDataMods;
	}
	...
	public void setNonHumanResources(Map<String, NonHumanResource> resources, DataSource source) {
		nonHumanMap = resources;
		setDataSource(ResUnit.NonHumanResource, source);
	}

	public void setNonHumanCategories(Map<String, NonHumanCategory> resources, DataSource source) {
	...
	public String addCapability(Capability c) {
		if (isDataEditable(ResUnit.Capability)) {
			String newID = getDataSource(ResUnit.Capability).insert(c); // persist
																		// it
	...
	public String addPosition(Position p) {
		if (isDataEditable(ResUnit.Position)) {
			String newID = getDataSource(ResUnit.Position).insert(p); // persist
																		// it
	...
	public String addOrgGroup(OrgGroup o) {
		if (isDataEditable(ResUnit.OrgGroup)) {
			String newID = getDataSource(ResUnit.OrgGroup).insert(o); // persist
																		// it
	...
	public String addNonHumanResource(NonHumanResource r) {
		if (isDataEditable(ResUnit.NonHumanResource)) {
			String newID = getDataSource(ResUnit.NonHumanResource).insert(r); // persist
																				// it
	...
	public void updateResource(AbstractResource r) {
		if (r instanceof Participant)
			updateParticipant((Participant) r);
		else
	...
	public AbstractResource getResource(String id) {
		...
		if (resource == null) {
			resource = getNonHumanResource(id); // may also be null - means
												// unknown id
		}
		return resource;
	}
	...
	public Set<AbstractResource> getRoleParticipantsWithCapability(String rid, String cid) {
		...
		if (role != null) {
			...
			if (cid != null) {
				...
				resourceSet = role.getResources(); // no cid means don't filter
		}
		return resourceSet;
	}
	 ...
	 * 
	 * @param pid
	 *            the id of the 'manager' Participant
	...
	/**
	 * Gets the immediate supervisor of a participant. If the participant holds
	 * multiple positions, and therefore has multiple supervisors, one is
	 * returned as a random selection.
	 * 
	 * @param pid
	 *            the id of the participant to get the supervisor of
	 * @return the Participant who is the supervisor of the pid passed, or null
	 ...
	 * 
	 * @param p
	 *            the participant to get the supervisor of
	 * @return the Participant who is the supervisor of the pid passed, or null
	 *         if there is no supervisor.
	 */
	...
	public String getNonHumanSubCategoriesAsXML() {
		...
		for (NonHumanSubCategory n : getNonHumanSubCategories())
			xml.append(n.toXML());
	...
	public String checkCyclicAttributeReference(AbstractResourceAttribute resource, String parentID) {
	...
	public String checkCyclicOrgGroupReference(OrgGroup orgGroup, String refID) {
		...
		String refName = owner.getGroupName(); // name of group attempting to
												// add to
		while (owner != null) {
	...
	private String constructCyclicAttributeErrorMessage(List<String> chain, String type, String refName) {
	...
	public boolean logResourceBusy(String caseID, String id, boolean isBusy) {
		...
		if (resource != null) {
			EventLogger.log(null, caseID, id, (isBusy ? EventLogger.event.busy : EventLogger.event.released));
		}
