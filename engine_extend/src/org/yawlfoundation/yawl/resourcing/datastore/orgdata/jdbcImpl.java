 ...
 *
 * @author Michael Adams v0.1, 28/08/2007
 */
...
public class jdbcImpl extends DataSource {
	...
	// the constructor - loads the driver and creates tables if they don't
	// already exist
	public jdbcImpl() {
		super();
		if (loadDriver(dbDriver))
			checkTables();
	}
	...
	/**
	 * @return a statement object (or null if the statement could not be
	 *         created)
	 */
	 ...
	 * 
	 * @param statement
	 *            the statement object to execute the query through
	 * @param sql
	...
	private enum tbl {
		rsj_capability, rsj_role, rsj_orggroup, rsj_position, rsj_participant, rsj_participant_role, rsj_participant_position, rsj_participant_capability
	}
		...
		switch (table) {
			...
			case rsj_orggroup :
				stmt.append(" (groupid varchar(255) NOT NULL, groupname varchar(255),")
						.append(" grouptype varchar(255), description text,").append(" belongsto varchar(255),")
			...
			case rsj_participant_role :
				stmt.append(" (participantid varchar(255) NOT NULL,").append(" roleid varchar(255) NOT NULL,")
						.append(" CONSTRAINT rsj_participant_role_pkey").append(" PRIMARY KEY (participantid, roleid),")
						.append(" CONSTRAINT fk_rsj_participant_role FOREIGN KEY (roleid)")
						.append(" REFERENCES rsj_role (roleid) ON UPDATE NO ACTION").append(" ON DELETE NO ACTION,")
			...
			case rsj_participant_capability :
				stmt.append(" (participantid varchar(255) NOT NULL,").append(" capabilityid varchar(255) NOT NULL,")
			...
			case rsj_participant_position :
				stmt.append(" (participantid varchar(255) NOT NULL,").append(" p_id varchar(255) NOT NULL,")
	 ...
	 * @param field
	 *            the [field] in 'WHERE [field] = [value]'
	 * @param value
	 *            the [value] in the above
	...
	/**
	 * selects objects stored in the table passed, qualified by the field and
	 * value passed. Effectively 'SELECT * FROM [table] WHERE [field] = [value]'
	 * 
	 * @param table
	 *            the table to get the rows from
	 * @param field
	 *            the name of the field to query in the where clause
	 * @param value
	 *            the value to compare against the field values
	 * @return a List of instantiated objects, one for each row in the table
	 *         that matches: [field] = [value]
	 */
	private List select(tbl table, String field, String value) {
		...
		// *'
		String qry = (field != null) ? selectWhere(table.name(), field, value) : selectAll(table.name());
		// execute the selection
		ResultSet rs = execSelect(qry);

		if (rs != null) {
			try {
				// for each table row, create its object and add it to the
				// result List
				switch (table) {
					case rsj_participant :
						while (rs.next())
							result.add(makeParticipant(rs));
						break;
					case rsj_role :
						while (rs.next())
							result.add(makeRole(rs));
						break;
					case rsj_position :
						while (rs.next())
							result.add(makePosition(rs));
						break;
					case rsj_orggroup :
						while (rs.next())
							result.add(makeOrgGroup(rs));
						break;
					case rsj_capability :
						while (rs.next())
							result.add(makeCapability(rs));
						break;
				}
			} catch (SQLException sqle) {
	 ...
	 * @param value
	 *            the value to compare against the field values
	 * @return the object matching [field] = [value]. If more than one row is
	 *         matched, only the first if returned - the rest are ignored.
	 */
		...
		if (rs != null) {
			try {
				if (rs.next()) { // first record wanted only
					...
					switch (table) {
						case rsj_participant :
							return makeParticipant(rs);
						case rsj_role :
							return makeRole(rs);
						case rsj_position :
							return makePosition(rs);
						case rsj_orggroup :
							return makeOrgGroup(rs);
						case rsj_capability :
							return makeCapability(rs);
					}
				}
			} catch (SQLException sqle) {
				_log.error("Exception working with ResultSet", sqle);
	 ...
	 * 
	 * @param rs
	 *            the ResultSet containing the row of data
	 * @return a (partially) constructed Participant. Note that only the data
	 *         members are added here to the Participant object. The
	 *         many-to-many Roles, Positions and Capability Sets references need
	 *         to be added elsewhere.
	 * @throws SQLException
	 *             if there's a ResultSet issue
	 */
	 ...
	 * @param rs
	 *            the ResultSet containing the row of data
	 * @return a (mostly) constructed Role. Note that the id of the Role this
	 *         Role belongs to is stored here for instatiation of the Role
	 *         reference elsewhere.
	 * @throws SQLException
	 *             if there's a ResultSet issue
	 */
	 ...
	 * @param rs
	 *            the ResultSet containing the row of data
	 * @return a (mostly) constructed Position. Note that the id of the Position
	 *         this Position reports to, and the id of the OrgGRoup this
	 *         Position belongs to, are stored here (in the Position object) for
	 *         instatiation of the Position and OrgGroup object references
	 *         elsewhere.
	 * @throws SQLException
	 *             if there's a ResultSet issue
	 */
	 ...
	 * @param rs
	 *            the ResultSet containing the row of data
	 * @return a (mostly) constructed OrgGroup. Note that the id of the OrgGroup
	 *         this OrgGroup belongs to are stored here (in the OrgGroup object)
	 *         for instatiation of the OrgGroup object references elsewhere.
	 * @throws SQLException
	 *             if there's a ResultSet issue
	 */
	...
	/**
	 * gets a List of Role, Position or Capability object id's (depending on the
	 * table passed) corresponding to a particular participant id.
	 * 
	 * @param table
	 *            the (foreign-key) table to get the id's from
	 * @param pid
	 *            the Participant id
		...
		if (rs != null) {
			try {
				switch (table) {
					case rsj_participant_role :
						while (rs.next())
							ids.add(rs.getString("RoleID"));
						break;
					case rsj_participant_position :
						while (rs.next())
							ids.add(rs.getString("p_id"));
						break;
					case rsj_participant_capability :
						while (rs.next())
							ids.add(rs.getString("CapabilityID"));
						break;
				}
				rs.close();
			} catch (SQLException sqle) {
	...
	/**
	 * gets a List of Participant object id's corresponding to a particular
	 * attribute id (ie Role, Position or Capability) depending on the table
	 * passed
	 * 
	 * @param table
	 *            the (foreign-key) table to get the id's from
	...
	private List<String> selectAttributeParticipantIDs(tbl table, String id) {
		...
		if (rs != null) {
			try {
				while (rs.next())
					ids.add(rs.getString("ParticipantID"));
				rs.close();
			} catch (SQLException sqle) {
				_log.error("Exception working with " + table.name() + " ResultSet", sqle);
				return null;
	 ...
	 * 
	 * @return a map of Capability objects of the form [capabilityID,
	 *         Capability]
	 */
	...
	/**
	 * Loads all OrgGroup records from the data store, and correctly maps
	 * 'belongs to' OrgGroup references
	 * 
	...
	private HashMap<String, OrgGroup> loadOrgGroups() {
		...
		if (ogList != null) {
			...
			for (OrgGroup o : ogList)
				map.put(o.getID(), o);

			// ... then set the relevant 'belongs to' OrgGroup (by id)
			for (OrgGroup o : ogList)
				o.setBelongsTo(map.get(o.get_belongsToID()));
		}
		return map;
	}

	/**
	 * Loads all Position records from the data store, and correctly maps
	 * 'belongs to' OrgGroup and 'reports to' Position references
	 * 
	...
	private HashMap<String, Position> loadPositions(Map<String, OrgGroup> ogMap) {
		...
		if (posList != null) {
			...
			for (Position p : posList)
				p.setReportsTo(map.get(p.get_reportsToID()));
		}
		return map;
	}

	/**
	 * Loads all Role records from the data store, and correctly maps 'belongs
	 * to' Role references
	 * 
	...
	private HashMap<String, Role> loadRoles() {
		...
		if (roleList != null) {
			...
			for (Role r : roleList)
				r.setOwnerRole(map.get(r.get_belongsToID()));
		}
		return map;
	}
	 ...
	 * many-to-many relations between each Participant and its Role, Position
	 * and Capability Sets
	 * 
	 * @return a map of Participant objects of the form [participantID,
	 *         Participant]
	 */
	...
	private void updateParticipant(Participant p) {
		...
		String qry = String.format(
				...
				p.getDescription(), p.getNotes(), p.isAvailable(), p.getLastName(), p.getFirstName(), p.getUserID(),
				p.getPassword(), p.isAdministrator(), p.getID());
		execUpdate(qry);
	...
	private String insertParticipant(Participant p) {
		...
		String qry = String.format(
				"INSERT INTO rsj_participant VALUES " + "('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%b')", id,
				p.getDescription(), p.getNotes(), p.isAvailable(), p.getLastName(), p.getFirstName(), p.getUserID(),
				p.getPassword(), p.isAdministrator());
		if (qry != null)
			execUpdate(qry);
	...
	private void deleteParticipant(Participant p) {
		String qry = String.format("DELETE FROM rsj_participant WHERE participantID = '%s'", p.getID());
		execUpdate(qry);
		deleteParticipantAttributeRows(p.getID()); // cascade to joined tables
	}

	private void deleteParticipantAttributeRows(String pid) {
		String qry = String.format("DELETE FROM rsj_participant_capability WHERE participantID = '%s'", pid);
		execUpdate(qry);
		qry = String.format("DELETE FROM rsj_participant_position WHERE participantID = '%s'", pid);
		execUpdate(qry);
		qry = String.format("DELETE FROM rsj_participant_role WHERE participantID = '%s'", pid);
		execUpdate(qry);
	}
	...
	private void insertParticpantAttributeRows(Participant p, String id) {
		...
		for (Role r : p.getRoles()) {
			qry = String.format("INSERT INTO rsj_participant_role VALUES ('%s', '%s')", id, r.getID());
			if (qry != null)
				execUpdate(qry);
		}
		for (Capability c : p.getCapabilities()) {
			qry = String.format("INSERT INTO rsj_participant_capability VALUES ('%s', '%s')", id, c.getID());
			if (qry != null)
				execUpdate(qry);
		}
		for (Position po : p.getPositions()) {
			qry = String.format("INSERT INTO rsj_participant_position VALUES ('%s', '%s')", id, po.getID());
			if (qry != null)
				execUpdate(qry);
		}
	}
	...
	private void updateRole(Role r) {
		...
		String qry = String.format(
				"UPDATE rsj_Role SET rolename = '%s', description = '%s', " + "belongsTo = '%s' WHERE roleID = '%s'",
	...
	private String insertRole(Role r) {
		...
		String ownerRole = (r.getOwnerRole() != null) ? r.getOwnerRole().getID() : null;
		String qry = String.format("INSERT INTO rsj_role VALUES ('%s', '%s', '%s', '%s')", id, r.getName(),
				r.getDescription(), ownerRole);
		if (qry != null)
			execUpdate(qry);
		return id;
	}
	...
	private void deleteRole(Role r) {
		String qry = String.format("DELETE FROM rsj_Role WHERE roleID = '%s'", r.getID());
		execUpdate(qry);
	}
	...
	private void updateCapability(Capability c) {
		String qry = String.format(
				"UPDATE rsj_Capability SET capability = '%s', description = '%s' " + "WHERE capabilityID = '%s'",
	...
	private String insertCapability(Capability c) {
		...
		String qry = String.format("INSERT INTO rsj_capability VALUES ('%s', '%s', '%s')", id, c.getCapability(),
				c.getDescription());
		if (qry != null)
			execUpdate(qry);
		return id;
	...
	private void deleteCapability(Capability c) {
		String qry = String.format("DELETE FROM rsj_Capability WHERE CapabilityID = '%s'", c.getID());
		execUpdate(qry);
	}
	...
	private void updatePosition(Position p) {
		...
		String qry = String.format(
				...
				p.getPositionID(), p.getTitle(), p.getDescription(), belongsTo, reportsTo, p.getID());
		execUpdate(qry);
	}
	...
	private String insertPosition(Position p) {
		...
		String belongsTo = (p.getOrgGroup() != null) ? p.getOrgGroup().getID() : null;
		String reportsTo = (p.getReportsTo() != null) ? p.getReportsTo().getID() : null;
		String qry = String.format("INSERT INTO rsj_position VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", id,
				p.getPositionID(), p.getTitle(), p.getDescription(), belongsTo, reportsTo);
		if (qry != null)
			execUpdate(qry);
		return id;
	}
	...
	private void deletePosition(Position p) {
		String qry = String.format("DELETE FROM rsj_Position WHERE P_ID = '%s'", p.getID());
		execUpdate(qry);
	}
	...
	private String insertOrgGroup(OrgGroup o) {
		...
		String qry = String.format("INSERT INTO rsj_orggroup VALUES ('%s', '%s', '%s', '%s', '%s')", id,
				o.getGroupName(), o.get_groupType(), o.getDescription(), belongsTo);
	...
	private void updateOrgGroup(OrgGroup o) {
		...
		String qry = String.format(
				...
				o.getGroupName(), o.get_groupType(), o.getDescription(), belongsTo, o.getID());
		execUpdate(qry);
	}

	private void deleteOrgGroup(OrgGroup o) {
		String qry = String.format("DELETE FROM rsj_orggroup WHERE groupID = '%s'", o.getID());
		execUpdate(qry);
	}

	/***********************************************************************************/

	// BASE CLASS IMPLEMENTATIONS (see DataSource for the purpose of each
	// method) //
