...
/**
 * 1. When called directly (by the ResourceManager) will refresh the externally
 * sourced org data. 2. When enabled via context configuration, repeatedly
 * refreshes the externally sourced org data after each specified period.
 * 
...
public class OrgDataRefresher {
	...
	/**
	 * Constructs an instance to be used for 'manual' refreshing (via a call to
	 * refresh())
	 * 
	 * @param rm
	...
	/**
	 * Constructs an instance that starts a timer to refresh org data every
	 * 'period'
	 * 
	 * @param rm
	 *            the ResourceManager instance
	...
	private void refresh(long period) {
		_refresherTask = _scheduler.scheduleAtFixedRate(new RefreshRunnable(), period, period, TimeUnit.MINUTES);
	}
