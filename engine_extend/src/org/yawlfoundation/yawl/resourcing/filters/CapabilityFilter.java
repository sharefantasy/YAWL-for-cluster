...
public class CapabilityFilter extends AbstractFilter {
	...
	public CapabilityFilter() {
		...
		setDescription("This Capability Filter will filter a distribution set "
				+ "based on whether each Participant in the set has a " + "the Capability specified.");
	}
	...
	private Set<AbstractResource> parse(String expression) {
		if (expression != null) {
			...
			for (String capName : expression.split("[&|]")) {
				Capability c = ResourceManager.getInstance().getOrgDataSet().getCapabilityByLabel(capName.trim());
				if (c == null)
					c = new Capability();
