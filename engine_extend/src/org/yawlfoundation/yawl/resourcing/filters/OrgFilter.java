...
public class OrgFilter extends AbstractFilter {
	...
	private Set<AbstractResource> parsePositionValue(String expression) {
		if (expression != null) {
			...
			for (String posName : expression.split("[&|]")) {
				Position p = ResourceManager.getInstance().getOrgDataSet().getPositionByLabel(posName.trim());
				if (p == null)
					p = new Position();
	...
	private Set<AbstractResource> parseOrgGroupValue(String expression, Set<Participant> distSet) {
		if (expression != null) {
			List<Set<AbstractResource>> pSets = new ArrayList<Set<AbstractResource>>();
			for (String ogName : expression.split("[&|]")) {
				OrgGroup og = ResourceManager.getInstance().getOrgDataSet().getOrgGroupByLabel(ogName.trim());
				if (og == null)
					og = new OrgGroup();
				Set<AbstractResource> resources = new HashSet<AbstractResource>();
				for (Participant p : distSet) {
					if (p.isOrgGroupMember(og))
						resources.add(p); // hierarchical
				}
				pSets.add(resources);
			}
			return evaluate(pSets, expression);
