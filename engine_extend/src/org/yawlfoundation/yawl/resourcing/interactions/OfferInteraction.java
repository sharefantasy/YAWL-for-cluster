 ...
 *
 * @author Michael Adams v0.1, 02/08/2007
 */
...
public class OfferInteraction extends AbstractInteraction {
	 ...
	 * 
	 * @param p
	 *            - the Participant object to add to the initial distribution
	 *            list
	 */
	...
	/**
	 * Takes the initial distribution set of participants, then expands any
	 * roles and/or dynamic parameters to their 'set of participants'
	 * equivalents, then applies the specified filters and/or constraints, and
	 * returns the final distribution set of participants.
	 *
	 * @param wir
	 *            the workitem being offered
	...
	public Set<Participant> performOffer(WorkItemRecord wir) {
		...
		// if familiar task specified, get the participant(s) who completed that
		// task,
	...
	public void withdrawOffer(WorkItemRecord wir, HashSet<Participant> offeredSet) {
		...
		// a fired instance of a multi-instance workitem on the unoffered queue
		// will
		// never have been offered, so the warning should be suppressed for
		// those
		else if (!wir.getStatus().equals(WorkItemRecord.statusFired)) {
			_log.warn("Workitem '" + wir.getID() + "' does not have 'Offered' status, " + "or is no longer active");
		}
	}
	...
	/**
	 * A class that encapsulates one dynamic parameter - i.e. a data variable
	 * that at runtime will contain a value corresponding to a participant or
	 * role that the task is to be offered to.
	 */
	private class DynParam {
		 ...
		 *
		 * @param name
		 *            - the name of a data variable of this task that will
		 *            contain a runtime value specifying a particular
		 *            participant or role.
		 * @param refers
		 *            - either USER_PARAM or ROLE_PARAM
		 */
			...
			if (_refers == USER_PARAM) {
				...
				for (String varID : getVarIDList(wir)) {
					...
					if (r != null) {
						...
						_log.error("Unknown role '" + varID + "' in dynamic parameter: " + _name);
				}
			}
			return result;
		}
		...
		private String getNetParamValue(WorkItemRecord wir, String name) {
			...
			try {
				...
				if (result == null)
					_log.error("Unable to retrieve value from net parameter '" + name
							+ "' for deferred allocation of workitem '" + wir.getID() + "'.");
			} catch (IOException ioe) {
				_log.error("Caught exception attempting to retrieve value from net parameter '" + name
						+ "' for deferred allocation of workitem '" + wir.getID() + "'.");
			}
			return result;
		}
