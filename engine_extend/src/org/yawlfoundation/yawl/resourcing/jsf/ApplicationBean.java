 ...
 *
 * @author Michael Adams BPM Group, QUT Australia v0.1, 21/10/2007
 *         <p/>
...
public class ApplicationBean extends AbstractApplicationBean {
	...
	public void init() {
		...
		// Add init code here that must complete *after* managed components are
		// initialized
	...
	public enum PageRef {
		adminQueues, caseMgt, customServices, dynForm, Login, orgDataMgt, nonHumanMgt, participantData, selectUser, userWorkQueues, viewProfile, addInstance, teamQueues, externalClients, calendarMgt, secResMgt;
	...
	public enum TabRef {
		offered, allocated, started, suspended, unoffered, worklisted
	}
	...
	private Map<String, SessionBean> sessionReference = new ConcurrentHashMap<String, SessionBean>();
	...
	private Map<String, Map<String, FormParameter>> _workItemParams = new ConcurrentHashMap<String, Map<String, FormParameter>>();
	...
	// the footer text is the text line at the bottom of every worklist and
	// admin page
	...
	public void setFooterPanel(PanelLayout panel) {
		footerPanel = panel;
	}
	...
	public PanelLayout getFooterTextPanel() {
		return footerTextPanel;
	}

	public void setFooterTextPanel(PanelLayout panel) {
		footerTextPanel = panel;
	}

	private StaticText footerStaticText;

	public StaticText getFooterStaticText() {
		return footerStaticText;
	}

	public void setFooterStaticText(StaticText st) {
		footerStaticText = st;
	}
		...
		if (_footerText.length() == 0) { // only need to build the text once
			...
			if (_rm.successful(engBuildProps)) {
				...
				engBuild = String.format(" | Engine build %s (%s)",
						formatBuildNumber(propsNode.getChildText("Version"), propsNode.getChildText("BuildNumber")),
	...
	// if an offered workitem has no resource map (usually a pre-2.0 spec), it
	// can't
	// be reoffered on the admin screen. This map keeps track of listed
	// workitems and
	...
	public void refreshExternalClients() {
		try {
			List<YExternalClient> clients = new ArrayList<YExternalClient>(_rm.getClients().getExternalClients());
