...
/**
 * Author: Michael Adams Creation Date: 1/09/2009
 */
public class FormViewer {
		...
		if (result == null) {
			...
			if (factory.makeForm(title, null, _sb.getTaskSchema(wir), wir)) {
				...
				result = "<failure>Cannot view item contents - problem initialising "
						+ "dynamic form from task specification. " + "Please see the log files for details.</failure>";
			}
		}
		return result;
	}
	...
	private String showCustomForm(WorkItemRecord wir) {
		...
		if (url != null) {
			...
			if (validateMsg.equals("<success/>")) {
				...
				return uriPlusParams; // return validated custom form url incl.
										// params
			} else
				_log.warn("Missing or invalid custom form: '" + uriPlusParams.substring(0, uriPlusParams.indexOf('?'))
						+ "', message: " + StringUtil.unwrap(validateMsg) + ". Defaulting to dynamic form.");
		} else
			_log.warn("Unspecified form URI. Defaulting to dynamic form.");
	...
	private String buildURI(WorkItemRecord wir) {
		...
		redir.append((redir.indexOf("?") == -1) ? "?" : "&") // any static
																// params?
				.append("workitem=").append(wir.getID()).append("&participantid=").append(_sb.getParticipant().getID())
				.append("&handle=").append(_sb.getSessionhandle()).append("&source=").append(getSourceURI());
		...
		try {
			...
			if (dataStr != null) {
				...
				for (int i = 0; i < parts.length; i++) {
					String part = parts[i];
					if (part.startsWith("{")) {
						parts[i] = evaluateXQuery(part.substring(1, part.lastIndexOf('}')), dataDoc);
					}
				}
