...
public class Login extends AbstractPageBean {
	...
	public void setLblUserName(Label l) {
		lblUserName = l;
	}
	...
	public String btnLogin_action() {
		...
		if ((_sb.getUserid() != null) && _sb.hasNavigationBegun()) {
			_msgPanel.error("User '" + _sb.getUserid() + "' is already logged on in this"
					...
					+ " instance is possible. If you wish to logon, please " + " logout the current user first.");
		}

		// session is free, so if there's a valid org data source --> process
		// the logon
	...
	private void initSession(String userid, String handle) {
		...
		((pfMenubar) getBean("pfMenubar")).construct(userid.equals("admin")); // make
																				// the
																				// menu
																				// bar
	}
