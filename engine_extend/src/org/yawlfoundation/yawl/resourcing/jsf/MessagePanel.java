 ...
 *
 * Author: Michael Adams Creation Date: 28/02/2008 Last Date: 17/04/2010
 */
...
public class MessagePanel extends PanelLayout {
	...
	public MessagePanel() {
		_messages = new LinkedHashMap<String, MsgType>(); // keeps insertion
															// order
		setId("msgPanel001");
	...
	public void show(int top, int left, String position) {
		_style = String.format("top: %dpx; left: %dpx; position: %s;", top, left, position);
		showPanel();
	}
	...
	private ImageComponent constructImage() {
		...
		_imgIcon.setStyle(getPosStyle(10, 30) + "background-color: #f0f0f0; " + "height: 48px; width: 48px;");
		return _imgIcon;
	}
	...
	private PanelLayout constructTitleBar() {
		...
		titleBar.setStyle("background-color: #b0b0b0; border-bottom: 1px solid black;"
				+ "width: 100%; text-align: left; font-weight: bold; " + "height: " + TITLEBAR_HEIGHT + "px;");
	...
	private MethodBinding bindButtonListener() {
		...
		return app.createMethodBinding("#{SessionBean.messagePanelOKBtnAction}", new Class[]{ActionEvent.class});
	}
	...
	/**
	 * Each message goes inside its own panel, which in turn goes inside
	 * _pnlMessages
	 * 
	...
	private MsgType getDominantType() {
		if (_messages.containsValue(MsgType.error))
			return MsgType.error;
		else if (_messages.containsValue(MsgType.warn))
			return MsgType.warn;
		else if (_messages.containsValue(MsgType.info))
			return MsgType.info;
		else if (_messages.containsValue(MsgType.success))
	...
	private void listMessages() {
		if (hasMessage()) {
			...
			for (String message : _messages.keySet()) {
				String msg = mixed ? getTitle(_messages.get(message)) + ": " + message : message;
	...
	private void addVSpace() {
		...
		label.setStyle("color:#f0f0f0; font-size:6px;"); // same colour as
															// background
	...
	private void setButtonPanelStyle(int height) {
		_btnPanel.setStyle("background-color: #f0f0f0; width: 100%; height: " + BTNPANEL_HEIGHT
				+ "px; position: absolute; top:" + (height - BTNPANEL_HEIGHT) + "px;");
	}
	...
	private void setStyle(int width, int height) {
		...
		String style = String.format("%s height: %dpx; width: %dpx;", _style, height, width);
	...
	private int getMessagesHeight(int width) {
		...
		if (hasMessage()) {
			...
			height += MESSAGE_VSPACE * _messages.size() - (totLines * 2); // add
																			// vspace
		}
		return height;
	}
	...
	private int sizeAndPositionContent(int minWidth) {
		...
		_pnlMessages.setStyle(String.format("background-color: #f0f0f0; %s width: %dpx", getPosStyle(70, 35), width));
