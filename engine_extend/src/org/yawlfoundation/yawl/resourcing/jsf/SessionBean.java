...
public class SessionBean extends AbstractSessionBean {
	...
	public void setBtnRefresh(Button btn) {
		btnRefresh = btn;
	}
	...
	public void setQueueSet(QueueSet qSet) {
		queueSet = qSet;
	}
	...
	public Set<WorkItemRecord> getQueue(int qType) {
		...
		if (qSet != null)
			result = qSet.getQueuedWorkItems(qType);
	...
	private Map<ResourceDataSet.ResUnit, Long> _changeStamp = new Hashtable<ResourceDataSet.ResUnit, Long>();
	...
	public String getWorklistChoice() {
		return worklistChoice;
	}
	public YSpecificationID getLoadedSpecListChoice() {
		return loadedSpecListChoice;
	}
	public String getRunningCaseListChoice() {
		return runningCaseListChoice;
	}
	public String getSelectUserListChoice() {
		return selectUserListChoice;
	}
	public String getOrgDataChoice() {
		return orgDataChoice;
	}
	public String getPiledTasksChoice() {
		return piledTasksChoice;
	}
	public String getChainedCasesChoice() {
		return chainedCasesChoice;
	}
	public String getOrgDataBelongsChoice() {
		return orgDataBelongsChoice;
	}
	public String getOrgDataGroupChoice() {
		return orgDataGroupChoice;
	}
	public String getNhResourcesChoice() {
		return nhResourcesChoice;
	}
	public String getNhResourcesCategoryChoice() {
		return nhResourcesCategoryChoice;
	}
	public String getNhResourcesSubcategoryChoice() {
		return nhResourcesSubcategoryChoice;
	}

	public void setWorklistChoice(String choice) {
		worklistChoice = choice;
	}
	public void setRunningCaseListChoice(String choice) {
		runningCaseListChoice = choice;
	}
	public void setSelectUserListChoice(String choice) {
		selectUserListChoice = choice;
	}
	public void setOrgDataChoice(String choice) {
		orgDataChoice = choice;
	}
	public void setPiledTasksChoice(String choice) {
		piledTasksChoice = choice;
	}
	public void setChainedCasesChoice(String choice) {
		chainedCasesChoice = choice;
	}
	public void setOrgDataBelongsChoice(String choice) {
		orgDataBelongsChoice = choice;
	}
	public void setOrgDataGroupChoice(String choice) {
		orgDataGroupChoice = choice;
	}
	public void setNhResourcesChoice(String choice) {
		nhResourcesChoice = choice;
	}
	public void setNhResourcesCategoryChoice(String choice) {
		nhResourcesCategoryChoice = choice;
	}
	public void setNhResourcesSubcategoryChoice(String choice) {
		nhResourcesSubcategoryChoice = choice;
	}
	...
	public void setDirectToMeChoice(String choice) {
		directToMeChoice = choice;
	}

	public String getDirectToMeChoice() {
		return directToMeChoice;
	}
	...
	public Listbox getLbxRunningCases() {
		return lbxRunningCases;
	}

	public void setLbxRunningCases(Listbox l) {
		lbxRunningCases = l;
	}
	...
	public String getRunningCasesCaption() {
		...
		if (runningCaseListOptions != null)
			caption += " (" + runningCaseListOptions.length + ")";
		return caption;
	}
	...
	public void setCaseLaunch(boolean caseLaunch) {
		this.caseLaunch = caseLaunch;
	}
	...
	public boolean isCustomFormPost() {
		return customFormPost;
	}

	public void setCustomFormPost(boolean flag) {
		customFormPost = flag;
	}
	...
	private String formatPiledTaskString(ResourceMap map) {
		...
		return String.format("%s (%s)::%s", specID.getUri(), specID.getVersionAsString(), map.getTaskID());
	}
		...
		if (label != null) {
			...
			if (loadedSpecs != null) {
				...
				for (SpecificationData specData : loadedSpecs) {
					if (specData.getSpecURI().equals(specURI) && specData.getSpecVersion().equals(version)) {
						specID = new YSpecificationID(specData.getSpecIdentifier(), specData.getSpecVersion(),
								specData.getSpecURI());
						break;
					}
				}
				if (specID != null) {
					...
					if (map == null)
						map = _rm.getPersistedPiledTask(specID, taskID);
					return map;
				}
			}
		}
	...
	public String getLoadedSpecsCaption() {
		...
		if (loadedSpecs != null)
			caption += " (" + loadedSpecs.size() + ")";
		return caption;
	}

	public SpecificationData getLoadedSpec(int listIndex) {
		return ((getLoadedSpecs() != null) && (!loadedSpecs.isEmpty())) ? loadedSpecs.get(listIndex) : null;
	}
	...
	public boolean isLoadedSpec(String uri, String version, String documentation) {
		...
		if (specs != null) {
			for (SpecificationData sd : specs) {
				if (sd.getSpecURI().equals(uri) && sd.getSpecVersion().equals(version)
	...
	public void populateAdminQueueAssignedList(WorkItemRecord wir) {
		...
		if (pSet != null) {
			...
			for (Participant p : pList) {
				adminQueueAssignedList[i++] = new Option(p.getID(), p.getLastName() + ", " + p.getFirstName());
			}
		}
	}
	...
	public boolean orgDataIsRefreshing() {
		if (_rm.isOrgDataRefreshing()) {
			messagePanel.info("Organisational data is currently refreshing. " + "Please try again in a moment.");
			showMessagePanel();
			return true;
		}
		return false;
	...
	public boolean performAdminQueueAction(String action) {
		...
		if (directToMeChoice != null) {
			...
			if (selected != null) { // null if browser-back-btn'ed
				...
				// If its a multi-select list, selected will be a String[]; if
				// its a
				// single-select list, selected will be a single String, in
				// which case
				// we need to create a new String array and add the selected
				// String as
				// its only member. Each String represents a participant id.
				pidList = (selected instanceof String[]) ? (String[]) selected : new String[]{(String) selected};
			}
		}
		if (pidList != null) {
	...
	public Option[] getOrgDataParticipantList() {
		if ((orgDataParticipantList == null) || orgDataUpdated(ResourceDataSet.ResUnit.Participant)) {
	...
	public void refreshOrgDataParticipantList() {
		...
		if (pMap != null) {
			...
			Collections.sort(pList, new ParticipantNameComparator());
			for (Participant p : pList) {
				orgDataParticipantList[i++] = new Option(p.getID(), p.getLastName() + ", " + p.getFirstName());
			}
		} else
	...
	public void saveParticipantUpdates(Participant cloned) throws CloneNotSupportedException, ResourceGatewayException {
	...
	public void loadSecondaryResources() {
		int queue = getActiveTab().equals("tabUnOffered") ? WorkQueue.UNOFFERED : WorkQueue.WORKLISTED;
	...
	public String getSecondaryResourcesItemID() {
		return secondaryResourcesItemID;
	}

	public void setSecondaryResourcesItemID(String s) {
		secondaryResourcesItemID = s;
	}
	...
	public boolean removeSelectedSecondaryResource() {
		return (selectedSecondaryResource != null) && secResDataSet.remove(selectedSecondaryResource);
	}

	private String selectedSecondaryResource = null;

	public String getSelectedSecondaryResource() {
		return selectedSecondaryResource;
	}

	public void setSelectedSecondaryResource(String s) {
		selectedSecondaryResource = s;
	}
	...
	private void selectResourceAttribute(String id, Participant p) throws ResourceGatewayException {
		if (activeResourceAttributeTab.equals("tabRoles"))
			p.addRole(id);
		else if (activeResourceAttributeTab.equals("tabPosition"))
			p.addPosition(id);
	...
	public Participant resetParticipant() throws CloneNotSupportedException {
		if (editedParticipant != null) {
			String actualID = editedParticipant.getID().substring(7); // remove
																		// 'CLONE_'
																		// from
																		// edited
																		// id
	...
	public void removeParticipant(Participant p) {
		String actualID = p.getID().substring(7); // remove 'CLONE_' from edited
													// id
	...
	public void setLblAgeText(String text) {
		lblAgeText = text;
	}
	...
	public String getInitTabText(int n) {
		...
		if (queueSet != null) {
			...
			if (qSize > 0)
				result = result + String.format(" (%d)", qSize);
		}
		return result;
	}

	public String getInitOfferedTabText() {
		return getInitTabText(WorkQueue.OFFERED);
	}

	public String getInitAllocatedTabText() {
		return getInitTabText(WorkQueue.ALLOCATED);
	}

	public String getInitStartedTabText() {
		return getInitTabText(WorkQueue.STARTED);
	}

	public String getInitSuspendedTabText() {
		return getInitTabText(WorkQueue.SUSPENDED);
	}

	public String getInitUnOfferedTabText() {
		return getInitTabText(WorkQueue.UNOFFERED);
	}

	public String getInitWorklistedTabText() {
		return getInitTabText(WorkQueue.WORKLISTED);
	}
	...
	public void showMessagePanel() {
		String style = (activePage != ApplicationBean.PageRef.dynForm) ? null : "border: none;";
	...
	public boolean isAddInstance() {
		return addInstanceParamVal != null;
	}
	...
	public boolean isDelayDurationRBSelected() {
		return delayDurationRBSelected;
	}
	...
	public void setTeamRBSelected(boolean selected) {
		teamRBSelected = selected;
	}

	public boolean getOrgGroupRBSelected() {
		return orgGroupRBSelected;
	}
	...
	public void setOrgGroupRBSelected(boolean selected) {
		orgGroupRBSelected = selected;
	}

	public boolean getTeamRBDisabled() {
		return (participant == null)
				|| (!(participant.isAdministrator() || participant.getUserPrivileges().canViewTeamItems()));
	}

	public boolean getOrgGroupRBDisabled() {
		return (participant == null)
				|| (!(participant.isAdministrator() || participant.getUserPrivileges().canViewOrgGroupItems()));
	}
	...
	public String getDelayValueError() {
		return delayValueError;
	}

	public void setDelayValueError(String error) {
		delayValueError = error;
	}
	...
	public boolean isShowYAWLBanner() {
		return showYAWLBanner;
	}

	public void setShowYAWLBanner(boolean show) {
		showYAWLBanner = show;
	}
	...
	public boolean isSubCatAddMode() {
		return subCatAddMode;
	}

	public void setSubCatAddMode(boolean adding) {
		subCatAddMode = adding;
	}

	// list of resources in a selected category - fills the members combo on
	// category tab
	...
	public void setSelectedNonHumanResource(NonHumanResource resource, boolean editing)
		...
		if (resource != null) {
			...
			// an id of _TEMP_ means a new resource (in add mode), so don't
			// clone it
			selectedNonHumanResource = (!resource.getID().equals("_TEMP_")) ? resource.clone() : resource;
		} else
			selectedNonHumanResource = null;
	}
	...
	// sets the 'top' of each page's main panel based on the nbr of menubars on
	// view
	...
	public CalendarRow getSelectedCalendarRow() {
		return (selectedCalendarRowIndex > -1) ? calendarRows.get(selectedCalendarRowIndex) : null;
	}
	...
	public int getSelectedCalendarRowIndex() {
		return selectedCalendarRowIndex;
	}

	public void setSelectedCalendarRowIndex(int index) {
		selectedCalendarRowIndex = index;
	}
	...
	public Date getSelectedCalMgtDate() {
		return selectedCalMgtDate;
	}

	public void setSelectedCalMgtDate(Date date) {
		selectedCalMgtDate = date;
	}
	...
	public String getCalFilterSelection() {
		return calFilterSelection;
	}

	public void setCalFilterSelection(String selection) {
		calFilterSelection = selection;
	}

	private String calResourceSelection = null;

	public String getCalResourceSelection() {
		return calResourceSelection;
	}

	public void setCalResourceSelection(String selection) {
		calResourceSelection = selection;
	}
	...
	public void addCalendarEntry(long startTime, long endTime, int workload, String comments) throws CalendarException {
		if (calFilterSelection.startsWith("All")) {
			ResourceCalendar.ResourceGroup group = getSelectedResourceGroup();
			long entryID = _rm.getCalendar().addEntry(group, startTime, endTime, ResourceCalendar.Status.unavailable,
					getUserid(), comments);
	...
	public void setAddCalendarRowMode(boolean mode) {
		addCalendarRowMode = mode;
	}

	public String calEditedResourceName;

	public String getCalEditedResourceName() {
		return calEditedResourceName;
	}

	public void setCalEditedResourceName(String name) {
		calEditedResourceName = name;
	}
		...
		if (!calFilterSelection.startsWith("All")) {
			for (CalendarRow row : calendarRows) {
				if ((add || (row.getEntryID() != entry.getEntryID()))
						...
						&& (row.getStartTime() < entry.getEndTime()) && (row.getEndTime() > entry.getStartTime())
	...
	public String getTxtDocumentationStyle() {
		boolean lower = (activeTab != null && activeTab.equals("tabWorklisted"))
				|| activePage == ApplicationBean.PageRef.teamQueues;
		return String.format("height: %dpx; top: %dpx;", (lower ? 44 : 84), (lower ? 314 : 274));
	}
