...
/**
 * Allows Workitems to be sorted on enablement time for display in age order in
 * workqueues
 *
 * Author: Michael Adams Creation Date: 15/01/2008
 */
public class WorkItemAgeComparator implements Comparator<WorkItemRecord> {
	...
	public int compare(WorkItemRecord wir1, WorkItemRecord wir2) {
		...
		// if one object is null, ignore it and return the other as having
		// precedence
		if (wir1 == null)
			return -1;
		if (wir2 == null)
	 ...
	 * compare on caseids (loose assumption: caseids get allocated in
	 * chronological order - which is not *always* true)
	 */
	private int compareCaseIDs(WorkItemRecord wir1, WorkItemRecord wir2) {
		...
		// if identical enablement times *and* caseid's, make an arbitrary
		// decision
		if (result == 0)
			result = 1;
