 ...
 *
 * @author Michael Adams BPM Group, QUT Australia v0.1, 08/01/2008
 *
 *
...
public class dynForm extends AbstractPageBean {
	...
	public void setBottomPanel(PanelLayout pnl) {
		bottomPanel = pnl;
	}
	...
	/**
	 * Same as btnOK, but also sets a flag to complete the item directly. Note:
	 * this button is only visible for item edits, and not for case starts
	 * 
	 * @return a reference to the referring page
	 */
	...
	private String getReferringPage() {
		getDynFormFactory().resetFormHeight(); // for subsequent message panel
												// location
		String result;
		if (_sb.getDynFormType() == ApplicationBean.DynFormType.netlevel) {
			result = "showCaseMgt";
	...
	/**
	 * Loads the background image or colour for the entire page (not the form
	 * within)
	 */
		...
		if (!docIDs.isEmpty()) {
			...
			if (client != null) {
				try {
					...
					// an id of -1 means no doc was uploaded by the doc
					// component
