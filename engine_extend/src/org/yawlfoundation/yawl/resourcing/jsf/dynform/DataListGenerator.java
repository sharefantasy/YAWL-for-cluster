...
public class DataListGenerator {
	...
	private String generateDataList(PanelLayout panel, List<DynFormField> fieldList) {
		...
		if (o instanceof RadioButton) {
			...
			if (!headedPanel(panel)) {
				...
				// if this is the outermost panel, its title may be user
				// defined, so get
				// the default form name
				if (!(panel instanceof SubPanel)) {
					parentTag = _factory.getDefaultFormName();
				} else {
					// otherwise get the default name of the panel (and thus the
					// element name)
		...
		for (int i = start; i < stop; i++) {
			...
			if (child instanceof SubPanel) {
				...
				result.append(generateDataList((PanelLayout) child, field.getSubFieldList()));
			}
	...
	private String getFieldValue(PanelLayout panel, Label label, DynFormField field) {
		...
		if (component instanceof TextField)
			...
			value = (String) ((FieldBase) component).getText(); // default
																// fallthrough
	...
	private String formatField(String value, DynFormField field) {
		...
		if (field.isEmptyComplexTypeFlag()) {
			if ((value == null) || value.equals("false")) {
				...
				value = ""; // empty data element for this field
		}
	...
	private boolean headedPanel(PanelLayout panel) {
		List children = panel.getChildren();
		return (children != null) && (!children.isEmpty()) && (children.get(0) instanceof StaticText);
	}
	...
	private DynFormField getField(UIComponent component, List<DynFormField> fieldList) {
		...
		if (component instanceof SubPanel) {
			...
			try {
				DynFormField formField = _factory.getFieldForComponent((UIComponent) label.getLabeledComponent());
				if (formField != null)
					return formField;
			...
			// fallback to label text for identification (may be ambiguous if
			// two
	...
	/**
	 * Calculates and holds the bounds of the selected field(s) within a choice
	 * panel
	 */
	static class SelectedChoiceBounds {
		...
		/**
		 * Works through a choice panel's components, finding the selected radio
		 * button then marking the bounds of it and the last component of the
		 * selection.
		 * 
		 * @param children
		 *            the panel's child components
		 */
		static void calcBounds(List children) {
			...
			// if no later radio found, upper bound is last component in the
			// list
			if (stop == -1)
				stop = children.size();
		}
