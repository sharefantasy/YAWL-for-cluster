...
public class DocComponent extends PanelLayout {
	...
	public String getOutputXML() {
		...
		if (docID > -1)
			sb.append(StringUtil.wrap(String.valueOf(docID), "id"));
			...
			if (client != null) {
				...
				try {
					...
					if (!((id == null) || id.startsWith("<fail"))) {
						...
						errorMsg = StringUtil.unwrap(id);
				} catch (IOException ioe) {
					errorMsg = ioe.getMessage();
				}
			} else
				errorMsg = "Unable to upload document: missing Document Store.";
		} else
			errorMsg = "Error uploading document. File length is zero";
	...
	private MethodBinding bindButtonListener() {
		...
		return app.createMethodBinding("#{dynForm.btnDocumentAction}", new Class[]{ActionEvent.class});
	}
		...
		if (client != null) {
			try {
				...
				if (doc.getDocument() != null) {
					...
					errorMsg = "Unable to download document: unknown document id.";
			} catch (IOException ioe) {
				errorMsg = "Unable to download document. Please see the log for details.";
			}
		} else
			errorMsg = "Unable to download document: missing Document Store.";
