...
public class DynFormComponentBuilder {
	...
	public SubPanel makeSubPanel(DynFormField field, SubPanelController spc) {
		...
		if (spc != null) {
			...
			spc = new SubPanelController(subPanel, field.getMinoccurs(), field.getMaxoccurs(), field.getLevel(),
					_factory.getFormBackgroundColour(), _factory.getFormAltBackgroundColour());
	...
	public DynFormComponentList makeInputField(DynFormField input) {
		...
		if (input.isEmptyComplexTypeFlag() || type.equals("boolean")) {
			...
			if (type.equals("date")) {
				...
				field = makeTextField(input);
		}
		fieldList.add(field);
		if (!(field instanceof DocComponent))
			label.setFor(field.getId());
		if (!focusSet)
			focusSet = setFocus(field);
	...
	private String makeStyle(UIComponent field, DynFormField input) {
		...
		// increment y-coord for each component's top (relative to current
		// panel)
	...
	public Checkbox makeCheckbox(DynFormField input) {
		...
		cbox.setSelected((input.getValue() != null) && input.getValue().equalsIgnoreCase("true"));
	...
	public Calendar makeCalendar(DynFormField input) {
		...
		cal.setSelectedDate(createDate(input.getValue(), -1)); // default to
																// today
	...
	private Date getAdjustedDate(int period, int adjustment, Date startDate) {
		...
		if (startDate != null)
			greg.setTimeInMillis(startDate.getTime());
	...
	private Date getMinDate(DynFormField input) {
		...
		if (restriction != null) {
			if (restriction.hasMinInclusive()) {
				minDate = createDate(restriction.getMinInclusive(), 1); // def.
																		// 1/1/70
	...
	private String getEnumeratedListValue(DynFormField input) {
		...
		if (StringUtil.isNullOrEmpty(value))
			value = null; // nullify empty strings
		return value;
	}
	...
	public TextField makeTextField(DynFormField input) {
		...
		textField.setText(input.hasBlackoutAttribute() ? "" : JDOMUtil.decodeEscapes(input.getValue()));
		if (textField.isVisible())
			setMaxTextValueWidth(input, (String) textField.getText(), 0);
		input.setRestrictionAttributes();
		return textField;
	}
	...
	public DocComponent makeDocumentField(DynFormField input, Label label) {
		...
		DocComponent docField = new DocComponent(id.getValue(), name.getValue(), uniqueID, textField, inputOnly);
		if (isVisible(input))
			setMaxTextValueWidth(input, (String) textField.getText(),
	...
	private ImageComponent makeImageComponent(String imagePath) {
		...
		image.setStyle(
				String.format("position: absolute; width: %dpx; height: %dpx; ", image.getWidth(), image.getHeight()));
	...
	public boolean setFocus(UIComponent component) {
		if (component instanceof PanelLayout)
			return false;
	...
	private Date createDate(String dateStr, long defaultDate) {
		// set the date to the param's input value if possible, else default to
		// a
		// date representation of the long defaultDate, or today if defaultDate
		// < 0
		Date result = null;
	...
	private void setMaxLabelWidth(DynFormField input) {
		_maxLabelWidth = Math.max(_maxLabelWidth, getTextWidth(input, input.getLabelText() + ":"));
	}
	...
	private int getTextWidth(DynFormField input, String text) {
		...
		if (font == null)
			font = _factory.getFormFonts().getFormFont();
