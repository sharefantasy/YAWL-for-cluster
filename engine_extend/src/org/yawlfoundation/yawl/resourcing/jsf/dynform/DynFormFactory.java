...
public class DynFormFactory extends AbstractSessionBean implements DynamicForm {
	...
	// components & settings of the dynamic form that are managed by this
	// factory object
	...
	public String getBtnCompleteStyle() {
		return btnCompleteStyle;
	}

	public void setBtnCompleteStyle(String style) {
		btnCompleteStyle = style;
	}
	...
	private Map<String, SubPanelController> _subPanelTable = new HashMap<String, SubPanelController>();

	// a map of inputs to the textfields they generated (required for
	// validation)
	private Map<UIComponent, DynFormField> _componentFieldTable;

	// a map of the non-SubPanel components of the outermost panel and their
	// y-coords
	...
	static final int Y_CHOICE_DECREMENT = 20; // dec of y coord for choice
												// container top
	static final int SUBPANEL_INSET = 10; // gap between panel side walls
	static final int OUTER_PANEL_TO_BUTTONS = 20; // gap from panel bottom to
													// buttons
	 ...
	 *            The form's title
	 * @param header
	 *            A header text for the form top
	 * @param schema
	 *            An XSD schema of the data types and attributes to display
	 * @param wir
	 ...
	 *            An XSD schema of the data types and attributes to display
	 * @param parameters
	 *            a list of the root net's input parameters
	 * @return true if form creation is successful
	 */
	public boolean makeForm(String title, String header, String schema, List<YParameter> parameters) {
		reset();
		return buildForm(title, header, schema, null, getCaseParamMap(parameters));
	}

	/**
	 * Gets the form's data list on completion of the form. The data list must
	 * be a well-formed XML string representing the expected data structure for
	 * the work item or case start. The opening and closing tag must be the name
	 * of task of which the work item is an instance, or of the root net name of
	 * the case instance.
	 *
	...
	protected String getWorkItemData() {
		return getWorkItemData(_displayedWIR);
	}

	private String getWorkItemData(WorkItemRecord wir) {
		if (wir == null)
			return null;
		Element data = wir.getUpdatedData() != null ? wir.getUpdatedData() : wir.getDataList();
	...
	private void setHeader(String header) {
		...
		if (getAttributes() != null) {
			if (header == null)
				header = getAttributeValue("title");
			if (header == null)
				header = "Edit Work Item: " + _displayedWIR.getCaseID();
		}
	...
	private void buildForm() {
		...
		DynAttributeFactory.adjustFields(fieldList, _displayedWIR, _sb.getParticipant()); // 1st
																							// pass
		...
		DynAttributeFactory.applyAttributes(compPanel, _displayedWIR, _sb.getParticipant()); // 2nd
																								// pass
	...
	private void setBaseWidths(DynFormComponentBuilder builder) {
		...
		// find the left offset to position fields by getting the composite
		// width of
		// longest label + offset of label start from panel side + the default
		// gap
		...
		FIELD_WIDTH = builder.hasOnlyCheckboxes()
				? CHECKBOX_FIELD_WIDTH
				: Math.max(buttonBlockWidth - X_FIELD_OFFSET, getMaxFieldWidth(builder));

		// set the width of the innermost panel content (not including panel
		// insets from
	...
	private void adjustFieldOffsetsAndWidths(List content) {
		...
		for (Object o : content) {
			if (o instanceof SubPanel) {
				...
				field.setStyle(String.format(template, field.getStyle(), X_FIELD_OFFSET, FIELD_WIDTH));
			} else if (o instanceof TextArea) {
				TextArea field = (TextArea) o;
				field.setStyle(String.format(template, field.getStyle(), X_FIELD_OFFSET, FIELD_WIDTH + 6));
			} else if (o instanceof DropDown) {
				DropDown field = (DropDown) o;
				field.setStyle(String.format(template, field.getStyle(), X_FIELD_OFFSET, FIELD_WIDTH + 8));
			} else if (o instanceof Calendar) {
				Calendar field = (Calendar) o;
				field.setStyle(String.format("%s; left:%dpx;", field.getStyle(), X_FIELD_OFFSET));
			} else if (o instanceof Checkbox) {
				Checkbox field = (Checkbox) o;
				field.setStyle(String.format("%s; left:%dpx;", field.getStyle(), X_FIELD_OFFSET));
			} else if (o instanceof FlatPanel) {
				FlatPanel field = (FlatPanel) o;
				field.setStyle(String.format("%s; width:%dpx;", field.getStyle(), getFormWidth() - 20));
	...
	private double getComponentHeight(UIComponent component) {
		...
		if (component instanceof SubPanel) {
			...
			height = getFieldHeight(component) * ((TextArea) component).getRows() + FIELD_VSPACE;
		} else {
	...
	private void sizeAndPositionContent(PanelLayout panel) {
		...
		// set the size and position of inner panels relative to their nested
		// level
		...
		// calc and set height (adjusted for field count), width & top of
		// outermost panel
	...
	private void setPanelStyles(int containerWidth, int height, int top) {
		...
		String outerPanelStyle = String.format("position: absolute; height: %dpx; width: %dpx; top: %dpx", height,
				outerPanelWidth, top);
		...
		containerStyle = String.format("position: relative; height: %dpx; top: 0; width: %dpx",
				top + height + OUTER_PANEL_TO_BUTTONS + FORM_BUTTON_HEIGHT + BOTTOM_PANEL_HEIGHT, containerWidth);
	}
	...
	private DynFormComponentList buildInnerForm(SubPanel container, DynFormComponentBuilder builder,
		...
		for (DynFormField field : fieldList) {
			...
			} else if (!field.isEmptyOptionalInputOnly()) { // create the field
															// (inside a panel)

				// if min and/or max defined at the field level, enclose it in a
				// subpanel
	...
	private DynFormComponentList buildChoiceSelector(SubPanel container, DynFormComponentBuilder builder,
			DynFormField field) {
		DynFormComponentList compList = new DynFormComponentList();

		RadioButton rButton = builder.makeRadioButton(field);
		rButton.setSelected((container == null) || isFirstRadioGroupMember(container.getChildren(), rButton.getName()));

		compList.add(rButton);
		return compList;
	}

	private DynFormComponentList buildSubPanel(DynFormComponentBuilder builder, DynFormField field) {
	...
	private MethodBinding bindOccursButtonListener() {
		...
		return app.createMethodBinding("#{dynForm.btnOccursAction}", new Class[]{ActionEvent.class});
	}
	...
	private String createUniqueID(String id) {
		return IdGenerator.uniquify(id);
	}
	...
	private String replaceInternalChars(String text, char pre, char post) {
		if ((text == null) || (text.length() < 3) || (text.indexOf(pre) < 0))
			return text;
	...
	private String getFormFontStyle() {
		return (getUserAttributes() == null) ? null : getUserAttributes().getUserDefinedFontStyle();
	}

	private Font getFormFont() {
		return (getUserAttributes() == null) ? null : getUserAttributes().getUserDefinedFont();
	}

	private Font getFormHeaderFont() {
		return (getUserAttributes() == null) ? null : getUserAttributes().getFormHeaderFont();
	}
	...
	protected String getFormJustify() {
		return (getUserAttributes() == null) ? null : getUserAttributes().getTextJustify();
	}
	...
	private String getAttributeValue(String key) {
		return (getUserAttributes() == null) ? null : getUserAttributes().getValue(key);
	}
