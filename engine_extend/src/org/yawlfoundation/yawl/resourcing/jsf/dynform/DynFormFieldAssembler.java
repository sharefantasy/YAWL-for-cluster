...
public class DynFormFieldAssembler {
	...
	public DynFormFieldAssembler(String schemaStr, String dataStr, Map<String, FormParameter> params)
	...
	private List<DynFormField> createFieldList(Element schema, Element data, Namespace ns, int level)
			throws DynFormException {
		...
		if (schema.getName().equals("choice")) {
			...
			if (complex == null) {
				throw new DynFormException("Malformed data schema, at element: " + JDOMUtil.elementToString(schema));
			}

			next = complex.getChild("sequence", ns);
			if (next == null)
				next = complex.getChild("all", ns);
			if (next != null) {
	...
	private List<DynFormField> createSequence(Element sequence, Element data, Namespace ns, int level)
			throws DynFormException {
	...
	private List<DynFormField> createChoice(Element parent, Element data, Namespace ns, int level)
			throws DynFormException {
		List<DynFormField> fieldList = new ArrayList<DynFormField>();
	...
	private DynFormField addGroupField(String name, Element eField, Namespace ns, Element data, String minOccurs,
			String maxOccurs, int level) throws DynFormException {
		DynFormField field;
		List<DynFormField> simpleContents = new ArrayList<DynFormField>();
		int instances = getInitialInstanceCount(minOccurs, data, name);

		if (instances == 1) {
			field = addField(name, createFieldList(eField, data, ns, level), minOccurs, maxOccurs, level);
		} else {
			...
			for (int i = 0; i < instances; i++) {
				...
				List<DynFormField> subFieldList = createFieldList(eField, data4Inst, ns, level);
				DynFormField subField = subFieldList.get(0); // the multi-inst
																// field
	...
	private List<DynFormField> addElementField(String name, String type, Element data, String minOccurs,
	...
	private DynFormField addField(String name, List<DynFormField> subFieldList, String minOccurs, String maxOccurs,
			int level) {
		if (name == null)
			name = "choice";
	...
	private void populateField(DynFormField input, String name, String minOccurs, String maxOccurs, int level) {
	...
	private DynFormField addContainingField(String name, String minOccurs, String maxOccurs, int level) {
		if (name == null)
			name = "choice";
	...
	private void applySimpleTypeFacets(Element simple, Namespace ns, DynFormField field) {
		Element restriction = simple.getChild("restriction", ns);
		if (restriction != null) {
			DynFormFieldRestriction restrict = new DynFormFieldRestriction(restriction, ns);
	...
	private List<DynFormField> addSimpleContents(List<DynFormField> simpleContents, List<DynFormField> subFieldList) {
