...
/**
 * Author: Michael Adams Creation Date: 15/08/2008
 */
public class DynFormFieldRestriction {
	...
	public String getToolTipExtn() {
		...
		if (hasMinLength() && hasMaxLength()) {
			if (getMinLengthValue() == getMaxLengthValue())
				msgList.add(String.format(" with a length of exactly %s characters", _minLength));
			else
				msgList.add(String.format(" with a length between %s and %s characters", _minLength, _maxLength));
		...
		if (hasTotalDigits()) {
			msgList.add(String.format(" having no more than %s digits (excluding the decimal point)", _totalDigits));
		}
		if (hasFractionDigits()) {
			msgList.add(
					String.format(" having no more than %s digits to the right of the decimal point", _fractionDigits));
		}
	...
	private String getSchemaLeadIn(String prefix) {
		return String.format("<%s:simpleType><%s:restriction base=\"%s:%s\">", prefix, prefix, prefix,
				getBaseTypeUnprefixed());
	}
	...
	public boolean hasMinInclusive() {
		return _minInclusive != null;
	}
	public boolean hasMinExclusive() {
		return _minExclusive != null;
	}
	public boolean hasMaxInclusive() {
		return _maxInclusive != null;
	}
	public boolean hasMaxExclusive() {
		return _maxExclusive != null;
	}
	public boolean hasTotalDigits() {
		return _totalDigits != null;
	}
	public boolean hasFractionDigits() {
		return _fractionDigits != null;
	}
