...
/**
 * This class manages the set of user-defined (at design-time) extended
 * attributes of a workitem, some of which will affect the way the dynamic form
 * is displayed.
 *
 * Author: Michael Adams Date: 24/02/2009
 */
public class DynFormUserAttributes {
	...
	public String getUserDefinedFontStyle(boolean header) {
		...
		if (fontFamily != null)
			style += String.format(";font-family: %s", fontFamily);
		String fontSize = getValue(head + "font-size");
		if (fontSize != null)
			style += String.format(";font-size: %spx", fontSize);
	...
	private boolean hasFontAttributes(boolean header) {
		String head = header ? "header-" : "";
		return !((getValue(head + "font-family") == null) && (getValue(head + "font-size") == null)
