...
/**
 * Author: Michael Adams Creation Date: 7/08/2008
 */
...
public class DynFormValidator {
	...
	public boolean validate(PanelLayout panel, Map<UIComponent, DynFormField> componentFieldLookup,
	...
	private boolean validateInputs(PanelLayout panel) {
		...
		if (components != null) {
			...
			// checkboxes & dropdowns are self validating - only need to do
			// textfields
			// & calendars (calendars only for non-empty required values)
			for (Object o : components) {
				UIComponent component = (UIComponent) o;

				// if a choice is involved, the unselected field(s) must be
				// ignored
	...
	private boolean validateExpiry(DynFormField input, String value) {
		try {
			DatatypeFactory.newInstance().newDuration(value); // try duration
																// 1st
			return true;
		} catch (Exception e) {
	...
	private boolean validateBase(String fieldName, String type, String text, boolean untreated) {
		boolean result = true;
	...
	private void addErrorMessage(DynFormField input, String value) {
		...
		if (msg == null) {
			String base = "The value '%s' is not valid for field '%s'. " + "This field requires a value of %s type";
	...
	private void addValidationErrorMessage(String value, String fieldName, String type, boolean untreated) {
		if (_msgPanel == null)
			return;
		String name = untreated ? normaliseFieldName(fieldName) : fieldName;
		String msg = String.format("Invalid value '%s' in field '%s', expecting a valid %s value", value, name, type);
		_msgPanel.error(msg);
	}
	...
	private String buildInputSchema(DynFormField input) {
		...
		schema.append("<").append(NS_PREFIX).append(":element name=\"").append(input.getName()).append("\"");

		if (input.hasUnion()) {
			schema.append(">").append(input.getUnion().getBaseElement()).append("</xsd:element>");
		} else if (input.hasRestriction()) {
			schema.append(">").append(input.getRestriction().getBaseElement()).append("</xsd:element>");
		} else {
			schema.append(" type=\"xsd:").append(input.getDataTypeUnprefixed()).append("\"/>");
		}
