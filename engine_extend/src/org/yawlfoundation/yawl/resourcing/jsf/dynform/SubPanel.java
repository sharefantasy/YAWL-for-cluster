 ...
 *
 * @author Michael Adams Date: 26/02/2008
 */
...
public class SubPanel extends PanelLayout implements Cloneable {
	...
	// mapping of the tops (y-coords) of the non-panel component members of this
	// subpanel
	...
	/**
	 * Calculates the screen co-ords for this panel (relative to the entire
	 * form)
	 * 
	 * @param maxLevel
	 *            the highest depth level of all panels on this form
	 */
	public void assignStyle(int maxLevel) {

		// resize width for the nested depth of this panel
		width = DynFormFactory.PANEL_BASE_WIDTH
				+ (DynFormFactory.SUBPANEL_INSET * 2 * (maxLevel - controller.getDepthlevel() + 1));

		String style = String.format("position: absolute; top: %dpx; left: %dpx; height: %dpx; width: %dpx", top,
				DynFormFactory.SUBPANEL_INSET, (int) height, width);

		// set user-defined background colour (if given)
		String backColour = controller.getUserDefinedBackgroundColour();
		if (backColour != null)
			style += "; background-color: " + backColour;
		...
		if (btnPlus != null) {
			btnPlus.setStyle(String.format("left: %dpx", width - 21)); // 20 =
																		// btn
																		// width
	...
	public void enableOccursButtons(boolean enable) {
		if (btnPlus != null)
			btnPlus.setDisabled(!enable);
		if (btnMinus != null)
			btnMinus.setDisabled(!enable);
	}
	 ...
	 * 
	 * @param top
	 *            the y-coord below which components should be moved down
	 * @param adjustment
