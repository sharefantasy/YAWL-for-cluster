 ...
 *
 * Author: Michael Adams Date: 21/02/2008
 */
...
public class SubPanelController {
	...
	public SubPanelController(SubPanel panel, long minOccurs, long maxOccurs, int level, String udBgColour,
	...
	/**
	 * A recursive method that resizes and repositions all the containing
	 * subpanels of the one passed.
	 * 
	 * @param subPanel
	 *            the subpanel that has instigated the change
	 * @param adjustment
	 *            how much to increase panel heights & tops by
	 * @param top
	...
	private SubPanel repositionAncestor(SubPanel subPanel, int adjustment, int top) {
		...
		if (parentPanel instanceof SubPanel) {
			...
			return subPanel; // the level 0 'patriarch'
	}

	/**
	 * Increment the relevant subpanel tops for all subpanels at the same depth
	 * level that are not ancestors of the subpanel passed
	 * 
	 * @param ancestor
	 *            an outer subpanel of the one that triggered the repositioning
	 * @param controller
	 *            the controller of the triggering subpanel
	 * @param top
	 *            the new top position of the triggering subpanel
	 * @param adjustment
	 *            how much to add to the top of affected sibling subpanels
	 */
	private void incSiblingSubPanelTops(SubPanel ancestor, SubPanelController controller, int top, int adjustment) {
		...
		for (Object component : components) {
			if (component instanceof SubPanel) {
				...
				if (!controller.hasPanel(subPanel)) {
					if (subPanel.getTop() > top)
						subPanel.incTop(adjustment);
				}
			}
		}
	}
	 ...
	 * 
	 * @param top
	 *            the y-coord below which subpanels should be moved down
	 * @param adjustment
	 *            how much to move them down by
	 */
