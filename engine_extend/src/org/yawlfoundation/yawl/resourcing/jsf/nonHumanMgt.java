...
public class nonHumanMgt extends AbstractPageBean {
	...
	public void setTabResources(Tab t) {
		tabResources = t;
	}
	...
	public void setLpResources(PanelLayout pl) {
		lpResources = pl;
	}
	...
	public void setTabCategories(Tab t) {
		tabCategories = t;
	}
	...
	public void setLpCategories(PanelLayout pl) {
		lpCategories = pl;
	}
	...
	public void prerender() {
		...
		if (tabSet.getSelected() == null) { // first rendering
			...
			if (!_sb.getActiveTab().equals(tabSet.getSelected())) { // tab
																	// change
				nullifyChoices();
				setMode(SessionBean.Mode.edit);
			...
			if (getSelTypeForActiveTab() == SelType.resource) {
				if (!_sb.hasAtLeastOneNonHumanCategory()) {
					_msgPanel.warn("Cannot add a new non-human resource yet, because "
							...
							+ "add at least one category first (on the 'Categories' tab), " + "then try again.");
					return null;
				}
		...
		if (id != null) {
			try {
				switch (getSelTypeForActiveTab()) {
					case resource :
						_orgDataSet.removeNonHumanResource(id);
						break;
					case category :
						_orgDataSet.removeNonHumanCategory(id);
						break;
				}
	...
	private void showButtons() {
		boolean noSelectedListItem = (_sb.getNhResourcesOptions().length == 0) || (_sb.getNhResourcesChoice() == null);
