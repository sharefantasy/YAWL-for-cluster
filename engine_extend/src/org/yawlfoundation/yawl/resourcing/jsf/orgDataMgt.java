...
public class orgDataMgt extends AbstractPageBean {
	...
	public void setBtnExport(Button btn) {
		btnExport = btn;
	}
	...
	public void setBtnImport(Button btn) {
		btnImport = btn;
	}
	...
	public void setBtnCancelUpload(Button b) {
		btnCancelUpload = b;
	}
	...
	public void setPnlUpload(PanelLayout pnl) {
		pnlUpload = pnl;
	}
	...
	public enum AttribType {
		role, position, capability, orggroup
	}
	...
	public String btnExport_action() {
		...
		try {
			if (result != null) {
				...
				response.setHeader("Content-Disposition", "attachment;filename=\"YAWLOrgDataExport.ybkp\"");
	...
	public String btnUpload_action() {
		...
		if (fileName.length() > 0) {
			if (fileName.endsWith(".ybkp")) {
				...
				msgPanel.error("Only exported YAWL Org Data files with an extension of '.ybkp' may be uploaded.");
		}
	...
	public String btnRemove_action() {
		...
		if (id != null) {
			...
			try {
				switch (type) {
					case role :
						orgDataSet.removeRole(id);
						break;
					case capability :
						orgDataSet.removeCapability(id);
						break;
					case position :
						orgDataSet.removePosition(id);
						break;
					case orggroup :
						orgDataSet.removeOrgGroup(id);
				}
	...
	private int populateForm(AttribType aType) {
		...
		if (aType != null) {
			...
			if ((attribs != null) && (attribs.length > 0)) {
				String id = _sb.getOrgDataChoice();
				if (id == null)
					id = (String) attribs[0].getValue();
				showItem(id, aType);
				result = attribs.length;
			} else
				innerForm.clearFields();
		}
		return result;
	}
	...
	private AttribType getAttribType(String tabName) {
		if (tabName != null) {
			...
			if (tabName.equals("tabCapability"))
				return AttribType.capability;
			if (tabName.equals("tabPosition"))
				return AttribType.position;
			if (tabName.equals("tabOrgGroup"))
	...
	private void showMessagePanel() {
		body1.setFocus(msgPanel.hasMessage() ? "form1:pfMsgPanel:btnOK001" : "form1:pfOrgData:txtName");
