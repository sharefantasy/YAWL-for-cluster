...
public class pfMenubar extends AbstractFragmentBean {
	...
	public void construct(boolean isAdminID) {
		...
		if (adminMenu != null) {
			...
			menuCount++;
			if (userMenu != null)
				userMenu.setStyle(userMenu.getStyle() + "left: 285px;");
		}
	...
	private PanelLayout constructAdminMenu(Participant p) {
		...
		if ((p == null) || p.isAdministrator()) {
			...
			int top = (p != null) ? 40 : 0;
			adminMenu.setStyle(String.format("width: %dpx; top: %dpx; position: absolute;", width, top));
		}
		return adminMenu;
	}
	...
	private String getButtonText(String btnType) {
		if (btnType.equals(BTN_WORKQUEUES))
			return "Work Queues";
		if (btnType.equals(BTN_ADMINQUEUES))
			return "Admin Queues";
		if (btnType.equals(BTN_CASEMGT))
			return "Cases";
		if (btnType.equals(BTN_USERMGT))
			return "Users";
		if (btnType.equals(BTN_ORGDATAMGT))
			return "Org Data";
		if (btnType.equals(BTN_LOGOUT))
			return "Logout";
		if (btnType.equals(BTN_NHRESOURCES))
			return "Assets";
		if (btnType.equals(BTN_CALENDAR))
			return "Calendar";
		if (btnType.equals(BTN_PROFILE))
			return "Edit Profile";
		if (btnType.equals(BTN_SERVICEMGT))
			return "Services";
		if (btnType.equals(BTN_EXTCLIENTS))
			return "Client Apps";
		if (btnType.equals(BTN_TEAMQUEUES))
			return "Team Queues";
		return "";
	}
	...
	private boolean canViewTeamQueues(Participant p) {
		return p.getUserPrivileges().canViewOrgGroupItems() || p.getUserPrivileges().canViewTeamItems();
	}
