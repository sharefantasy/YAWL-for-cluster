...
public class pfNHResources extends AbstractFragmentBean {
	...
	public String btnRemoveSubCat_action() {
		...
		if (category != null) {
			if (category.removeSubCategory(_sb.getNhResourcesSubcategoryChoice())) {
				...
				_msgPanel.error("Failed to remove subcategory.");
		}
		return null;
	}
		...
		if ((name != null) && (name.length() > 0)) {
			...
			if (category != null) {
				if (category.addSubCategory(name)) { // won't add if duplicate
					...
					_msgPanel.error("Subcategory already exists: " + name);
			}
		} else
			_msgPanel.error("Please enter a subcategory to add.");
		return null;
	}

	/*
	 * reset mode to browse/edit for subcategories - triggered by categories tab
	 * only
	 */
	...
	private void populateGUI(NonHumanResource resource, String id) {
		...
		if (resource != null) {
			...
			if (category != null) {
				...
				_sb.setNhResourcesSubcategoryItems(null); // no categories
															// defined
		}
		_sb.setNhResourcesSubcategoryList(null); // empty the category tab list
													// box
	}
	...
	private void populateGUI(NonHumanCategory category) {
		...
		_sb.setNhResourcesSubcategoryItems(null); // empty the resource tab
													// combo box
	}
	...
	public void setVisibleComponents(nonHumanMgt.SelType sType) {
		...
		if (catTab) {
			...
			lbxSubCatItems.setItems(null);
	}

	/*
	 * enable or disable fields depending on whether we are in browse/edit or
	 * add mode
	 */
	...
	public boolean addResource(String name) {
		if (!_orgDataSet.isKnownNonHumanResourceName(name)) {
			...
			if (_rm.successful(newID)) {
				...
				_msgPanel.error(_msgPanel.format(newID));
		} else
			addDuplicationError("Resource");
	...
	/*
	 * enables or disables fields depending on whether a sub category is being
	 * added
	 */
