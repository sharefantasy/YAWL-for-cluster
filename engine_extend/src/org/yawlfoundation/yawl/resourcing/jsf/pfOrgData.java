...
public class pfOrgData extends AbstractFragmentBean {
	...
	public boolean saveChanges(String id) {
		...
		if ((attrib != null) && isValidNameChange(attrib)) {
			...
			if (hasCyclicReferences(attrib, belongsToID))
				return false;
	...
	private boolean isValidNameChange(AbstractResourceAttribute attrib) {
		String name = toUTF8((String) txtName.getText());
		if (attrib instanceof Capability) {
			if (!name.equals(((Capability) attrib).getCapability())) { // if
																		// name
																		// changed
	...
	/*
	 * This method is required to workaround a bug in JSF 1.2 page fragments,
	 * where the encoding for the fragment can't be changed from the default
	 * ISO-8859-1 when the parent form contains a file upload component
	 */
