...
public class teamQueues extends AbstractPageBean {
	...
	public void setLpQueues(PanelLayout pl) {
		lpQueues = pl;
	}
	...
	public void setRbGroup(PanelLayout pl) {
		rbGroup = pl;
	}
	...
	public void setRbTeam(RadioButton rb) {
		rbTeam = rb;
	}
	...
	public void setRbOrgGroup(RadioButton rb) {
		rbOrgGroup = rb;
	}
	...
	private WorkItemRecord buildMembersList(qSet setType, String selectedID) {
		...
		if (!underlings.isEmpty()) {
			...
			for (WorkItemRecord wir : wirSet) {
				if (i == 0)
					result = wir; // return first wir by default
		...
		if (result != null)
			_sb.populateAdminQueueAssignedList(result);
		return result;
	}
