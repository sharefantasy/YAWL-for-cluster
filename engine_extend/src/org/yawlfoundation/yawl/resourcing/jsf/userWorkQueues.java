...
public class userWorkQueues extends AbstractPageBean {
	...
	public Button getBtnNewInstance() {
		return btnNewInstance;
	}

	public void setBtnNewInstance(Button b) {
		btnNewInstance = b;
	}
	...
	public Button getBtnAcceptStart() {
		return btnAcceptStart;
	}

	public void setBtnAcceptStart(Button b) {
		btnAcceptStart = b;
	}
	...
	public void setBtnVisualiser(Button btn) {
		btnVisualiser = btn;
	}
	...
	public void setBtnException(Button btn) {
		btnException = btn;
	}
	...
	public void setBtnCost(Button btn) {
		btnCost = btn;
	}
	...
	public void setBtnExceptionStyle(String style) {
		btnExceptionStyle = style;
	}
	...
	public void setBtnCostStyle(String style) {
		btnCostStyle = style;
	}
	...
	public void prerender() {
		...
		if (_sb.isDelegating())
			postDelegate();
		else if (_sb.isReallocating())
			postReallocate();
		else if (_sb.isCustomFormPost() || _sb.isWirEdit())
			postFormDisplay();
		else if (_sb.isAddInstance())
			postAddInstance();
	...
	private Tab initDefaultTab() {
		...
		if (wir != null)
			((pfQueueUI) getBean("pfQueueUI")).populateTextBoxes(wir);
	...
	public String btnView_action() {
		...
		if (wir == null) {
			msgPanel.error("Cannot view item contents - it appears that the "
					+ "selected item may have been removed or cancelled. " + "Please see the log files for details.");
			return null;
		}
	...
	public String btnException_action() {
		...
		if (activeQueue != WorkQueue.UNDEFINED) {
			...
			msgPanel.error("No work item selected to raise exception against.");
	 ...
	 *
	 * @param stateful
	 *            - if true, reallocate with state preserved
	...
	private String buildSubordinatesList(String action) {
		...
		// participant can only delegate/reallocate to those reporting to he or
		// she
		...
		if (underlings != null) {
			...
			msgPanel.warn(String.format("There are no participants that you are authorised to %s to.", action));
		}
		return result;
	}
	...
	private String doAction(int queueType, String action) {
		...
		try {
			if (action.equals("deallocate")) {
				...
				if (isEngineSuspended(wir, "skipped"))
					return null;
				_rm.skipWorkItem(p, wir);
			} else if (action.equals("start")) {
				if (isEngineSuspended(wir, "started"))
					return null;
				if (!_rm.start(p, wir)) {
					msgPanel.error(
				...
				if (isEngineSuspended(wir, "suspended"))
					return null;
				...
				if (isEngineSuspended(wir, "piled"))
					return null;
				...
				if (isEngineSuspended(wir, "chained"))
					return null;
				...
				if ((!getApplicationBean().isEmptyWorkItem(wir)) && (wir.getUpdatedData() == null)
						&& (!_sb.hasWarnedForNonEdit(wir.getID()))) {
					msgPanel.info("Warning: This item has not been edited. If you are "
							+ "sure you want to complete without editing, click " + "the 'Complete' button again.");
	...
	private void postDelegate() {
		if (_sb.isDelegating()) {
			Participant pFrom = _sb.getParticipant();
			String pIDTo = (String) _sb.getSelectUserListBoxSelections(); // this
																			// is
																			// the
																			// p-id
	...
	private void postAddInstance() {
		...
		if (paramValue != null) {
			...
			WorkItemRecord newWir = _rm.createNewWorkItemInstance(_sb.getAddInstanceItemID(), data);
			if (newWir != null) {
				...
				msgPanel.error(
						"Create Instance Error: " + "Engine failed to create new instance. See logs for details.");
		} else
			msgPanel.error("Create Instance Error: " + "Problem reading parameter value from New Instance form.");
	...
	private void updateTabHeaders(Tab selected) {
		...
		if (selected != null)
			selected.setStyle("color: #3277ba");
	 ...
	 *
	 * @param rate
	 *            if <0, disables page refreshes; if >0, set refresh rate to
	 *            that number of seconds; if 0, set the rate to the default
	 *            provided by the resourceService's web.xml
	 */
	public void setRefreshRate(int rate) {
		if (rate < 0)
			...
			if (rate == 0)
				rate = getApplicationBean().getDefaultJSFRefreshRate();
			metaRefresh.setContent(rate + "; url=./userWorkQueues.jsp");
		}
	}

	/**
	 * Populates the specified queue's workitem list and shows the selected
	 * items details
	 *
	 * @param queueType
	...
	private int populateQueue(int queueType) {
		...
		if (queueHasItems) {
			...
			if (!(_sb.isDelegating() || _sb.isReallocating() || _sb.isCustomFormPost() || _sb.isWirEdit()))
	 ...
	 */
	private void addItemsToListOptions(Set<WorkItemRecord> queue, WorkItemRecord selected) {
		Option[] options = new Option[queue.size()];
		WorkItemRecord first = null;
		boolean listContainsSelected = false;
		SortedSet<WorkItemRecord> qSorted = new TreeSet<WorkItemRecord>(new WorkItemAgeComparator());
		qSorted.addAll(queue);
		int i = 0;
		for (WorkItemRecord wir : qSorted) {
			if (wir != null) {
				if (i == 0)
					first = wir; // get first non-null item
	...
	private boolean isEngineSuspended(WorkItemRecord wir, String action) {
		...
		if (suspended) {
			msgPanel.warn("The selected work item cannot be " + action + " because it has been suspended by the engine."
	...
	/**
	 * Enable/disable buttons depending on user/task privileges Pre: wir != null
	 */
	private void processTaskPrivileges(WorkItemRecord wir, int qType) {
		...
		if (qType == WorkQueue.OFFERED) {
			...
			btnDeallocate.setDisabled(!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_DEALLOCATE));
			btnDelegate.setDisabled(!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_DELEGATE));
			btnSkip.setDisabled((!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_SKIP)) || suspended);
			btnPile.setDisabled((!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_PILE)) || suspended);
		} else if (qType == WorkQueue.STARTED) {
			btnSuspend.setDisabled((!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_SUSPEND)) || suspended);
			btnStateful.setDisabled(!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_REALLOCATE_STATEFUL));
			btnStateless.setDisabled(!_rm.hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_REALLOCATE_STATELESS));
			...
			// set 'New Instance' button (not a task priv but convenient to do
			// it here)
	...
	private boolean validateSavedData(WorkItemRecord wir) {
		...
		return data != null && _rm.checkWorkItemDataAgainstSchema(wir, data).startsWith("<success");
	}
		...
		if (!p.isAdministrator()) {
			...
			if ((queue == WorkQueue.ALLOCATED) && (_sb.getQueueSize(WorkQueue.ALLOCATED) > 0)) {
				...
				if (!btnStart.isDisabled()
						&& (!(p.getUserPrivileges().canChooseItemToStart() || p.getUserPrivileges().canReorder()))) {
	...
	private boolean isStartDisabled(Participant p) {
		return (!p.getUserPrivileges().canStartConcurrent())
				&& ((_sb.getQueueSize(WorkQueue.STARTED) > 0) || (_sb.getQueueSize(WorkQueue.SUSPENDED) > 0));
	}
	...
	private void showCostInfo() {
		...
		WorkItemRecord wir = _sb.getChosenWIR(activeQueue);
		if (wir != null) {
			System.out.println(LogMiner.getInstance().getTaskEvents(new YSpecificationID(wir), wir.getTaskName()));
	...
	private void showMessagePanel() {
		body1.setFocus(msgPanel.hasMessage() ? "form1:pfMsgPanel:btnOK001" : "form1:pfQueueUI:lbxItems");
