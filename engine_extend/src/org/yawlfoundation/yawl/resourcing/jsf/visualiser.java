...
public class visualiser extends AbstractPageBean {
	...
	public String btnView_action() {
		...
		// maybe the wir isn't started or was part of a cancellation set and now
		// it's gone
		if (selectedWIR == null) {
		...
		if (df.makeForm(title, header, _sb.getTaskSchema(selectedWIR), selectedWIR)) {
			...
			msgPanel.error("Cannot view item contents - problem initialising "
					+ "dynamic form from task specification. " + "Please see the log files for details.");
			return null;
		}
	}
	...
	private void postEditWIR() {
		if (_sb.isWirEdit()) {
			...
			if (wir != null) {
				Element data = JDOMUtil.stringToElement(((DynFormFactory) getBean("DynFormFactory")).getDataList());
	...
	// it was necessary to do it this way rather than using 'jsp:cluster' in the
	// jsf because the 'codebase' parameter does not support dynamic value
	// setting
	public String getAppletHtml() {
		...
		result.append(view.width).append("\" height=\"").append(view.height).append("\"")
				...
				.append(" codebase=\"../../visualiserApplet\"").append(" code=\"worklist.WRKLApplet.class\" MAYSCRIPT>")
				.append(" <param name=\"user\" value=\"").append(p.getUserID()).append("\"/>")
				.append(" <param name=\"pass\" value=\"").append(p.getPassword()).append("\"/>")
				.append(" <param name=\"urYAWL\" value=\"").append(baseURI).append("\"/>").append("</applet>");
