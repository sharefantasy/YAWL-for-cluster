...
public class SecondaryResources {
	...
	private void announceUnavailable(AbstractResource resource, WorkItemRecord wir) {
		ResourceManager.getInstance().getClients().announceResourceUnavailable(resource, wir, false);
		_log.warn("Secondary Resource '" + resource.getName() + "' unavailable for work item: " + wir.getID());
	}

	private void announceUnavailable(String name, WorkItemRecord wir) {
		ResourceManager.getInstance().getClients().announceResourceUnavailable(null, wir, false);
		_log.warn("There are no available members of " + name + "' to allocate as a secondary resource for work item: "
				+ wir.getID());
	}
	...
	public class SecResDataSet {
		...
		private final Set<NonHumanResource> nonHumanResources; // unique
																// resource
		...
		public void addParticipant(String id) {
			...
			if (p != null)
				...
				_log.warn("Unknown Participant ID in secondary resources spec: " + id);
		}
		...
		public void addParticipant(Participant p) {
			if (_rm.getOrgDataSet().isKnownParticipant(p))
				...
				_log.warn("Could not add unknown Participant to secondary resources: " + p.getID());
		}
		...
		public void addRole(String id) {
			...
			if (role != null)
				...
				_log.warn("Unknown Role ID in secondary resources spec: " + id);
		}
		...
		public void addRole(Role role) {
			if (_rm.getOrgDataSet().isKnownRole(role))
				...
				_log.warn("Could not add unknown Role to secondary resources: " + role.getID());
		}
		...
		public void addNonHumanResource(String id) {
			...
			if (resource != null)
				...
				_log.warn("Unknown nonhuman resource ID in secondary resources spec: " + id);
		}
		...
		public void addNonHumanResource(NonHumanResource resource) {
			if (_rm.getOrgDataSet().isKnownNonHumanResource(resource))
				...
				_log.warn("Could not add unknown nonhuman resource to secondary resources: " + resource.getID());
		}
		...
		public boolean remove(String id) {
			...
			if (resource != null) {
				if (resource instanceof Participant)
					return participants.remove(resource);
				else
					return nonHumanResources.remove(resource);
			}
		...
		public int getResourcesCount() {
			return participants.size() + roles.size() + nonHumanResources.size() + getCategoryCount();
		}
		...
		public List<String> checkAvailability() {
			...
			if (hasResources()) {
				for (Participant p : participants) {
					if (!isAvailable(p))
						problems.add(problemMsg(p.getName()));
				}
				...
				for (Role r : roles) {
					...
					if (selection == null)
						problems.add(problemMsg("Role ", r.getName(), isMultiSelection(r, roles)));
					else
						taken.add(selection);
				}
				for (NonHumanResource r : nonHumanResources) {
					if (!isAvailable(r))
						problems.add(problemMsg(r.getName()));
				}
				taken.clear();
			...
			if (hasResources()) {
				...
				for (Role r : roles) {
					...
					if (selection != null)
						...
						announceUnavailable("role '" + r.getName(), wir);
				}
				for (NonHumanResource r : nonHumanResources) {
					if (isAvailable(r))
						selected.add(r);
					else
						announceUnavailable(r, wir);
				}
				for (String id : nonHumanCategories.keySet()) {
					CategoryStack c = nonHumanCategories.get(id);
					String subcat = getSubcatFromID(id);
					...
					if (selection != null)
						...
						announceUnavailable("nonhuman resource category '" + c.get().getName(), wir);
				}
			}
			return selected;
		}
		...
		private boolean addNonHumanCategory(String id, NonHumanCategory category) {
			...
			_log.warn("Could not add unknown nonhuman category to secondary resources: " + id);
			return false;
		}
		...
		private AbstractResource selectOneFromCategory(NonHumanCategory c, String subcat,
			...
			List<AbstractResource> categoryList = new ArrayList<AbstractResource>(c.getSubCategoryResources(subcat));
		...
		private String problemMsg(String type, String name, boolean multi) {
			...
			return String.format("%s %s has %s resources currently available.", type, name, constraint);
		}
	...
	class CategoryStack {
		...
		CategoryStack(NonHumanCategory category) {
			add(category);
		}
