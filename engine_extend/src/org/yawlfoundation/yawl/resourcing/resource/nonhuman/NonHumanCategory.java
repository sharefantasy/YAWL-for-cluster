...
/**
 * A NHR Category is a grouping class for a set of NHR (much like a Role groups
...
public class NonHumanCategory implements Comparable<NonHumanCategory> {
	...
	public int getSubCategoryCount() {
		return _subcategories.size();
	}

	public boolean addSubCategory(String subCategory) {
		return (!hasSubCategory(subCategory)) && _subcategories.add(new NonHumanSubCategory(subCategory));
	}
	...
	public Set<NonHumanResource> getSubCategoryResources(String subCategoryName) {
		if (subCategoryName != null) {
			...
			if (subCategory != null) {
				...
				return null; // no subcat of that name
		} else
			return getResources(); // no subcat name supplied means get all
	}
