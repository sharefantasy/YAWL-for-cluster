...
/**
 * The Resource Gateway class acts as a gateway between the Resource Service and
 * the external world for resource (org data) maintenance. It also initialises
 * the service with values from 'web.xml'.
 *
...
public class ResourceGateway extends YHttpServlet {
		...
		if (!ResourceManager.serviceInitialised) {
			try {
				...
				_rm.getClients().initClients(context.getInitParameter("InterfaceB_BackEnd"),
						context.getInitParameter("InterfaceX_BackEnd"), context.getInitParameter("InterfaceS_BackEnd"),
						context.getInitParameter("CostService_BackEnd"), context.getInitParameter("DocStore_BackEnd"));

				// set the path to external cluster classes (if any)
				...
				try {
					...
					_log.warn("ResourceGateway: Invalid integer value in web.xml" + " for OrgDataRefreshRate; value '"
				...
				// enable/disable blocking process when 2ndary resources
				// unavailable
				String blockOnMissingResources = context.getInitParameter("BlockOnUnavailableSecondaryResources");
				_rm.setBlockOnUnavailableSecondaryResources(getInitBooleanValue(blockOnMissingResources, false));
				...
				_rm.initBuildProperties(context.getResourceAsStream("/WEB-INF/classes/version.properties"));
				...
				try {
					...
					_log.warn("ResourceGateway: Invalid integer value in web.xml"
							+ " for GenerateRandomOrgData; value '" + generateOrgDataCount + "' will be ignored.");
				}
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if (action == null) {
			result = "<html><head>" + "<title>YAWL Resource Service</title>" + "</head><body>"
					+ "<H3>Welcome to the YAWL Resource Service \"Gateway\"</H3>"
					+ "<p> The Resource Gateway acts as a bridge between the Resource "
					+ "Service and the external world (it isn't meant to be browsed " + " to directly).</p>"
					+ "</body></html>";
	...
	public String doAddResourceAction(HttpServletRequest req, String action) {
		...
		if (action.equalsIgnoreCase("addParticipant")) {
			...
			if ((userid != null) && (!_rm.isKnownUserID(userid))) {
				...
				if (encrypt != null) {
					p.setPassword(req.getParameter("password"), encrypt.equalsIgnoreCase("true"));
				} else {
				...
				result = fail("Add", "Participant", userid);
			...
			if ((name != null) && (!getOrgDataSet().isKnownNonHumanResourceName(name))) {
				...
				result = fail("Add", "NonHumanResource", name);
			...
			if ((name != null) && (!getOrgDataSet().isKnownCapabilityName(name))) {
				...
				result = fail("Add", "Capability", name);
			...
			if (categoryName != null) {
				if (getOrgDataSet().getNonHumanCategoryByName(categoryName) == null) {
					...
					result = fail("Category '" + categoryName + "' already exists");
			} else
				result = fail("Category name is null");
			...
			result = success
					? "<success/>"
					: fail("Subcategory '" + subcategory + "' already exists OR category is invalid.");
		}
		return result;
	}
		...
		if (action.equalsIgnoreCase("updateParticipant")) {
			...
			if (pid != null) {
				...
				if (p != null) {
					...
					if (admin != null)
						p.setAdministrator(admin.equalsIgnoreCase("true"));
					String password = req.getParameter("password");
					if (password != null) {
						String encrypt = req.getParameter("encrypt");
						if (encrypt != null) {
			...
			if (id != null) {
				...
				if (resource != null) {
					...
					result = fail("NonHumanResource", id);
			} else
			...
			if (rid != null) {
				...
				if (role != null) {
					...
					if (ownerID != null)
						role.setOwnerRole(ownerID);
					role.save();
				} else
					result = fail("role", rid);
			} else
			...
			if (pid != null) {
				...
				if (position != null) {
					...
					if (orgGroupID != null)
						position.setOrgGroup(orgGroupID);
					position.save();
				} else
					result = fail("position", pid);
			} else
			...
			if (oid != null) {
				...
				if (orgGroup != null) {
					...
					if (groupType != null)
						orgGroup.setGroupType(groupType);
					String ownerID = req.getParameter("containinggroupid");
					if (ownerID != null)
						orgGroup.setBelongsTo(ownerID);
					orgGroup.save();
				} else
					result = fail("org group", oid);
			} else
			...
			if (cid != null) {
				...
				if (category != null) {
					...
					result = fail("non-human category", cid);
			} else
				result = fail("non-human category", null);
		}
		return result;
	}
	...
	public String doRemoveResourceAction(HttpServletRequest req, String action) {
		...
		if (action.equalsIgnoreCase("removeNonHumanResource")) {
			...
			result = success
					? "<success/>"
					: fail("Subcategory '" + subcategory + "' not found OR category id is invalid.");
			...
			result = success
					? "<success/>"
					: fail("Subcategory '" + subcategory + "' not found OR category name is invalid.");
		}
		return result;
	}
	...
	public String doGetResourceAction(HttpServletRequest req, String action) {
		...
		if (action.equalsIgnoreCase("getResourceConstraints")) {
			...
			if ((format != null) && format.equals("JSON")) {
				...
				result = stringMapToJSON(getOrgDataSet().getNonHumanCategoryIdentifiers(), callback);
			} else
				result = getOrgDataSet().getNonHumanCategoriesAsXML();
			...
			if (category != null) {
				...
				if ((format != null) && format.equals("JSON")) {
					...
					result = getOrgDataSet().getNonHumanSubCategoriesAsXML(category.getID());
			} else
				result = fail("Unknown category id: " + id);
			...
			if (category != null) {
				...
				result = fail("Unknown category name: " + categoryName);
			...
			if (p != null) {
				...
				result = (up != null) ? up.toXML() : fail("No privileges available for participant id: " + id);
			} else
				result = fail("Unknown participant id: " + id);
		}
		return result;
	}
		...
		if (action.equalsIgnoreCase("setContainingRole")) {
			...
			if (role != null) {
				...
				if (role.setOwnerRole(ownerID)) {
					...
					result = fail("containing role", ownerID);
			} else
				result = fail("role", roleID);
			...
			if (orgGroup != null) {
				...
				if (orgGroup.setBelongsTo(ownerID)) {
					...
					result = fail("containing org group", ownerID);
			} else
				result = fail("org group", groupID);
			...
			if (position != null) {
				...
				if (position.setReportsTo(ownerID)) {
					...
					result = fail("containing position", ownerID);
			} else
				result = fail("position", posID);
			...
			if (pid != null) {
				...
				if (p != null) {
					...
					if (bits != null) {
						...
						result = fail("No privileges received");
				} else
					result = fail("No participant found with id: " + pid);
			} else
				result = fail("Null participant id");
		}
		return result;
	}
	...
	private void updateCommonFields(AbstractResourceAttribute resource, HttpServletRequest req) {
		String desc = req.getParameter("description");
		if (desc != null)
			resource.setDescription(desc);
		String notes = req.getParameter("notes");
	...
	private String fail(String name, String id) {
		return (id == null)
				? fail(String.format("Unrecognised or null %s id.", name))
