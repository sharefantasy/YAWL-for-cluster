...
/**
 * The WorkQueue Gateway provides a gateway (or a set of API) between the
 * Resource Service and the participant workqueue jsps.
 *
 * @author Michael Adams v0.1, 13/08/2007
 *         <p/>
...
public class WorkQueueGateway extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		if (action == null) {
			if (gadgetPostback) {
				...
				result = "<html><head>" + "<title>YAWL Resource Service WorkQueue Gateway</title>" + "</head><body>"
						...
						+ "(it isn't meant to be browsed to directly).</p>" + "</body></html>";
			}
			...
			result = _rm.login(userid, password, req.getSession().getId()); // user
																			// connect
		} else if (action.equalsIgnoreCase("checkConnection")) {
			result = _rm.checkServiceConnection(handle) ? success : fail("Invalid or disconnected session handle");
		} else if (action.equals("isValidUserSession")) {
			result = _rm.isValidUserSession(handle) ? success : fail("Invalid or disconnected session handle");
	...
	private String doAction(String action, HttpServletRequest req) throws IOException {
		...
		if (action.equals("getParticipantFromUserID")) {
			...
			if (p != null) {
				...
				result = (up != null) ? up.toXML() : fail("No privileges available for participant id: " + pid);
			} else
				result = fail("Unknown participant id: " + pid);
		} else if (action.equals("getTaskPrivileges")) {
			TaskPrivileges privileges = _rm.getTaskPrivileges(itemid);
			result = (privileges != null) ? privileges.toXML(true) : fail("Unknown workitem id: " + itemid);
		} else if (action.equals("getParticipantsReportingTo")) {
			Set<Participant> set = getOrgDataSet().getParticipantsReportingTo(pid);
			result = (set != null)
					? _marshaller.marshallParticipants(set)
			...
			if (og != null) {
				Set<Participant> set = getOrgDataSet().getOrgGroupMembers(og); // set
																				// never
																				// null
				result = _marshaller.marshallParticipants(set);
			} else
				result = fail("Unknown org group id: " + groupid);
			...
			if (getOrgDataSet().isKnownRole(rid)) {
				...
				result = fail("Unknown role id: " + rid);
			...
			result = (set != null) ? _marshaller.marshallParticipants(set) : fail("No participants found");
		} else if (action.equals("getDistributionSet")) {
			Set<Participant> set = _rm.getDistributionSet(itemid);
			result = (set != null) ? _marshaller.marshallParticipants(set) : fail("No distribution set found");
			...
			result = (children != null)
					? _marshaller.marshallWorkItemRecords(children)
			...
			if (WorkQueue.isValidQueueType(queueType)) {
				...
				result = fail("Invalid queue type: " + req.getParameter("queue"));
			...
			SpecificationData specData = _rm.getSpecData(new YSpecificationID(specid, specversion, specuri));
			result = _marshaller.marshallSpecificationData(specData);
		} else if (action.equals("getRunningCases")) {
			result = _rm.getClients().getRunningCases(new YSpecificationID(specid, specversion, specuri));
			...
			if (wir != null) {
				...
				result = response(fail("Malformed or empty work item XML record"));
		...
		// the following calls are convenience pass-throughs to engine
		// interfaces A & B
			...
			result = response(_rm.launchCase(new YSpecificationID(specid, specversion, specuri), caseData, handle));
	...
	// returns true if the action can be performed without admin-level
	// privileges
	private boolean authorisedCustomFormAction(String action, String handle) {
		return (action.equals("getWorkItem") || action.equals("getWorkItemParameters")
				|| action.equals("updateWorkItemData") || action.equals("getWorkItemOutputOnlyParameters"))
	...
	private String doResourceAction(HttpServletRequest req, String action) {
		...
		if (p != null) {
			...
			if (wir != null) {
				...
				result = fail("Unknown workitem: " + itemid);
		} else
			result = fail("Unknown participant: " + pid);
	...
	private String doAdminQueueAction(HttpServletRequest req, String action) {
		...
		if (pids != null) {
			...
			if (wir != null) {
				...
				if (action.startsWith("Re")) {
					...
					if (wir.hasResourceStatus(WorkItemRecord.statusResourceUnoffered)) {
						result = _rm.assignUnofferedItem(wir, pids, action)
								? success
			...
			result = fail("Missing or empty offer set.");
	...
	private String doResourceMoveAction(HttpServletRequest req, String action) {
		...
		if (pOrig != null) {
			...
			if (pDest != null) {
				...
				result = fail("Unknown destination participant: " + pDest);
		} else
			result = fail("Unknown source participant: " + pFrom);

		return result;
	}

	private String doResourceAction(Participant p, WorkItemRecord wir, HttpServletRequest req, String action) {
		...
		if (action.equals("acceptOffer")) {
			...
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceAllocated)) {
				...
				result = fail("Allocated", "piled", wir);
		} else if (action.equals("suspendWorkItem")) {
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceStarted)) {
				successful = _rm.suspendWorkItem(p, wir);
				result = successful ? success : fail("Could not suspend workitem: " + wir.getID());
			} else
				result = fail("Started", "suspended", wir);
		} else if (action.equals("unsuspendWorkItem")) {
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceSuspended)) {
				successful = _rm.unsuspendWorkItem(p, wir);
				result = successful ? success : fail("Could not unsuspend workitem: " + wir.getID());
			} else
				result = fail("Suspended", "unsuspended", wir);
		} else if (action.equals("completeWorkItem")) {
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceStarted)) {
				result = _rm.checkinItem(p, wir);
			} else
				result = fail("Started", "completed", wir);
		}
		return result;
	}

	private String doResourceAction(Participant pOrig, Participant pDest, WorkItemRecord wir, String action) {
		String result = "<failure/>";
		boolean successful;
		...
		if (action.equals("delegateWorkItem")) {
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceAllocated)) {
				...
				result = successful ? success : fail("Could not delegate workitem: " + wir.getID());
			} else
				result = fail("Allocated", "delegated", wir);
		} else if (action.equals("reallocateStatefulWorkItem")) {
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceStarted)) {
				successful = _rm.reallocateStatefulWorkItem(pOrig, pDest, wir);
				result = successful ? success : fail("Could not reallocate workitem: " + wir.getID());
			} else
				result = fail("Started", "reallocated", wir);
		} else if (action.equals("reallocateStatelessWorkItem")) {
			if (wir.hasResourceStatus(WorkItemRecord.statusResourceStarted)) {
	...
	private String fail(String reqStatus, String action, String itemid, String hasStatus) {
		return fail(String.format("Only a workitem with '%s' status can be %s. Workitem '%s' has '%s' status.",
