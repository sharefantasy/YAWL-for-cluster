...
/**
 * Executes a codelet in a separate thread, and announces its completion when
 * done.
 *
	...
	public void run() {
		...
		try {
			...
			if (_codelet != null) {
				...
				throw new CodeletExecutionException("Codelet '" + codeletName + "' could not be located.");
		} catch (Exception e) {
			_log.error(MessageFormat.format(
					"Exception executing codelet ''{0}'': {1}. "
