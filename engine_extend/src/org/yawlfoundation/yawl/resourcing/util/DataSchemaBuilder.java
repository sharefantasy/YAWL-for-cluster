...
/**
 * Expands a data schema for a case start or a task to its base xsd types for
 * use in the definition of a dynamic form.
 *
 * Author: Michael Adams Creation Date: 5/04/2010
 */
public class DataSchemaBuilder {
	 ...
	 * 
	 * @param schemaMap
	 *            a map of user defined types of this particular specification
	 */
	...
	/**
	 * Constructs the expanded data schema, using the user-defined-types map
	 * passed in via the constructor
	 * 
	 * @param specData
	 *            the SpecificationData object for the spec to construct a
	 *            schema for
	 ...
	 * 
	 * @param taskInfo
	 *            the TaskInformation object for the task to construct a schema
	 *            for
	 * @return the constructed schema (as a string)
	 */
	 ...
	 * 
	 * @param rootName
	 *            the name of the root element (task or root net name)
	 * @param parameters
	 *            the List of parameters to build the schema for
	 * @return the constructed schema (as a string)
	 */
	 ...
	 * 
	 * @param rootName
	 *            the name to give to the root element
	 * @param varName
	 *            the name to give to the data element
	 * @param dataType
	 *            the datatype for the data element
	 * @return a schema for the datatype that variables of the name supplied can
	 ...
	 * 
	 * @param param
	 *            the parameter to construct a schema for
	 * @param defNS
	...
	private Element createParamElement(YVariable param, Namespace defNS) {
		...
		// simple types are defined by attribute, user-defined types are defined
		// by
		// sub elements
	 ...
	 * 
	 * @param param
	 *            the parameter with a user-defined type definition
	 * @param base
	 *            the base parameter element
	 * @param defNS
	 *            the default namespace
	...
	private Element createComplexType(YVariable param, Element base, Namespace defNS) {
		...
		if (URI != null) {
			addNamespace(URI, param.getDataTypePrefix()); // add any new
															// namespace
		}
	...
	/**
	 * Clones a user-defined type definition to use within a schema. May be
	 * called recursively.
	 * 
	 * @param base
	 *            the 'parent' element this type is defined for
	 * @param type
	...
	private Element cloneUserDefinedType(Element base, String type, Namespace defNS) {
		...
		// set the first element name to the name of the udt's element (eg.
		// sequence)
	 ...
	 * 
	 * @param base
	 *            the 'parent' element this content will form the schema for
	 * @param toCopy
	...
	private Element cloneContent(Element base, Element toCopy, Namespace defNS) {
		...
		for (Element child : toCopy.getChildren()) {
			...
			if (type != null) {
				...
				// if there's a type attribute and its a simple type, set it for
				// the clone
				if (isXSDType(type)) {
			...
			// recurse to process this element's children (til there are no more
			// children)
	 ...
	 * parental attachment to the attribute; and (ii) to fix any errant
	 * namespace prefixes
	 * 
	 * @param element
	 *            the element with the attributes to clone
	 * @param defNS
	 *            the default namespace
	...
	/**
	 * Gets the value of an element's type attribute with the namespace prefix
	 * removed
	 * 
	 * @param e
	 *            the element with the type attribute
