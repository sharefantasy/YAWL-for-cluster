...
/**
 * Author: Michael Adams Creation Date: 27/08/2009
 */
public class GadgetFeeder {
	...
	private final String _script = "<script type=\"text/javascript\">if (window.location.hash) { "
	...
	private String checkCredentials(Participant p) {
		...
		if (p != null) {
			...
			errMsg = "Unknown Userid: " + _userid;

		return errMsg;
	}

	private String getErrBody(String msg) {
		return "<div id=\"errMsg\" style=\"color:red;font-size:16px;font-style:bold\">" + msg + "</div>";
	}
	...
	private String getLibs() {
		...
		for (String lib : libs) {
			result.append("<script type=\"text/javascript\" src=\"http://www.google.com/ig/f/").append(lib)
					.append("\"></script>\n");
		}
	...
	private String getQueueContents(WorkQueue queue, int qType) {
		...
		if (queue != null) {
			...
			SortedSet<WorkItemRecord> wirSet = new TreeSet<WorkItemRecord>(new WorkItemAgeComparator());
			wirSet.addAll(queue.getAll());
			for (WorkItemRecord wir : wirSet) {
				result.append("<tr><td><a href=\"").append(_rootURI).append("resourceService/faces/rssFormViewer.jsp")
						.append(getParams(wir.getID())).append("\" target=\"_blank\">").append(wir.getIDForDisplay())
						.append("</a></td></tr>");
			}
			result.append("</table>");
		}
	...
	private String getParams(String itemid) {
		...
		result.append(ServletUtils.urlEncode(itemid)).append("&amp;userid=").append(ServletUtils.urlEncode(_userid))
				.append("&amp;password=").append(ServletUtils.urlEncode(_encryptedPassword)).append("&amp;parent=")
