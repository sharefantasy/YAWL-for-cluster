...
public class YDecompositionParser {
	 ...
	 * 
	 * @param decompElem
	 *            the XML doclet containing the decomp configuration.
	 * @param specificationParser
	 *            a reference to parent spec parser.
	 * @param version
	 *            the version of XML structure
	 */
	YDecompositionParser(Element decompElem, YSpecificationParser specificationParser, YSchemaVersion version) {
	...
	private YDecomposition createDecomposition(Element decompElem) {
		...
		/**
		 * AJH: Added to support XML attribute pass-thru from specification into
		 * task output data doclet. Load element attributes
		 */
		for (Attribute attr : decompElem.getAttributes()) {
			String attname = attr.getName();
			boolean isXsiNS = attr.getNamespace() == schemaInstanceNS;

			// don't add the standard YAWL schema attributes to the pass through
			// list.
	...
	private YTask parseTask(Element taskElem) {
		...
		if ("NetFactsType".equals(decompositionType)) {
			task = new YCompositeTask(taskID, splitJoinTypes[1], splitJoinTypes[0], (YNet) _decomposition);
		} else if ("WebServiceGatewayFactsType".equals(decompositionType) || decomposesToID == null || // empty
																										// task
																										// or
																										// no
																										// decomp
			...
			task = new YAtomicTask(taskID, splitJoinTypes[1], splitJoinTypes[0], (YNet) _decomposition);
		}
	...
	private void parseExternalTaskRoles(Element externalTaskElem, YTask externalTask) {
		...
		if ("MultipleInstanceExternalTaskFactsType".equals(taskType)) {
			...
			String remoteVarName = externalTaskElem.getChild("miDataInput", _yawlNS).getChildText("formalInputParam",
					_yawlNS);
	...
	private List<Element> parseRemoveSetFromFlow(Element externalTaskElem) {
		List<Element> removeSetForFlows = externalTaskElem.getChildren("removesTokensFromFlow", _yawlNS);
	...
	private YCondition parseCondition(YNet aNet, Element conditionElem) {
		...
		if ("condition".equals(conditionType)) {
			...
			condition = new YCondition(id, aNet); // should never hit this
	...
	private void parseCustomFormURL(YTask task, Element taskElem) {
		...
		if (formStr != null) {
			try {
				...
				Logger.getLogger(this.getClass()).error("Invalid custom form URL in specification for task "
						+ task.getID() + ": " + formStr + ". Custom form URL will be ignored.");
			}
		}
	}
	...
	private List<FlowStruct> parsePostset(Element netElementElem) {
		...
		for (Element flowsIntoElem : netElementElem.getChildren("flowsInto", _yawlNS)) {
			...
			FlowStruct flowStruct = new FlowStruct(nextElementRef, flowsIntoElem.getChildText("predicate", _yawlNS),
	...
	private YNet parseNet(YNet net, Element netElem) {
		if (!isBeta2Version()) {
			...
			if ("true".equals(isRootNet)) {
				...
				if (gateway != null)
					net.setExternalDataGateway(gateway);
			}
		}
	...
	/**
	 * Adds variable specific information to the YParameter argument (in &amp;
	 * out var).
	 * 
	...
	public static void parseLocalVariable(Element localVariableElem, YVariable variable, Namespace ns,
		...
		if (version2) {
			...
			if (null != dataType) {
				...
				// if data type is of QName form eliminate the first bit as it
				// is useless for
				// version 2 anyway
		...
		// the variable either is data typed xor linked to an element
		// declaration
	...
	/**
	 * Adds parameter specific information to the YParameter argument (in &amp;
	 * out var).
	 * 
	 * @param paramElem
	 *            the XML data
	...
	public static void parseParameter(Element paramElem, YParameter parameter, Namespace ns, boolean version2) {
		...
		String defaultValue = JDOMUtil.decodeEscapes(paramElem.getChildText("defaultValue", ns));
	...
	private void parseExternalInteraction(YDecomposition decomposition, Element decompElem) {
		String interactionStr = decompElem.getChildText("externalInteraction", _yawlNS);
		if (interactionStr != null)
			decomposition.setExternalInteraction(interactionStr.equalsIgnoreCase("manual"));
	}
	...
	private void parseCodelet(YDecomposition decomposition, Element decompElem) {
		...
		if (codelet != null)
			decomposition.setCodelet(codelet);
	}
					...
					for (YExternalNetElement maybeImplicit : flowSource.getPostsetElements()) {
						if (((YCondition) maybeImplicit).isImplicit()) {
							...
							if (taskAfterImplicit.equals(flowDest)) {
								List<YExternalNetElement> additionalRemoves = new ArrayList<YExternalNetElement>();
			...
			for (String currentElementID : elementIDs) {
				...
				if (currentNetElement instanceof YTask) {
					for (FlowStruct flowStruct : _postsetMap.get(currentElementID)) {
						...
						// if one task flows directly to another task then we
						// create an
						// anonymous condition and insert it between the two
						// tasks.
						if (netElement instanceof YTask) {
							String anonID = makeAnonID(currentNetElement.getID(), netElement.getID());
							YCondition condition = new YCondition(anonID, (YNet) _decomposition);
