...
public abstract class AbstractCheckSumTask extends Task {
	...
	protected boolean shouldBeIncluded(File f) {
		return !shouldBeExcluded(f) && (_antIncludes.isEmpty() || containsMatch(f, _antIncludes));
	}
	...
	private void createOutputXML(String rootDir, String toDir, String file) throws IOException {
		if (rootDir == null)
		...
		if (toDir == null)
			...
			toDir = rootDir + "/" + toDir;
		if (file == null)
			file = DEFAULT_TO_FILE;
		if (_antIncludes == null)
			_antIncludes = Collections.emptyList();
		if (_antExcludes == null)
			_antExcludes = Collections.emptyList();
