...
/**
 * Some static utility methods for converting JDOM Documents and Elements to
 * Strings and files & vice versa.
 *
 * @author Michael Adams 04/07/2006
 *
 *         Last date: 22/06/08
 */
...
public class JDOMUtil {
	...
	public synchronized static Document fileToDocument(File file) {
		try {
			...
			_log.error("JDOMException loading file into Document, filepath = " + file.getAbsolutePath(), jde);
		} catch (IOException ioe) {
			_log.error("IOException loading file into Document, filepath = " + file.getAbsolutePath(), ioe);
		}
		return null;
	}
	...
	public static String getDefaultValueForType(String dataType) {
		if (dataType == null)
			...
			return "false";
		else if (dataType.equalsIgnoreCase("string") || (!XSDType.isBuiltInType(dataType)))
			return "";
		else
			return "0";
	}
	...
	public static String decodeEscapes(String s) {
		if ((s == null) || (s.indexOf('&') < 0))
			return s; // short circuit if no encodes
