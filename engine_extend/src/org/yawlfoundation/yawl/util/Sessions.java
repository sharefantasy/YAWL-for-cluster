...
/**
 * A generic session manager utility that can be used by custom services to
 * allow connections from any service or client application that has been
...
public class Sessions {
	...
	private Map<String, ScheduledFuture> handleToTimer; // sessionhandle <->
														// activity timer
	...
	public Sessions() {
		...
		credentials.put("admin", PasswordEncryptor.encrypt("YAWL", "YAWL")); // def.
																				// user
	}

	/**
	 * Constructs a new Sessions object, and uses the parameters passed to
	 * connect to the engine via Interface A so that the registered services'
	 * and client applications' credentials can be retrieved.
	 * 
	 * @param iaURI
	 *            The URI of the engine's Interface A
	 * @param iaUserid
	 *            the userid of the registered service or client app calling
	 *            this method
	 * @param iaPassword
	 *            the password of the registered service or client app calling
	 *            this method
	 */
	...
	/**
	 * Uses the parameters passed to connect to the engine via Interface A so
	 * that the registered services' and client applications' credentials can be
	 * retrieved. Note this method must be called before any sessions can be
	 * authenticated
	 * 
	 * @param iaURI
	 *            The URI of the engine's Interface A
	...
	/**
	 * Cancels all current sessions and their inactivity timers. Usually called
	 * from Servlet#destroy
	 */
	 ...
	 * 
	 * @param userid
	 *            the userid of a registered service or client application
	 * @param password
	 *            the corresponding password
	 * @return a session handle if successful, or a diagnostic error if not
	 *         (incl. unknown userid, invalid password, invalid or missing
	 *         Interface A credentials, or other problems connecting to the
	 *         engine or retrieving registered credentials)
	 */
	...
	/**
	 * Checks that a session handle is valid and active (i.e. has not timed out
	 * through inactivity)
	 * 
	 * @param handle
	...
	/**
	 * Removes a userid/password pair from the local cache (e.g. in the event
	 * that the password has been changed in the engine)
	 * 
	 * @param userid
	 *            the userid to clear
	 ...
	 * 
	 * @param userid
	 *            the userid the create the handle for. It is assumed that the
	 ...
	 * 
	 * @param handle
	 *            the session handle to restart the timer for
	 */
	...
	/**
	 * Starts an inactivity timer for a session handle. The handle will expire
	 * after 60 minutes of inactivity
	 * 
	 * @param handle
	 *            the session handle to start the timer for
	 */
	private void startActivityTimer(final String handle) {
		final ScheduledFuture<?> inactivityTimer = scheduler.schedule(new Runnable() {
			public void run() {
				disconnect(handle);
			}
		}, 60, TimeUnit.MINUTES);
	 ...
	 * 
	 * @param handle
	 *            the session handle to cancel the timer for
	 */
	...
	/**
	 * Creates a client to the engine via IA to obtain logon credentials for
	 * custom services and client application registered with the engine. A
	 * registered client is accepted as a valid client of the service using this
	 * class.
	 */
	class InterfaceAClient {
		...
		/**
		 * Creates an instance of this class, and sets the uri of engine's
		 * Interface A and the logon credentials that can be used to connect via
		 * that interface
		 * 
		 * @param uri
		 *            the uri of engine's Interface A
		 * @param userid
		 *            a userid registered as a custom service or client
		...
		/**
		 * Gets the password from the engine for the userid passed, and if
		 * successful adds it to the local cache
		 * 
		 * @param userid
		 *            the userid to get the password for
		 * @return true if the password is already in the local cache, or if it
		 *         is successfully retrieved; false if otherwise
		 * @throws IOException
		 *             if there's a problem connecting to the engine, or if
		 *             there's some other problem getting the password from the
		 *             engine
		 */
		 ...
		 * 
		 * @param userid
		 *            the userid of the service to get
		...
		/**
		 * Checks if the IA URI and credentials are available, then if there is
		 * an active IA connection and, if not, creates one
		 * 
		 * @throws IOException
		 *             if the URI or credentials are missing, or if there's a
		...
		void checkConnection() throws IOException {
			...
			// if we have no handle, or if it has expired, establish a new
			// connection
			if ((iaHandle == null) || (!iaClient.successful(iaClient.checkConnection(iaHandle)))) {
