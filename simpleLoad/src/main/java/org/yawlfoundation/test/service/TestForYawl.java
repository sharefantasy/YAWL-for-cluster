...
public class TestForYawl extends InterfaceBWebsideController {
	...
	/*
	 * static Timer timer; static { timer = new Timer();
	 * 
	 * TimerTask task = new TimerTask() {
	 * 
	 * @Override public void run() { _logger.info("engine load: " +
	 * workitemCounter.getReportCounter()); } }; timer.schedule(task,0, 1000); }
	 */
	public TestForYawl() {
		_logger.info("counter started, connected to ");
	}
	...
	public void handleEnabledWorkItemEvent(WorkItemRecord wir) {
		try {
			...
			this.checkInWorkItem(wir.getID(), wir.getDataList(), this.getOutputData(wir.getTaskID(), ioe),
					(String) null, this._handle);
	...
	public YParameter[] describeRequiredParams() {
		// YParameter[] params = new YParameter[]{new
		// YParameter((YDecomposition)null, 0), null};
