...
/**
 * Allows XES cluster.event nodes within a trace to be sorted based on timestamp
 * <p/>
...
public class XESTimestampComparator implements Comparator<XNode> {
    ...
    public int compare(XNode node1, XNode node2) {
        ...
        // if either node is not an 'cluster.event' node, return it as having precedence
        if (!node1.getName().equals("cluster.event")) return -1;
        if (!node2.getName().equals("cluster.event")) return 1;
        ...
        // if identical timestamps, return the longer id (i.e. the child item cluster.event)
