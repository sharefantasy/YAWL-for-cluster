...
public class YEventLogger {
    ...
    /**
     * Get a reference to the cluster.event logger object. This one is called by the engine on
     * startup.
     * @param engine a reference to the running engine
     * @return an instantiated cluster.event logger
     */
    ...
    /* enables cluster.event logging (the default) */
    public void enable() { _enabled = true; }

    /* disables cluster.event logging */
     ...
     * @param datalist a list of data entries to log with this cluster.event
    ...
    /**
     * Logs a workitem cluster.event (change of status).
     * @param pmgr the active persistence manager object
     * @param workItem the workitem that triggered the cluster.event
     * @param eventName the cluster.event that has occurred
     * @param datalist a list of data entries to log with this cluster.event
     */
    public void logWorkItemEvent(YPersistenceManager pmgr, YWorkItem workItem, String eventName,
        ...
        if (loggingEnabled()) {
            try {
                ...
                _log.error(getWarnMsg("workitem cluster.event"), ype);
            }
        }
    }
     ...
     * @param pmgr the active persistence manager object
     * @param workItem the workitem that triggered the cluster.event
     * @param event the cluster.event that has occurred
     * @param datalist a list of data entries to log with this cluster.event
     */
    ...
    public void logDataEvent(YPersistenceManager pmgr, YWorkItem workitem, String descriptor,
        ...
        if (loggingEnabled() && (datalist.size() > 0)) {
            try {
                ...
                _log.error(getWarnMsg("data cluster.event"), ype);
            }
        }
    }
    ...
    /**
     * Logs an cluster.event and its associated data items
     * @param pmgr the active persistence manager object
     * @param instanceID a foreign key to either the net instance or the task instance
     * that generated the cluster.event
     * @param descriptor a label that describes the kind of cluster.event
     * @param datalist the list of data items to insert
     * @param serviceID a foreign key to the client service initiating the cluster.event
     * @param rootNetInstanceID a foreign key to the root net instance that (eventually)
     * encapsulates this cluster.event
     * @throws YPersistenceException if there's a problem with the persistence layer
     */
    ...
    /**
     * Inserts a new cluster.event record
     * @param pmgr the active persistence manager object
     * @param instanceID a foreign key to either the net instance or the task instance
     * that generated the cluster.event
     * @param descriptor a label that describes the kind of cluster.event
     * @param serviceID a foreign key to the service that triggered the cluster.event
     * @param rootNetInstanceID a foreign key to the root net instance that (eventually)
     * encapsulates this cluster.event
     * @return the primary key of the inserted record
     * @throws YPersistenceException if there's a problem with the persistence layer
    ...
    /**
     * Inserts a list of data items records for an cluster.event
     * @param pmgr the active persistence manager object
     * @param eventID a foreign key to the cluster.event record associated with the data items
     * @param datalist the list of data items to insert
     * @throws YPersistenceException if there's a problem with the persistence layer
     */
